...
u8 homeRFInit(void)
{
    ...
    memset(homeRFCmdQueue, 0, sizeof(HOMERF_CMD_QUEUE)*HOMERF_QUEUE_SIZE);
        	...
        	if((gHomeRFSensorList->sSensor[i].type == HOMERF_DEVICE_SIREN) || (gHomeRFSensorList->sSensor[i].type == HOMERF_DEVICE_TEMP_HYG))
        	{
		        ...
		        {      
					...
					if(gHomeRFSensorList->sSensor[i].pushOnOff == HOMERF_SENSOR_ON)
					{
						...
						sysback_Net_SetEvt(SYS_BACKRF_NTE_SEND_EVENT, gHomeRFSensorList->sSensor[i].sID,EVENT_SIREN);   
					}
					gHomeRFSensorList->sSensor[i].battery=0;
					gHomeRFSensorList->sSensor[i].lifeCount = 0;
					homeRFWriteLog(i, HOMERF_MARS_TYPE_SIREN, 0, HOMERF_EVENT_NONE, EventID_Unlink, 0);  //Lost Link Log			
				}
        	}
        	else
        	{
...
void homeRFWriteToFlash(u8* bufAddr, u8 type)
{
    HOMERF_ROOM_DATA* tempRoom;
...
u8  homeRFCheckPID(u16 sensorID, u64 pairID, u8 *idx)
{
    ...
    for(i=0; i< HOMERF_SENSOR_MAX; i++)
    {
        if(gHomeRFSensorList->sSensor[i].sID == sensorID) 
        {   
            gHomeRFSensorID=sensorID;
            *idx=i;
            return HOMERF_PAIR_SAME;
...
void homeRFHandleEvent(u8 *cmdStr)
{
    ...
    //printf("\x1B[96madc_val=%x\x1B[0m\n",adc_val);
    ...
    if(type == HOMERF_MARS_TYPE_DOOR)
    {
        if(status & 0x80 ) /* trigger bit*/
        {
            if(status & 0x01) /* high */
            {
				...
				{
					sysback_Net_SetEvt(SYS_BACKRF_NTE_SEND_EVENT, gHomeRFSensorList->sSensor[idx].sID,EVENT_DOOR); 
#if CLOUD_SUPPORT			
    ...
    else if(type == HOMERF_MARS_TYPE_PIR)
    {
        if(status & 0x80 ) /* trigger bit*/
        {
            ...
            if(gHomeRFSensorList->sSensor[idx].pushOnOff == HOMERF_SENSOR_ON)
			{
				sysback_Net_SetEvt(SYS_BACKRF_NTE_SEND_EVENT, gHomeRFSensorList->sSensor[idx].sID,EVENT_PIR);	  
#if CLOUD_SUPPORT	
				//sysback_Net_SetEvt(SYS_BACKRF_NTE_SEND_CLOUD,gHomeRFSensorList->sSensor[idx].sID, TARGET_EVENT);
#endif					
			}
...
/*******************************Panasonic****************************End***/
    ...
    else if(type == HOMERF_MARS_TYPE_REMOTE)
    {
        if(subType == HOMERF_SUB_TYPE_TRANWO_REMOTE)
        {
            ...
            else if(keyTrigger & 0x08)
            {
				...
				homeRFSentEvent2UI(idx);
                sysback_Net_SetEvt(SYS_BACKRF_NTE_SEND_EVENT, gHomeRFSensorList->sSensor[idx].sID,EVENT_SOS); //2017525 Sean
            }
    ...
    else if((type == HOMERF_MARS_TYPE_TEMP) || (type == HOMERF_MARS_TYPE_TEMP_HUMIDITY))	//sync per 30 sec.
    {
        ...
        if(status & 0x80 ) /* trigger bit*/
        {
            if((gHomeRFSensorList->sSensor[idx].data.Temp.alarmSwitch == 1) && ((count%2) == 0)) /*high temp*/
            {
                gHomeRFSensorList->sSensor[idx].status |= HOMERF_SENSOR_STATUS_ALARM;            
            }
            else if((gHomeRFSensorList->sSensor[idx].data.Temp.alarmSwitch == 2) && ((count%2) == 1)) /*low temp*/
            {
                gHomeRFSensorList->sSensor[idx].status |= HOMERF_SENSOR_STATUS_ALARM;      
            }
        }     
        gHomeRFSensorList->sSensor[idx].data.Temp.value = status;
        count++;
	#else
		...
		if(((gHomeRFSensorList->sSensor[idx].data.Temp_HYG.Thermometer_H < gHomeRFSensorList->sSensor[idx].data.Temp_HYG.Temp_alarm_L) ||\
		   (gHomeRFSensorList->sSensor[idx].data.Temp_HYG.Thermometer_H > gHomeRFSensorList->sSensor[idx].data.Temp_HYG.Temp_alarm_H)) &&\
		   ((gHomeRFSensorList->sSensor[idx].data.Temp_HYG.Temp_alarm_H != 0) && (gHomeRFSensorList->sSensor[idx].data.Temp_HYG.Temp_alarm_L != 0) &&\
		   (gHomeRFSensorList->sSensor[idx].data.Temp_HYG.Humi_alarm_H != 0)))
		{
			...
			if(gHomeRFSensorList->sSensor[idx].pushOnOff == HOMERF_SENSOR_ON)
			{
				sysback_Net_SetEvt(SYS_BACKRF_NTE_SEND_EVENT, gHomeRFSensorList->sSensor[idx].sID,0);					
			}
#if(HOME_RF_OPTION == HOME_SENSOR_TRANWO)                
			if(gHomeRFSensorList->sSensor[idx].sirenOnOff == HOMERF_SENSOR_ON)
#endif  
    ...
    else if(type == HOMERF_MARS_TYPE_SIREN)
    {
		...
		if(status & 0x02) /* Tamper Switch Open*/
		{
			...
			if(((current_time - Siren_trigger_time) > 1200) || (Siren_trigger_time==0) || (status & 0x80)) // 1min
			{
				...
				sysback_Net_SetEvt(SYS_BACKRF_NTE_SEND_EVENT, gHomeRFSensorList->sSensor[idx].sID,EVENT_SIREN);   
			}
		}  
