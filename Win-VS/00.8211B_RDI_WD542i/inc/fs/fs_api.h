...
typedef struct {
  FS_u32 fileid_lo;          /* unique id for file (lo)      */	/* cytsai: unit where file is located */
  FS_u32 fileid_hi;          /* unique id for file (hi)      */	/* cytsai: first cluster number of file */
  FS_u32 fileid_ex;          /* unique id for file (ex)      */	/* cytsai: first cluster number of directory where file is located */
  FS_u32 FileEntrySect;                                      // Lucian: Optimize for fclose; record the location(sector) of file entry.
  FS_i32 EOFClust;           /* used by FAT FSL only         */
  FS_u32 CurClust;           /* used by FAT FSL only         */
  FS_i32 filepos;            /* current position in file     */	/* cytsai: current position of file */
...
struct FS_DIRENT {
    ...
    char      used_flag;
   #endif    
    unsigned short FileEntrySect;           //Lucian: recording file-entry location. (sector unit)
    unsigned short FileEntrySectOffset;
   
    unsigned short fsFileCreateTime_HMS;
    unsigned short fsFileCreateDate_YMD;

    unsigned short fsFileModifiedTime_HMS;
    unsigned short fsFileModifiedDate_YMD;
...
typedef struct {
  ...
  FS_u32 dirid_lo;           /* unique id for file (lo)      */	/* cytsai: unit where directory is located */
  FS_u32 dirid_hi;           /* unique id for file (hi)      */	/* cytsai: first cluster number of directory */
  FS_u32 dirid_ex;           /* unique id for file (ex)      */	/* cytsai: first cluster number of parent directory */
  FS_i32 dirpos;             /* current position in file     */	/* cytsai: FDB position in cluster of directory, dirpos = 0, 32, 64,... */
  FS_i32 size;               /* size of file                 */ /* cytsai: in unit sector */
  int dev_index;             /* index in _FS_devinfo[]       */
  FS_i16 error;              /* error code                   */
  unsigned char inuse;       /* handle in use mark           */
} FS_DIR;
...
#else
int FS_Remove(const char *pFileName,unsigned short EstFileEntrySect);
int _FS_Remove(const char *pFileName,unsigned short EstFileEntrySect, unsigned long *used_cluster, FS_DISKFREE_T *gdiskInfo);
#endif
