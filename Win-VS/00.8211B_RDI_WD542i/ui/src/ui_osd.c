...
u16 uiOsdClearArea(u16 now_x, u16 now_y, u16 width , u16 height, u8 clear, u8 buf)
{
    static u16 last_x = 0,last_y = 0;
    static u16 draw_width = 0 , draw_height = 0;
    ...
    if (clear == 0)  /*開始統計繪圖大小 */
    {
         ...
         if (now_x  < last_x)  /*設定 X 起始點*/
            last_x = now_x;

         if (now_y < last_y) /*設定 Y 起始點*/
            last_y = now_y;

         if (now_x+width > last_x+last_width)  /*取得最大X繪圖範圍*/
            draw_width = now_x+ width - last_x;

         if (now_y+height > last_y+last_height)  /*取得最大Y繪圖範圍*/
            draw_height = now_y+ height - last_y;


        //  DEBUG_UI ("Draw_width  %d  draw_height %d   ",draw_width , draw_height);
        //  DEBUG_UI ("start_x  %d  start_y %d\r\n",   last_x,last_y);
            last_width  = draw_width ;
            last_height = draw_height  ;

    }
    else   /*復原 . 等待下一次統計 */
    {
        DEBUG_UI( " Clear Rage  width %d  height %d  pos_x %d pos_y %d  buffers %d \r\n",draw_width , draw_height ,last_x , last_y , buf );
        uiMenuOSDFrame(osd_w, draw_width, draw_height, last_x, last_y, buf, 0);
        last_x = 0;
        last_y = 0;
        draw_width  = 0 ;
        draw_height = 0 ;
