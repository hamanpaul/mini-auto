...
#else
const INT32U gFlagDMAInt[DMA_ID_NUM]    = {FLAGDMA_INT_CH0, FLAGDMA_INT_CH1, FLAGDMA_INT_CH2, FLAGDMA_INT_CH3};

const INT32U gDMARegSrcAddr[DMA_ID_NUM] = {REG_DMACH0SRCADDR, REG_DMACH1SRCADDR, REG_DMACH2SRCADDR, REG_DMACH3SRCADDR};
const INT32U gDMARegDstAddr[DMA_ID_NUM] = {REG_DMACH0DSTADDR, REG_DMACH1DSTADDR, REG_DMACH2DSTADDR, REG_DMACH3DSTADDR};
const INT32U gDMARegCycCnt[DMA_ID_NUM]  = {REG_DMACH0CYCCNT, REG_DMACH1CYCCNT, REG_DMACH2CYCCNT, REG_DMACH3CYCCNT};
const INT32U gDMARegCmmd[DMA_ID_NUM]    = {REG_DMACH0CMD, REG_DMACH1CMD, REG_DMACH2CMD, REG_DMACH3CMD};
#endif
...
void marsDMAIntHandler(void)
{
    ...
    (CHIP_OPTION == CHIP_A1018A) || (CHIP_OPTION == CHIP_A1018B) || (CHIP_OPTION == CHIP_A1020A)||(CHIP_OPTION == CHIP_A1026A))
    #if 0
    //Non-stop mode, manual stop
    if(iisTrgSemEvt->OSEventCnt > 0)
    ...
    else
    {
        ...
        for(i=0; i<DMA_ID_NUM; i++)
        {
            if(CurDMAIntStat & uiDMAChkBit)
            {
                IisCtrl &= ~0x00000004;                 //IisCtrl &= ~IIS_RCV_ENA;
                IisCtrl  &= ~(0x00000020 | 0x00000002); //IisCtrl  &= ~(IIS_EXMT_ENA | IIS_XMT_ENA);
                *(volatile INT32U *)gDMARegCmmd[i]&= (~DMA_AUTONONSTOP_EN); //Auto Run Non-Stop mode
                OSFlagPost(gDMAFlagGrp, gFlagDMAInt_AUTO[i], OS_FLAG_SET, &err); // RPT_DMAi_Auto don't occur, so use the same flag.
            }
            uiDMAChkBit <<= 2;
        }
    }
...
void isr_marsDMA_StopAuto(INT32U uiDMAId)
{
    INT8U err;
    volatile INT32U *pDMARegSrcAddr, *pDMARegDstAddr, *pDMARegCycCnt, *pDMARegCmmd, *pDMARegAutoNum;
    volatile INT32U *pDMARegSrcAddr_alt, *pDMARegDstAddr_alt;
...
void isr_marsDMAAuto(int DmaID,u8* buf)
{
