...
u8	JOIN_DEFAULT_SSID[32]="";
u8	JOIN_DEFAULT_PSK[64]="";
u8	JOIN_DEFAULT_SSID_CHECK[32]="";
u8	JOIN_DEFAULT_PSK_CHECK[64]="";
...
OS_STK icommInitTaskStack[ICOMM_INIT_TASK_STACK_SIZE];
...
OS_FLAG_GRP  *gWifiScfgModeFlagGrp;
...
//printf("");
    ...
    gWifiScfgModeFlagGrp  = OSFlagCreate(0x00000000, &err);
    OSTaskCreate(ICOMM_INIT_TASK, (void *)0, ICOMM_INIT_TASK_STACK, ICOMM_INIT_TASK_PRIORITY);
    return 0;
}

void icomm_init_task(void* args)
{
	extern u8 WiFi_Mode;			// 1: STA Mode, 0: SCONFIG Mode.
	u8 mode;
sysDeadLockMonitor_OFF();

	printf("\x1B[96m JOIN_DEFAULT_SSID=%s, %d. \x1B[0m\n",JOIN_DEFAULT_SSID, strlen(JOIN_DEFAULT_SSID));
	//strcpy(JOIN_DEFAULT_SSID, "qwer");
	if(strlen(JOIN_DEFAULT_SSID) == 0)
	{
		WiFi_Mode = 0;
...
sysDeadLockMonitor_ON();
	printf("Init icomm 6030p %d Complete.\n",mode);

	OSTaskSuspend(ICOMM_INIT_TASK_PRIORITY);
}
...
//#define JOIN_DEFAULT_SSID    "qwer" //"china"
//#define JOIN_DEFAULT_SSID    "ASUS_2.4G_123" //"china"
//#define JOIN_DEFAULT_PSK     "12345678" //"12345678"


//#define AP_DEFAULT_SSID    "ssv-6030-AP"
extern u8 uiP2PID[];
#define AP_DEFAULT_SSID    "WD542i"
#define AP_DEFAULT_PSK     ""
...
ssv6xxx_result APmode_default()
{
    ...
    if(strchk(uiP2PID, STRLEN(uiP2PID)))
        strcpy(ap_cfg.ssid.ssid,uiP2PID);
    else
        strcpy(ap_cfg.ssid.ssid,AP_DEFAULT_SSID);

    printf("\x1B[31m===Enter AP mode, SSID=%s===\x1B[0m\n", ap_cfg.ssid.ssid);
    ap_cfg.ssid.ssid_len = STRLEN(ap_cfg.ssid.ssid);
...
ssv6xxx_result SCONFIGmode_default()
{
    ...
    mode = SSV6XXX_HWM_SCONFIG;
    MEMSET(&sta, 0 , sizeof(Sta_setting));
    printf("\x1B[96m111111111\x1B[0m\n");
    sta.status = TRUE;
    ret = netmgr_wifi_control_async(mode, NULL, &sta);
    if (ret != 0)
    {
...
ssv6xxx_result wifi_start(wifi_mode mode, bool static_ip, bool dhcp_server)
{
    ...
    if (mode == SSV6XXX_HWM_STA)
    {
        ...
        res = STAmode_default(true); //false: don't auto join JOIN_DEFAULT_SSID
        
        //res = SCONFIGmode_default(); //Sean: 20170626
    }
