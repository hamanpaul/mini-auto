...
//    printf("\x1B[92m SLINK_STATUS_CHANNEL_LOCKED \x1B[0m\n");
        return EN_SCONFIG_LOCK;
    }
    else if (ret==SLINK_STATUS_COMPLETE){
    	Enter_Wifi_Connect = 2;
//		printf("\x1B[91m SLINK_STATUS_COMPLETE \x1B[0m\n");
            return EN_SCONFIG_DONE;
    }
    else {
//    	printf("\x1B[96m Enter_Wifi_Connect \x1B[0m\n");
        return EN_SCONFIG_GOING;
    }
...
int CusSmartGetResult(SMART_COFIG_RESULT *sres)
{
    ...
    strcpy(JOIN_DEFAULT_SSID, info.ssid);
    strcpy(JOIN_DEFAULT_PSK, info.key);
...
#define TCP_SEND_LEN (STA_MAC_LEN+STA_IP_LEN)
    ...
    int i=0;
    char data_buffer[80], strMagic[] ="SmartLinkOK";
	...
	netmgr_wifi_sconfig_done(strMagic,(STA_MAC_LEN*3-1),TRUE,SENDER_PORT_NUM);
	OSTimeDly(10);
