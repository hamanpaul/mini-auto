...
static void _scan_down_handler (void *data)
{
	extern u8 Enter_Wifi_Scan; 	// 1: Enter Wifi Scan Mode.
...
extern u8 Check_fw_ver_net(u8 connected);
extern void uiMenuSet_UpgradeServer();
...
void cmd_mib(s32 argc, char *argv[])
{
	...
	printf("\x1B[96m Upgrade_fw_net \x1B[0m\n");
	...
	netmgr_wifi_switch_async(SSV6XXX_HWM_STA, NULL, &join_cfg);
#endif
#if 0
	//printf("\x1B[96m ret=%d \x1B[0m\n",netmgr_wifi_check_sta_connected());
	...
	Save_UI_Setting();
	printf("\x1B[96m RESET WIFI \x1B[0m\n");
	//netmgr_wifi_check_sta_connected();
#endif
	extern u8 Sean_Frame_data_size;

	Sean_Frame_data_size = !Sean_Frame_data_size;
...
u8 iComm_WIFI_Status(void)
{
	...
	if(is_valid_ether_addr(info.u.station.apinfo.Mac))
	{
		rfiu_TX_WifiPower=ssv6xxx_get_rssi_by_mac(info.u.station.apinfo.Mac);
		//LOG_PRINTF("RSSI = -%d (dBm)\r\n",rfiu_TX_WifiPower);
	}
	else
	{
    ...
    M7688_WifiCHNum= info.u.station.channel;
