...
static void netif_display(struct netdev *netif)
{
	...
	extern u8 Get_IP;				// 1: Got IP.
	extern u8 LwipIpAddrInfo[4];
	extern u8 LwipSubMaskInfo[4];
	extern u8 LwipDefaultGatewayInfo[4];
	...
	if(netif->mtu != 0)
	{
		for(i=0;i<4;i++)
		{
			LwipIpAddrInfo[i]			= (((u8*)(&netif->ipaddr))[i]);
			LwipSubMaskInfo[i]			= (((u8*)(&netif->netmask))[i]);
			LwipDefaultGatewayInfo[i]	= (((u8*)(&netif->gw))[i]);
		}
	}
