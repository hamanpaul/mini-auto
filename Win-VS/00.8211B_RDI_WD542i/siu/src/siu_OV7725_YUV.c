...
#define PULSE_TIME  250   //¨C¿é¥X250¦¸Hight Level ¤j¬ù ªá0.5ms
...
#define SIU_OV7725_PARAMETER    SIU_OV7725_RDI
#elif(HW_BOARD_OPTION == MR8120_TX_MAYON_LW604)
#define SIU_OV7725_PARAMETER    SIU_OV7725_MAYON
#else
#define SIU_OV7725_PARAMETER    SIU_OV7725_AIO
#endif

#if(HW_BOARD_OPTION == MR8120_TX_SKYSUCCESS)
u8 u_chrome[10]={0, 0x10, 0x20, SENSOR_U_SATURATION_INDOOR, 0x40, 0x50, 0x60, 0x70, 0x80, 0x90};
u8 v_chrome[10]={0, 0x10, 0x20, SENSOR_V_SATURATION_INDOOR, 0x40, 0x50, 0x60, 0x70, 0x80, 0x90};
#else
u8 u_chrome[10]={0, 0x10, 0x20, 0x30, 0x40, 0x50, 0x60, 0x70, 0x80, 0x90};
u8 v_chrome[10]={0, 0x10, 0x20, 0x30, 0x40, 0x50, 0x60, 0x70, 0x80, 0x90};
...
extern u8  SensorInitAddr[256];
extern u16 SensorInitData[256];
extern u32 SensorInitLength;
#endif
...
void SetOV7725_SUNIN1(void)
{
    ...
    i2cWrite_SENSOR( 0x64, 0x001F);  // «G«×¤£¨¬®É,³]¦¨ 0XFB Ãö±¼Gamma.
...
#if 0
// OV Áú°ê­ì¼t°Ñ¼Æ¾ã¦X­ì¥»Mars°Ñ¼Æ
...
void SetOV7725_Init(void)
{
     ...
     i2cWrite_SENSOR(0x91, 0x01);
     #if(HW_BOARD_OPTION == MR8120_TX_GCT )
         i2cWrite_SENSOR(0x94, 0xb0);
         i2cWrite_SENSOR(0x95, 0x9d);
         i2cWrite_SENSOR(0x96, 0x13);
         i2cWrite_SENSOR(0x97, 0x16);
...
s32 siuSensorInit(u8 siuSensorMode,u8 zoomFactor)
{
   ...
   gRfiuUnitCntl[0].RFpara.TX_SensorType=TX_SENSORTYPE_VGA;
    ...
    #if ( (HW_BOARD_OPTION == MR8120_TX_RDI2) || (HW_BOARD_OPTION == MR8120_TX_RDI3) || (HW_BOARD_OPTION == MR8120_TX_RDI_CA671)||\
        (HW_BOARD_OPTION == MR8120_TX_RDI_CA530))
...
void siuIntHandler(void)
{
	...
	if (intStat & SIU_INT_STAT_FRAM)
	{
        ...
        #if( (HW_BOARD_OPTION==GOS_DVRBOX) || (HW_BOARD_OPTION==VER100_CARDVR) || (HW_BOARD_OPTION==SUNIN1_CARDVR) )
...
#if ( (HW_BOARD_OPTION == MR8120_TX_RDI2) || (HW_BOARD_OPTION == MR8120_TX_RDI3) ||\
        (HW_BOARD_OPTION == MR8120_TX_RDI_CA530))
    i2cWrite_SENSOR(0x24, 0x40 + (expBiasValue - 4) * 12);
    i2cWrite_SENSOR(0x25, 0x38 + (expBiasValue - 4) * 12);
#elif(HW_BOARD_OPTION == MR8120_TX_RDI_CA671)
    i2cWrite_SENSOR(0x24, 0x58 + (expBiasValue - 4) * 12);
    i2cWrite_SENSOR(0x25, 0x50 + (expBiasValue - 4) * 12);
#elif(HW_BOARD_OPTION == MR8120_TX_GCT)
    i2cWrite_SENSOR(0x24, 0x40 + (expBiasValue - 4) * 12);
    i2cWrite_SENSOR(0x25, 0x38 + (expBiasValue - 4) * 12);
#elif (HW_BOARD_OPTION == MR8120_TX_SKYSUCCESS)
    i2cWrite_SENSOR(0x24, 0x48 + (expBiasValue - 4) * 10);
    i2cWrite_SENSOR(0x25, 0x38 + (expBiasValue - 4) * 10);
...
void siuTask(void* pData)
{
    ...
    u8 pulse_time=250;   //¨C¿é¥X250¦¸Hight Level ¤j¬ù ªá0.5ms
	while (1)
	{
    ...
    #if ( (HW_BOARD_OPTION == MR8120_TX_RDI2) ||(HW_BOARD_OPTION == MR8120_TX_RDI2) || (HW_BOARD_OPTION == MR8120_TX_RDI_CA671)|| (HW_BOARD_OPTION == MR8120_TX_HECHI))
        gpioGetLevel(GPIO_GROUP_SENSOR_DAYNIGHT,GPIO_BIT_SENSOR_DAYNIGHT, &level);
        if(Mode != level)
        {
    ...
    #elif (HW_BOARD_OPTION == MR8120_TX_RDI_CA530)  
    {
    ...
    #if 0   // G_SENSOR == G_SENSOR_BMA150 ¤@¿ý¼v¼v¹³´N·|ÅÜ¦¨¶Â¥Õªº
        i2cRead_SENSOR(0x0, &temp);
        Again_x16= ((temp>>4) + 1)*(16+ (temp & 0x0f));
        ...
        {
           i2cWrite_SENSOR( 0xac, 0xbf);    //Manual De-noise
           i2cWrite_SENSOR( 0x8e, 0x04);    // De-noise strength=4    
        }
        else
        {
           i2cWrite_SENSOR( 0xac, 0xbf);    //Manual De-noise
		...
		if(Mode ==SIU_NIGHT_MODE) //AGC > 16x: ¶i¤J©]¶¡¼Ò¦¡
		{  //©]¶¡¼Ò¦¡
		   i2cWrite_SENSOR( 0xac, 0xbf);    //Manual De-noise
           i2cWrite_SENSOR( 0x8e, 0x08);    // De-noise strength=8   
        #if(HW_BOARD_OPTION  != RDI_CARMREC) //RDI ©]¶¡¼Ò¦¡¥Ñ¥~³¡¨M©w.  
           i2cWrite_SENSOR( 0x64, 0xBF);  //Saturation enable
           i2cWrite_SENSOR( 0xa6, 0x02);  //Saturation enable
           i2cWrite_SENSOR( 0xa7, 0);     //set U component saturation gain
           i2cWrite_SENSOR( 0xa8, 0);     //set V component saturation gain
        #endif
		}
        else
        { //¤é¶¡¼Ò¦¡
		   ...
		   i2cWrite_SENSOR( 0x64, 0xBF);  //Saturation enable
           i2cWrite_SENSOR( 0xa6, 0x02);  //Saturation enable
           i2cWrite_SENSOR( 0xa7, 0x50);  //set U component saturation gain
           i2cWrite_SENSOR( 0xa8, 0x50);  //set V component saturation gain
        #endif
        }
    #endif
...
#endif
#if ( (HW_BOARD_OPTION ==MR8120_TX_RDI2) || (HW_BOARD_OPTION ==MR8120_TX_RDI3) ||\
    (HW_BOARD_OPTION ==MR8120_TX_SKYSUCCESS) || (HW_BOARD_OPTION == MR8120_TX_RDI_CA671)||\
    (HW_BOARD_OPTION == MR8120_TX_RDI_CA530) || (HW_BOARD_OPTION == MR8120_TX_FRESHCAM)||\
    (HW_BOARD_OPTION == MR8120_TX_HECHI) )
void siuSetSensorDayNight(u8 Level)
{
    if(Level ==SIU_NIGHT_MODE) //AGC > 16x: ¶i¤J©]¶¡¼Ò¦¡
	{  //©]¶¡¼Ò¦¡
       
        i2cWrite_SENSOR( 0x60, 0x80);     
        i2cWrite_SENSOR( 0x61, 0x80); 
         
         i2cWrite_SENSOR( 0xa6, 0x1a);  
    ...
    { 
        ...
        #if(HW_BOARD_OPTION == MR8120_TX_SKYSUCCESS )
        i2cWrite_SENSOR( 0xa7, 0x34);  //set U component saturation gain
        i2cWrite_SENSOR( 0xa8, 0x34);  //set V component saturation gain
        i2cWrite_SENSOR( 0x14, 0x31);
        #else
...
void siuSetSensorDayNight(u8 Level)
{
    if(Level ==SIU_NIGHT_MODE)
    {
        ...
        i2cWrite_SENSOR( 0x2d, 0xd6);
        i2cWrite_SENSOR( 0x2e, 0x0d);
        i2cWrite_SENSOR( 0x2f, 0x00);
        i2cWrite_SENSOR( 0x8e, 0x20);
        i2cWrite_SENSOR( 0x8f, 0x00);
        i2cWrite_SENSOR( 0x9f, 0x62);
        i2cWrite_SENSOR( 0xa7, 0x00);
        i2cWrite_SENSOR( 0xa8, 0x00);
    ...
    else
    {
        ...
        i2cWrite_SENSOR( 0x2d, 0x00);
        i2cWrite_SENSOR( 0x2e, 0x00);
        i2cWrite_SENSOR( 0x2f, 0x42);
        i2cWrite_SENSOR( 0x8e, 0x03);
        i2cWrite_SENSOR( 0x8f, 0x02);
        i2cWrite_SENSOR( 0x9f, 0xfa);

        i2cWrite_SENSOR( 0xa7, 0x65);
        i2cWrite_SENSOR( 0xa8, 0x65);
    }
...
void siuSetSensorChrome(u8 level)
{
    u16 value;
   // return ;
    printf("**** SIU SET CHROME %d \n",level);
 //   i2cRead_SENSOR(0x64, &value);
 //   value|= 0x40; /*ENABLE UV ADJUST FUCTION*/
 //   i2cWrite_SENSOR(0x64, value);
    
    
    i2cWrite_SENSOR(0xa7, u_chrome[level]);
    i2cWrite_SENSOR(0xa8, v_chrome[level]);
