...
#include "general.h"
#if ( (Sensor_OPTION == Sensor_OV7740_YUV601) || (Sensor_OPTION == Sensor_MI9V136_YUV601) || (Sensor_OPTION == Sensor_PC1089_YUV601) )
        #include "board.h"
...
u8 AE_LIGHT_sel=0;  //Lucian: 針對不同光源(0:300W, 1:150/60W)對 AE 做調整.
#endif

#if(HW_BOARD_OPTION==GOS_DVRBOX)
u8  AE_Mode;    // 1:Day, 0:Night
#endif
/*
 *********************************************************************************************************
 * Extern Variables
...
#if (Sensor_OPTION == Sensor_OV7740_YUV601)
...
#if(HW_BOARD_OPTION==GOS_DVRBOX)
void SiuOv7740SetDayNight(u32 mode)
{
    if(mode)
    {   // Day weighting
        //DEBUG_SIU("Set siu OV7740 Day Mode!!\n");
        i2cWrite_SENSOR(0x14, 0x18); //AGC max to 4x
        i2cWrite_SENSOR(0x24, 0x40);
        i2cWrite_SENSOR(0x25, 0x20);
        
        i2cWrite_SENSOR(0x56, 0x00);
        i2cWrite_SENSOR(0x57, 0x00);
        i2cWrite_SENSOR(0x58, 0x55);
        i2cWrite_SENSOR(0x59, 0x55);
    } else {   // Night weighting
        //DEBUG_SIU("Set siu OV7740 Night Mode!!\n");
        i2cWrite_SENSOR(0x14, 0x18); //AGC max to 4x
        i2cWrite_SENSOR(0x24, 0x50);
        i2cWrite_SENSOR(0x25, 0x30);

        i2cWrite_SENSOR(0x56, 0x00);
        i2cWrite_SENSOR(0x57, 0x28);
        i2cWrite_SENSOR(0x58, 0x3c);
        i2cWrite_SENSOR(0x59, 0x3c);
    }
    AE_Mode = (u8)mode;
}
#endif
   ...
   gRfiuUnitCntl[0].RFpara.TX_SensorType=TX_SENSORTYPE_VGA;
    ...
    {	
       ...
       case SIUMODE_MPEGAVI: 
            ...
            if(SensorMirror)
            {
                i2cWrite_SENSOR( 0x0c, 0x02); //H mirror, V flip, UYVYUYVY
    ...
    #if(HW_BOARD_OPTION==GOS_DVRBOX)
            i2cWrite_SENSOR(0x28, 0x10); //Vsync positive,Hsync negative
    #else
            ...
            //insert dummy pixel
    #if 0   
            i2cWrite_SENSOR(0x2a, 0x03);
            i2cWrite_SENSOR(0x29, 0x70);
    #else
            i2cWrite_SENSOR(0x2a, 0x03);
            i2cWrite_SENSOR(0x29, 0x17);
    #endif   
            //insert dummy line
    #if (HW_BOARD_OPTION==NEW_SIYUAN)   // 27 fps @ 24MHz
            i2cWrite_SENSOR(0x2b, 0x30);
            i2cWrite_SENSOR(0x2c, 0x02);
    #else   // 30 fps @ 24MHz
	 ...
	 #if(HW_BOARD_OPTION==GOS_DVRBOX)
	    #if 0
            i2cWrite_SENSOR(0x14, 0x48); //AGC max to 32x
            i2cWrite_SENSOR(0x24, 0x50);
            i2cWrite_SENSOR(0x25, 0x48);
        #elif 0   // OV FAE fine tune
            i2cWrite_SENSOR(0x14, 0x48); //AGC max to 32x
            //i2cWrite_SENSOR(0x14, 0x28); //AGC max to 32x
            //i2cWrite_SENSOR(0x15, 0xb8); //此寄存器為降幀用，建議在低照狀態時將幀率降到15幀
            i2cWrite_SENSOR(0x24, 0x40);
            i2cWrite_SENSOR(0x25, 0x20);
        #elif 0  // OV FAE fine tune again, 2011/11/04
            i2cWrite_SENSOR(0x14, 0x20); //AGC max to 8x
            i2cWrite_SENSOR(0x24, 0x60);
            i2cWrite_SENSOR(0x25, 0x40);
        #elif 0   // OV FAE fine tune again, 2011/11/14
            //i2cWrite_SENSOR(0x14, 0x18); //AGC max to 4x
            i2cWrite_SENSOR(0x14, 0x28); //AGC max to 8x
            i2cWrite_SENSOR(0x24, 0x50);
            i2cWrite_SENSOR(0x25, 0x30);
        #else   // lucian fine tune, 2011/12/08
            i2cWrite_SENSOR(0x14, 0x18); //AGC max to 8x
            i2cWrite_SENSOR(0x24, 0x50);
            i2cWrite_SENSOR(0x25, 0x30);
        #endif
            i2cWrite_SENSOR(0x26, 0x73);
	 #else
...
#if (HW_BOARD_OPTION == ES_LIGHTING)
        #if (Sensor_FREQ_DIV==0x09)
            i2cWrite_SENSOR(0x50, 0x78); // 2e/97/4c/26 for 60/30/15/7.5fps; //Banding starting step for 50Hz (LSB 7:0)
            i2cWrite_SENSOR(0x51, 0x64); //fc/7e/3f/20 for 60/30/15/7.5fps   //Banding starting step for 60Hz (LSB 7:0)
            i2cWrite_SENSOR(0x21, 0x34); //01/23/57/cf for 60/30/15/7.5fps   //Max banding step
            ...
            i2cWrite_SENSOR(0x21, 0x23); //01/23/57/cf for 60/30/15/7.5fps   //Max banding step
        #endif
	    ...
	    #else
            i2cWrite_SENSOR(0x38, 0x14); //reserved
            i2cWrite_SENSOR(0xe9, 0x00); //threshold for blackness

            i2cWrite_SENSOR(0x53, 0x00);

	      #if(HW_BOARD_OPTION==GOS_DVRBOX)
            i2cWrite_SENSOR(0x38, 0x15);
            i2cWrite_SENSOR(0xe9, 0x50);
            i2cWrite_SENSOR(0x38, 0x16);
            i2cWrite_SENSOR(0xe9, 0x78);
            i2cWrite_SENSOR(0x38, 0x12);
            i2cWrite_SENSOR(0xe9, 0x00);
            i2cWrite_SENSOR(0x38, 0x13);
            i2cWrite_SENSOR(0xe9, 0x00);
            i2cWrite_SENSOR(0x38, 0x14);
            i2cWrite_SENSOR(0xe9, 0x20);
            i2cWrite_SENSOR(0x53, 0x02);
            
            RTC_Get_Time(&curDateTime);
            SiuOv7740SetDayNight((curDateTime.hour >= 6) && (curDateTime.hour < 18));
		  #else
            i2cWrite_SENSOR(0x56, 0x55);
            i2cWrite_SENSOR(0x57, 0xff);
            i2cWrite_SENSOR(0x58, 0xff);
            i2cWrite_SENSOR(0x59, 0xff);
		  #endif
        #endif
            i2cWrite_SENSOR(0x5f, 0x04);

            //------ISP function setting------//
            i2cWrite_SENSOR(0x80, 0x7d);  
        #if( (HW_BOARD_OPTION==NEW_SIYUAN) || (HW_BOARD_OPTION==SIYUAN_CVR)  || (HW_BOARD_OPTION==ISP_PA9002D_SYS_256P) )
            i2cWrite_SENSOR(0x81, 0x3f);  //CIP/CMX/UV_avg/yuv422/SDE enable, uv_adj enable.
        #else
            ...
            //---Gamma/AWB/Lens shadding correction config--//
            ...
            {
                i2cWrite_SENSOR(0x9c, 0x10);
                i2cWrite_SENSOR(0x9d, 0x1E);
                i2cWrite_SENSOR(0x9e, 0x35);
                i2cWrite_SENSOR(0x9f, 0x5A);
                i2cWrite_SENSOR(0xa0, 0x69);
                i2cWrite_SENSOR(0xa1, 0x76);
                i2cWrite_SENSOR(0xa2, 0x80);
                i2cWrite_SENSOR(0xa3, 0x88);
                i2cWrite_SENSOR(0xa4, 0x90);
                i2cWrite_SENSOR(0xa5, 0x97);
                i2cWrite_SENSOR(0xa6, 0xA4);
                i2cWrite_SENSOR(0xa7, 0xAF);
                i2cWrite_SENSOR(0xa8, 0xC5);
                i2cWrite_SENSOR(0xa9, 0xD7);
                i2cWrite_SENSOR(0xaa, 0xE8);
            ...
            //---sharpness & Denoise ---//
    #if( (HW_BOARD_OPTION==ES_LIGHTING) ) //De-noise auto mode
            //for OV recommand on 2010/06/23: Denose auto enable
			i2cWrite_SENSOR(0xcc, 0x04); //auto mode for sharpness and denoise
			i2cWrite_SENSOR(0x85, 0x02); //Lower limit of denoise threshold=2.
			i2cWrite_SENSOR(0x86, 0x04); //sharpness upper limit=4
	        i2cWrite_SENSOR(0x87, 0x01);
			i2cWrite_SENSOR(0x88, 0x08); //sharpness gain range=8, K(slope of denoise curve)=1/2, Denoise gain range=max gain.
    #elif((HW_BOARD_OPTION==NEW_SIYUAN) || (HW_BOARD_OPTION==SIYUAN_CVR)  || (HW_BOARD_OPTION==ISP_PA9002D_SYS_256P))
            i2cWrite_SENSOR(0xcc, 0x04); //auto mode for sharpness and denoise
			i2cWrite_SENSOR(0x85, 0x00); //Lower limit of denoise threshold=0.
			i2cWrite_SENSOR(0x86, 0x03); //sharpness upper limit=3
			i2cWrite_SENSOR(0x87, 0x01); //sharpness low limit=3
			i2cWrite_SENSOR(0x88, 0x05); //sharpness gain range=16, K(slope of denoise curve)=1/4, Denoise gain range=max gain.
	#else
            i2cWrite_SENSOR(0xcc, 0x04); //auto mode for sharpness and denoise
        ...
        #else   // OV FAE fine tune
           //----Manual UV adjust-----//
            i2cWrite_SENSOR(0x5a, 0x24);    
            i2cWrite_SENSOR(0x5b, 0xaa);
            i2cWrite_SENSOR(0x5c, 0xaa);
            i2cWrite_SENSOR(0x5d, 0x60);
            
          #if(HW_BOARD_OPTION==ALM_DVRBOX)
            sysBack_Set_Sensor_Color(0);
          #else
            ...
            //--------------Flicker Auto detect configuration----//
            ...
            //----------------Anti Flicker mode select: use manual mode------------------//
            if(AE_Flicker_50_60_sel ==SENSOR_AE_FLICKER_60HZ )  //60Hz
               #if(HW_BOARD_OPTION==GOS_DVRBOX)
			   i2cWrite_SENSOR(0xec, 0x00); //02/82 for manual/auto
			   #else
               i2cWrite_SENSOR(0xec, 0x02); //02/82 for manual/auto
               #endif
            else
			   #if(HW_BOARD_OPTION==GOS_DVRBOX)
			   i2cWrite_SENSOR(0xec, 0x40); //02/82 for manual/auto
			   #else
...
void siuTask(void* pData)
{
        ...
        if(MotionDetect_en)
           MotionDetect_API(MD_SIU_ID);
   ...
   #if(Sensor_OPTION == Sensor_OV7740_YUV601)      
       #if( (HW_BOARD_OPTION==ES_LIGHTING) || (HW_BOARD_OPTION==NEW_SIYUAN) || (HW_BOARD_OPTION==SIYUAN_CVR)  || (HW_BOARD_OPTION==ISP_PA9002D_SYS_256P) ) //De-noise auto mode

       #elif(HW_BOARD_OPTION==GOS_DVRBOX)
            RTC_Get_Time(&curDateTime);
            i2cRead_SENSOR(0x0, &Again);
            //DEBUG_SIU("Again = 0x%02x\n", Again);
            
            //----Mode select----//
            if(Mode ==SIU_DAY_MODE)
            {
               if( Again>=0x18) //Again >= 3x
                  NightCount ++;
               else
                  NightCount =0;
    		...
    		{  //夜間模式
               ...
               SiuOv7740SetDayNight(0); //夜間 AE windowns
            ...
            {   //日間模式         
                ...
                if((curDateTime.hour >= 6) && (curDateTime.hour < 18))
                   SiuOv7740SetDayNight(1); // 日間AE windowns
                else
                   SiuOv7740SetDayNight(0); //夜間 AE windowns
            }
       #else   //De-noise manual mode
            i2cRead_SENSOR(0x0, &Again);
            ...
            #if( (HW_BOARD_OPTION == ALM_DVRBOX) || (HW_BOARD_OPTION == GOS_DVRBOX))   
