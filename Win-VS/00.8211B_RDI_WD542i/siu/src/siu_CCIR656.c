...
void siuIntHandler(void)
{
...
#if(DEBUG_SIU_BOTFD_END_INTR_USE_LED7 &&( (HW_BOARD_OPTION == A1013_FPGA_BOARD) || \
    (HW_BOARD_OPTION == A1016_FPGA_BOARD) || (HW_BOARD_OPTION == A1016B_FPGA_BOARD) || \
    (HW_BOARD_OPTION == A1018_FPGA_BOARD)|| (HW_BOARD_OPTION == A1018B_FPGA_BOARD) || (HW_BOARD_OPTION  == A1020A_FPGA_BOARD)||\
    (HW_BOARD_OPTION  == A1026A_FPGA_BOARD)))
	...
	if ( (intStat & SIU_INT_STAT_FRAM) || (intStat & SIU_INT_STAT_FRAME_STR) || (intStat & SIU_INT_STAT_CCIR_TOP_STR) ) //Lucian: 由AP決定該開哪一個
	{
        ...
        #if ( (HW_BOARD_OPTION==ELEGANT_KFCDVR)||(HW_BOARD_OPTION==SUNIN_CARDVR)||(HW_BOARD_OPTION==SUNIN1_CARDVR)||(HW_BOARD_OPTION==ITS_CARDVR))
            OSSemPost(LogStringTrgSemEvt);
...
void siuIntHandler(void)
{
...
#if(Sensor_OPTION == Sensor_CCIR601) //Lucian: 普行目前無FID, 先用虛擬的.
   #if (TV_DECODER==BIT1605 )
        FID=FieldTypeID & 0x01;
    ...
    if(FID==1)
       return;
#elif(Sensor_OPTION == Sensor_CCIR656) //656
    FID=(SiuSyncStat>>31) & 0x01;
    if(FID==1)
       return;
#elif(Sensor_OPTION == Sensor_CCIR601_MIX_OV7740YUV)
   if(sysVideoInSel == VIDEO_IN_TV)
   {
	...
	if (intStat & SIU_INT_STAT_FRAM)
	{
        ...
        #elif((HW_BOARD_OPTION==GOS_DVRBOX)||(HW_BOARD_OPTION==SUNIN_CARDVR)||(HW_BOARD_OPTION==SUNIN1_CARDVR)||(HW_BOARD_OPTION==SEENEDGE_CARDVR)||(HW_BOARD_OPTION==ITS_CARDVR))
            #if ISU_OVERLAY_ENABLE
            {
                static  u32 i   = 0;
                if((i % 3) == 0)
    ...
    #if(HW_BOARD_OPTION==SEENEDGE_CARDVR)
		if (siuOpMode == SIUMODE_PREVIEW)
		{
            static  u32 i   = 0;
            if((i % 3) == 0)
                sysbackSetEvt(SYS_BACK_DRAW_PREVIEW_OSD, 0);
            i++;
        }
    #endif
	}
...
void siuTask(void* pData)
{
	...
	while (1)
	{
      ...
      if(sysVideoInSel==VIDEO_IN_SENSOR)
      {
        ...
        #if(HW_BOARD_OPTION  != ALM_DVRBOX) //RDI 夜間模式由外部決定.  
...
s32 setSensorWinSize(s8 zoomFactor, s8 siuSensorMode)
{
    ...
    if(sysVideoInSel == VIDEO_IN_TV)
    {
            ...
            if(sysTVinFormat == TV_IN_PAL)
            {
               ...
               Img_Height=576/2;
               #if 0 // ((HW_BOARD_OPTION==MMR6720_EBELL)&&(PROJ_OPT == 2))
               sensor_validsize.imgStr.x = 126;//138;// 134
               #else
