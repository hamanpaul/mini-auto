...
__align(4) EXIF_THUMBNAIL_IMAGE exifThumbnailImage =
{
    ...
    {                       /* app1 */
        ...
        {                       /* ifd0Value */
            ...
            {   
                                /* modelValue[] */
            #if(HW_BOARD_OPTION==AURUM_DVRBOX)
			   'A',  'U',  'R',  'U',  'M',  '0',  '\0', '\0',
			#elif(HW_BOARD_OPTION==ELEGANT_KFCDVR)
			   'E',  'L',  'E',  'G',  'A',  'N',  'T', '\0',
			#elif(HW_BOARD_OPTION==SUNIN_CARDVR)
			   'C',  'A',  'R',  'D',  'V',  'R',  '\0', '\0',
			#elif(HW_BOARD_OPTION==SUNIN1_CARDVR)
			   'C',  'A',  'R',  'D',  'V',  'R',  '\0', '\0',
			#elif(HW_BOARD_OPTION==GOS_DVRBOX)
			   'G',  'O',  'S',  'P',  'E',  'L',  'L', '\0',
			#elif(HW_BOARD_OPTION==VER100_CARDVR)
			   'V',  'E',  'R',  '1',  '0',  '0',  '\0', '\0',
			#elif(HW_BOARD_OPTION==SUNWAY_SDV)
			   'S',  'U',  'N',  'W',  'A',  'Y',  '\0', '\0',
...
#if ADDAPP3TOJPEG   
    else if(type == JPEG_IMAGE_APP3IMG)
    {
        pLumQ   = (u8*)exifAPP3VGAImage.primary.dqt.lumQ;
        pChrQ   = (u8*)exifAPP3VGAImage.primary.dqt.chrQ;
    }
#endif
...
#if ADDAPP3TOJPEG 
    exifAPP3VGAImage.primary.sof0.X = bSwap16(JPGAPP3_WIDTH);
    exifAPP3VGAImage.primary.sof0.Y = bSwap16(JPGAPP3_HEIGHT);
#endif    
    ...
    *pSizeUsed  = 0;
    ...
    while ((((u32)pBuf) < bufEnd) && (eoi == 0) && (FindBS == 0)) /*BJ 0523 S*/
    {
        ...
        switch (marker)
        {
            ...
            #if 0 //((CHIP_OPTION != CHIP_A1016A)) //Lucian: A1016 內部無D2D SC engine. 故 無法 看Thumbnail.
                if(type == JPEG_IMAGE_THUMBNAIL)
                   exifApp1Parse(pBuf);
            #endif                 
                applength = (*(pBuf++)<<8);
...
/*BJ 0523 E*/
    ...
    if(fsize <= EXIFDECBUF_SIZE) 
    {
     ...
     if(exifAPP3VGAImg_flag)
     {
        ...
        if(jpegDecompression(exifDecBuf + sizeUsed, PKBuf) == 0)    
     ...
     else
     {
          ...
          sysISU_enable();
     #endif
         if(jpegDecompressionSlice(exifDecBuf + sizeUsed, PKBuf) == 0)
             DEBUG_DCF("Primary JPEG decode error!!!\n");
      #if DINAMICALLY_POWER_MANAGEMENT
         sysISU_disable();
      #endif
     ...
     if(jpegDecompression(exifDecBuf + sizeUsed, uiMenuBuf1) == 0)    
        DEBUG_DCF("Primary JPEG decode error!!!\n");   
   #else
     if(jpegDecompression(exifDecBuf + sizeUsed, PKBuf) == 0)    
        DEBUG_DCF("Primary JPEG decode error!!!\n");   
...
s32 exifReadThumbnailFile(void)
{
    ...
    //IDU_ClearBuf();
    //sysISU_enable();
    //isuScalar_D2D( PKBuf , testbuff , 160, 120, 40, 60);
    //IDU_Layer1(160 , testbuff , 40 , 60 , 8 , 48);
/*              
        //==>if detect next or previous, dcfClose(pFile); return 1;
        if(sysCheckNextEvtIsPrevOrNext())
        {
