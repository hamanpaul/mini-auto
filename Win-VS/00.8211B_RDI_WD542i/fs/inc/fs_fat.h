...
typedef struct
{
    ...
    FS_u16          RsvdSecCnt;       /* 1 for FAT12 & FAT16            */	/* cytsai: 32 for FAT32 */
    FS_u32          FatEndSec;        /* End sector of FAT table*/
    FS_u32          DirStartSec;      /* RSVD+ FAT*/
    FS_u32          Dsize;            /* ROOT*/
    FS_u8            MediaDesc;
    unsigned char   SecPerClus;       /* sec in allocation unit         */
    unsigned char   NumFATs;          /* 2                              */
    int             FATType;          /* (VCC) 1: FAT12 0: FAT16, 2:FAT32 */
}
FS__FAT_BPB;
/* FAT directory entry */
typedef struct
...
FS_i32              FS__fat_DeleteFileOrDir(int Idx, FS_u32 Unit,  const char *pName, FS_u32 DirStart, FS_u32 DirSize, FS_u16 FileEntrySect,char RmFile);
