...
extern s32 smcCacheBlockWrite(void);
    ...
    if (FS__pDevInfo[idx].fs_ptr->fsl_fopen)
    {
        ...
        if (i < _FS_maxopen)
        {
            ...
            */
            ...
            ret = (FS__pDevInfo[idx].fs_ptr->fsl_fopen)(s, pMode, &_FS_filehandle[i]);
            FS_X_OS_UnlockFileHandle();
            if(ret <= 0)
           		return NULL;
...
int _FS_Remove(const char *pFileName, unsigned short EstFileEntrySect, unsigned long *used_cluster, FS_DISKFREE_T *gdiskInfo)
{
	...
	u32 bytes_per_cluster, free_size;
    FS_FARCHARPTR s;
    unsigned int i;
    int idx;
    int x;
    ...
    if (idx < 0)
    {
        return FS_DEVICE_FIND_ERROR;  /* Device not found */
    }
    if (FS__pDevInfo[idx].fs_ptr->fsl_fopen)
    {
        /*  Find next free entry in _FS_filehandle */
        ...
        if(*used_cluster)
        {
	        ...
	        if (_FS_filehandle[i].mode_a)
	            *used_cluster = (_FS_filehandle[i].append_size+ bytes_per_cluster-1) / bytes_per_cluster;
	        else
	            *used_cluster = (_FS_filehandle[i].size + bytes_per_cluster-1) / bytes_per_cluster;
        }
        //
        if (i < _FS_maxopen)
        {
            /* Set file open mode to write & truncate */
            _FS_filehandle[i].mode_r = 0; // read mode
            _FS_filehandle[i].mode_w = 1; // write mode
            _FS_filehandle[i].mode_a = 0; // append
            _FS_filehandle[i].mode_c = 0; // create
            _FS_filehandle[i].mode_b = 0; // binary
            _FS_filehandle[i].dev_index = idx;
            _FS_filehandle[i].FileEntrySect = EstFileEntrySect;
            /*
               Call the FSL function 'open' with the parameter 'del' to indicate,
               that we want to delete the file.
            */
            x = (FS__pDevInfo[idx].fs_ptr->fsl_fopen)(s, "del", &_FS_filehandle[i]);
...
{
	return _FS_Remove(pFileName, EstFileEntrySect, 0, NULL);
}
#endif
/*********************************************************************
*
...
#if FS_USE_FAT_FSL
    u8 err;
    FSSecIncSemEvt = OSSemDel(FSSecIncSemEvt, OS_DEL_ALWAYS, &err);
#endif
    return FS_X_OS_Exit();
}
