...
int sdcDevRead(u32 Unit, u32 Sector, void *pBuffer)
{
    ...
    {
        DEBUG_SDC("SDC Err: Sector=%d,sdcTotalBlockCount=%d\n",Sector,sdcTotalBlockCount);
...
int sdcDevMulWrite(u32 Unit, u32 Sector, u32 NumofSector, void *pBuffer)
{
    ...
    if (sdcMmc == 1)
    {
        ...
        if (NumofSector==1)
        {
            retValue =  sdcDevWrite( Unit,  Sector, pBuffer);
        }
        else
        {
...
#if SD_SPEC_2DOT0
                if (ucSpec2Dot0)
                    retValue = sdcWriteMultipleBlock(256, Sector, ((u8 *)pBuffer));
                else
#endif
                    retValue = sdcWriteMultipleBlock(256, Sector * SDC_BLK_LEN, ((u8 *)pBuffer));
...
#if SD_SPEC_2DOT0
                if (ucSpec2Dot0)
                    retValue = sdcWriteMultipleBlock(NumofSector, Sector, ((u8 *)pBuffer));
                else
#endif
                    retValue = sdcWriteMultipleBlock(NumofSector, Sector * SDC_BLK_LEN, ((u8 *)pBuffer));
            }
        }
        if (retValue < SD_OK)
            return retValue;
