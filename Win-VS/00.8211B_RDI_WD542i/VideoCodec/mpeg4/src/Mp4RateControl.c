					...
					)
{
	...
	{
		case I_VOP:
			qp=pRateControlParam->QP_I;
			break;
		
		case P_VOP:
			qp=pRateControlParam->QP_P;
			break;
		default:
            qp=pRateControlParam->InitQP;
			break;
	}		
...
void RCQ2_init(DEF_RATECONTROL_PARA *pRateControlParam)
{
    ...
    if(pRateControlParam->QP_I == 0)
	  pRateControlParam->QP_I=7;                     // 1 - 31

    if(pRateControlParam->QP_P == 0)
	  pRateControlParam->QP_P=7;                     // 1 - 31

    if(pRateControlParam->InitQP == 0)
	  pRateControlParam->InitQP=7;   
