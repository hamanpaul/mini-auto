...
extern u8      MultiMP4_ErrCnt[];//[MULTI_CHANNEL_LOCAL_MAX]
...
#endif  //#if MULTI_CHANNEL_LOCAL_MAX
...
s32 MultiChannelMPEG4EncoderTaskDestroy(VIDEO_CLIP_OPTION *pVideoClipOption)
{
    ...
    switch(pVideoClipOption->VideoChannelID)
    {
        ...
        case 3:
            ...
            break;
        default:
            DEBUG_MP4("Error: MultiChannelMPEG4Destroy() don't support pVideoClipOption->VideoChannelID = %d", pVideoClipOption->VideoChannelID);
    }
...
s32 MultiChannelMPEG4Coding1Frame(u8* FrameBuf, u32 FrameType, s64* FrameTime, u32* CmpSize, u32 FrameIdx, u32 mpeg4StartBits, VIDEO_CLIP_OPTION *pVideoClipOption)
{
    ...
    Mpeg4SourceStride   = 800; // preview 800*480 --> 640*480 
 #else
...
s32 MultiChannelMPEG4Output1Frame(u8* pBuf, s64* pTime, u32* pSize, u32* Mpeg4EncCnt, VIDEO_CLIP_OPTION *pVideoClipOption)
{
    ...
    switch(pVideoClipOption->VideoChannelID)
    {
        ...
        case 1:
            ...
            #if AFN_DEBUG
                DEBUG_MP4("CIU_1_SYNC_RPT = 0x%08x.\n", CIU_1_SYNC_RPT);
                DEBUG_MP4("CIU_1_SYNC_RPT = 0x%08x.\n", CIU_1_SYNC_RPT);
                DEBUG_MP4("CIU_1_SYNC_RPT = 0x%08x.\n", CIU_1_SYNC_RPT);
                DEBUG_MP4("CIU_1_SYNC_RPT = 0x%08x.\n", CIU_1_SYNC_RPT);
                DEBUG_MP4("CIU_1_SYNC_RPT = 0x%08x.\n", CIU_1_SYNC_RPT);
				#if (AFN_DEBUG_MORE_CHECK>1)			
				if(pVideoClipOption->AudioChannelID<MULTI_CHANNEL_LOCAL_MAX)
				{
					MultiMP4_ErrCnt[pVideoClipOption->VideoChannelID]++;
					DEBUG_MP4("MultiMP4_ErrCnt[%d]=%d\n", pVideoClipOption->VideoChannelID, MultiMP4_ErrCnt[pVideoClipOption->VideoChannelID]);
				}
				#endif 
            #endif 
        ...
        case 2:
            ...
            #if AFN_DEBUG
                DEBUG_MP4("CIU_2_SYNC_RPT = 0x%08x.\n", CIU_2_SYNC_RPT);
                DEBUG_MP4("CIU_2_SYNC_RPT = 0x%08x.\n", CIU_2_SYNC_RPT);
                DEBUG_MP4("CIU_2_SYNC_RPT = 0x%08x.\n", CIU_2_SYNC_RPT);
                DEBUG_MP4("CIU_2_SYNC_RPT = 0x%08x.\n", CIU_2_SYNC_RPT);
                DEBUG_MP4("CIU_2_SYNC_RPT = 0x%08x.\n", CIU_2_SYNC_RPT);
...
void MultiChannelMPEG4EncoderTask(void* pData)
{
	...
	{
		MultiMP4_ErrCnt[pVideoClipOption->VideoChannelID]=0;
		DEBUG_MP4("reset MultiMP4_ErrCnt[%d]=%d\n", pVideoClipOption->VideoChannelID, MultiMP4_ErrCnt[pVideoClipOption->VideoChannelID]);
	}
    #endif
	#endif
    ...
    while (1)
    {
        ...
        //if(pVideoClipOption->mpeg4Width >= 1280)
            *pFlag                      = MultiChannelDeterminePictureType(pVideoClipOption->VideoPictureIndex, IVOP_PERIOD / 2, pVideoClipOption);   // I: 1, P: 0
        //else
        ...
        *pFlag                      = MultiChannelDeterminePictureType(pVideoClipOption->VideoPictureIndex, IVOP_PERIOD, pVideoClipOption);   // I: 1, P: 0
        ...
        if(!pVideoClipOption->Vop_Type) {     // I frame
            ...
            DEBUG_MP4("Ch%dI ", pVideoClipOption->VideoChannelID);
				...
				#if (USE_UI_APP_TASK)
					DMSG_OUT("Ch%dI ", pVideoClipOption->VideoChannelID);
				#else
			  	DEBUG_MP4("Ch%dI ", pVideoClipOption->VideoChannelID);
				#endif
			#endif
        ...
        *pTime  = 0;
    ...
    #if TUTK_SUPPORT  
      #if 0
        if(P2PEnableStreaming[pVideoClipOption->VideoChannelID - 1] && (P2PVideoCmpSemEvt[pVideoClipOption->VideoChannelID - 1]->OSEventCnt > 1))
        {
            DropFrame  += P2PVideoCmpSemEvt[pVideoClipOption->VideoChannelID - 1]->OSEventCnt - 1;
        }
      #else
        if(pVideoClipOption->VideoChannelID == LocalChannelSource)
        {
            if(P2PEnableStreaming[0] && (P2PVideoCmpSemEvt[0]->OSEventCnt > 1))
            {
                DropFrame  += P2PVideoCmpSemEvt[0]->OSEventCnt - 1;
            }
        }
      #endif
    #endif
        ...
        pVideoClipOption->CurrentVideoTime   += *pTime;
        OS_EXIT_CRITICAL();
	    ...
	    #if TUTK_SUPPORT
          #if 0
	        if(P2PEnableStreaming[pVideoClipOption->VideoChannelID - 1]) //Local_Record, Local_Playback
            {   
	            OSSemPost(P2PVideoCmpSemEvt[pVideoClipOption->VideoChannelID - 1]);  
                //DEBUG_MP4("P2PVideoCmpSemEvt[%d]=%d\n", pVideoClipOption->VideoChannelID - 1, P2PVideoCmpSemEvt[pVideoClipOption->VideoChannelID - 1]->OSEventCnt);
            }
          #else
            if(pVideoClipOption->VideoChannelID == LocalChannelSource)
            {
                ...
                {   
                    ...
                    //DEBUG_MP4("P2PVideoCmpSemEvt[%d]=%d\n", 0, P2PVideoCmpSemEvt[0]->OSEventCnt);
                }
            }
          #endif
	    #endif
    ...
    if( (pVideoClipOption->mpeg4Width == 320) || (pVideoClipOption->mpeg4Width == 352) ) //Lsk 090703 : QVGA frames
    {
        ...
        case MPEG4_VIDEO_FRAMERATE_15:
            ...
            case MPEG4_VIDEO_QUALITY_LOW:
                ...
                pVideoClipOption->mpRateControlParam.TargetBitrate    =  28 * 1000;
                #else
    ...
    else  //VGA size//
    {
        ...
        case MPEG4_VIDEO_FRAMERATE_30:
            ...
            case MPEG4_VIDEO_QUALITY_HIGH:
                  ...
                  pVideoClipOption->mpRateControlParam.TargetBitrate    = 1750 * 1000; // 2.33 Mb/sec
