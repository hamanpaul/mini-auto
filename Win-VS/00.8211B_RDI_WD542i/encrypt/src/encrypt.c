...
/*
  ...
  */
#if 0
void Encrypt_DES_Encrypt(u8 *input, u8 *key, u8 *output, u32 length)
{
    des_ctx dc;
    char *cp;
    u32 blocks = (length+7)/8;

    cp = input;
    des_key(&dc,key);
    des_enc(&dc,cp,blocks);
    memcpy(output, cp, length);
}

void Encrypt_DES_Decrypt(u8 *input, u8 *key, u8 *output, u32 length)
{
    des_ctx dc;
    char *cp;
    u32 blocks = (length+7)/8;
...
void TestEncrypt(void)
{
	...
	u8	tmp;
    dcfCurrFile     = dcfListFileEntHead;
    EnData          = PKBuf;
    ...
    while(1)
    {
        ...
        if ((pFile = dcfOpen((signed char*)dcfCurrFile->pDirEnt->d_name, "r")) == NULL)
        {
            ...
            if(dcfCurrFile == dcfListFileEntTail)
                break;
            else
