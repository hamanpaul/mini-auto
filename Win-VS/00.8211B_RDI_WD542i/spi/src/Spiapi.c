...
extern UI_FILE_CHAR_COUNT	uiFileListCharCount[26];
...
{
	//__align(4) u8 icommbuf4[4096];	
    bool ret=TRUE;
    
//#if (OS_CRITICAL_METHOD == 3)
//	unsigned int  cpu_sr = 0;
//#endif
	//memset_hw(icommbuf4, 0, 4096); // Sean 20170426 //20170621 Sean: Test ON will Enter DeadLock???
	memcpy_hw(icommbuf4,pucSrc,unSize);	//20171108 Sean: MUST 4-bytes align.
	//conver_endianness_tx(icommbuf4,(unSize-(unSize%4)));
	//OS_ENTER_CRITICAL(); //Sean 20170601.
...
s32 spi_master_in_cpu(u8 spi_cmd,u8* pucDstBuf, u32 unSize)
{
	...
	ret = spiRead_Icomm_cup(pucDstBuf,unSize);
...
#if(SW_APPLICATION_OPTION == MR9670_DOORPHONE) || (SW_APPLICATION_OPTION == MR6730_CARDVR_2CH) || (SW_APPLICATION_OPTION == MR6730_CARDVR_1CH)
...
s32 spiArrangeSpace(void)
{
    ...
    unAddr += SPI_UI_AUDIO_LIB_SIZE;
    unAddr = ((unAddr + (spiPageSize-1))/spiPageSize) * spiPageSize;		/* page alignemnt */

    /*UI setting value*/
    spiUIConfigStartAddr = unAddr;
    unAddr += SPI_UI_CONFIG_SIZE;
    unAddr = ((unAddr + (spiBlockSize-1))/spiBlockSize) * spiBlockSize;		/* Block alignemnt */

    /* check the space is enough or not */
    if (unAddr > spiTotalSize)
    {
        DEBUG_SPI("Space is not enough for arrangement\n");
        return 0;
    }
    else
        DEBUG_SPI("Residual Free Space is %d KBytes\n", (spiTotalSize-unAddr)/1024);
...
s32 spiArrangeSpace(void)
{
    ...
    unAddr += unSize;
    unAddr = ((unAddr + (spiPageSize-1))/spiPageSize) * spiPageSize;		/* page alignemnt */
    unAddr = (unAddr + 0xFFF ) & ~0xFFF;                                    /* Ensue nework addr is a multiple of 4000  */
...
u32 spiAddressLocate(u32 srcAddr, s32 operationStatus)
{
    switch(operationStatus)
    {
...
#if HOME_RF_SUPPORT
            if(SPI_CODE_END_ADDR <= srcAddr && srcAddr < spiHomeRFROStartAddr)
                return 1;
#else
            if(SPI_CODE_END_ADDR <= srcAddr && srcAddr < spiSysParaAudioFileCntListStartAddr)
                return 1;
#endif
            break;
#if HOME_RF_SUPPORT
        case SPI_AREA_HA:
            if(spiHomeRFROStartAddr <= srcAddr && srcAddr < spiSysParaAudioFileCntListStartAddr)
                return 1;
            break;
#endif
#if !UI_ICONFLAG_BACKUP
        case SPI_AREA_AUDIO_DATA:
            if(spiSysParaAudioFileCntListStartAddr <= srcAddr && srcAddr < spiNetwrokStartAddr)
                return 1;
            break;
#else
        case SPI_AREA_AUDIO_DATA:
            if(spiSysParaAudioFileCntListStartAddr <= srcAddr && srcAddr < spiUIConfigBackupStartAddr)
                return 1;
            break;
        ...
        case SPI_AREA_BMP:
#if HOME_RF_SUPPORT
            if(SPI_BOOT_CODE_START_ADDR <= srcAddr && srcAddr < spiHomeRFROStartAddr)
                return 1;
#else
            if(SPI_BOOT_CODE_START_ADDR <= srcAddr && srcAddr < spiSysParaAudioFileCntListStartAddr)
                return 1;
#endif
            break;
        case SPI_AREA_ALL:
...
s32 spiallCodeUpdate(u8* pucCodeAddr, u32 unCodeSize)
{
    ...
    sysDeadLockMonitor_OFF(); //20170420 Sean: Make Sure WATCHDOG is OFF.
#endif

    DEBUG_SPI("In-System Update ++ALL MODE++ Code to Serial Flash...\n");

#if (HW_BOARD_OPTION == MR6730_AFN)
    DEBUG_SPI("Erase from %08x, spiSectorSize=%08x, nCodeLen=%08x\n",SPI_BOOT_CODE_START_ADDR,spiSectorSize,nCodeLen);//debug only
#endif
...
s32 spiCodeUpdate(u8* pucCodeAddr, u32 unCodeSize)
{
    ...
    DEBUG_SPI("In-System Programming to Serial Flash...\n");
    ...
    /* erase code block */
    for (unAddr=0; unAddr<SPI_CODE_END_ADDR; unAddr+=spiBlockSize)
    {
        WDT_Reset_Count();
        // Wrong place operation detect
        if(!spiAddressLocate(unAddr, SPI_AREA_CODE))
        {
            DEBUG_SPI(" in spiCodeUpdate.\n");
            return 0;
        }
        ...
        {
            DEBUG_SPI("Error: Block erase failed\n");
            return 0;
        }
    }
    /* write code size at the first 256 bytes area */
    *(u32*)spiWriteBuf = unCodeSize;
    // Wrong place operation detect
    if(!spiAddressLocate(0, SPI_AREA_CODE))
    {
        DEBUG_SPI(" in spiCodeUpdate.\n");
        return 0;
    }
    if (spiWrite(0, spiWriteBuf, SPI_MAX_BUF_SIZE) == 0)
    {
        DEBUG_SPI("Error: SPI Write error!\n");
        return 0;
    }
    /* write code */
    for (i=spiPageSize; nCodeLen>0; i+=SPI_MAX_BUF_SIZE, nCodeLen-=SPI_MAX_BUF_SIZE, pucAddr+=SPI_MAX_BUF_SIZE)
    {
        WDT_Reset_Count();
        // Wrong place operation detect
        if(!spiAddressLocate(i, SPI_AREA_CODE))
        {
            DEBUG_SPI(" in spiCodeUpdate.\n");
            return 0;
        }
...
s32 spiCmpWriteData(u8* pucCodeAddr, u32 unCodeSize)
{
    ...
    for (i=SPI_MAX_BUF_SIZE; nCodeLen>0; i+=SPI_MAX_BUF_SIZE, nCodeLen-=SPI_MAX_BUF_SIZE, pucAddr+=SPI_MAX_BUF_SIZE)
    {
        memset(spiReadBuf, 0, SPI_MAX_BUF_SIZE);
        WDT_Reset_Count();
        if (spiRead(spiReadBuf, i, SPI_MAX_BUF_SIZE)==0)
...
void spiReadUIBackup(void)
{
    ...
    for (i = spiUIConfigBackupStartAddr; unLen > 0; i += spiPageSize, unLen -= spiPageSize)
    {
        ...
        {
            DEBUG_ISP("Error: SPI Backup Raed error!\n");
            return;
        }
    ...
    if (check == FALSE)
    {
        DEBUG_ISP("UI Setting Backup Check Fail, Set Default Value!\n");
        uiSetDefaultSetting();
    }
}
...
s32 spiWriteUIBackup(void)
{
    ...
    {
        DEBUG_ISP("Error! Block Erase Fail in spiWriteUIBackup.\n");
        return 0;
    }
    ...
    for (unAddr = startAddr; unTotalSize>0; unAddr+= spiPageSize, unTotalSize-=spiPageSize, targetBuf+= spiPageSize)
    {
        ...
        {
            DEBUG_ISP("Error! SPI Write Error in UI Config Bcakup Update.\n");
            return 0;
        }
    }

    DEBUG_ISP("SPI Write UI Config Backup Finish.\n");
...
void spiReadUI(void)
{
    ...
    for (i = spiUIConfigStartAddr; unLen > 0; i += spiPageSize, unLen -= spiPageSize)
    {
        ...
        while(1)
        {
            WDT_Reset_Count();
            count ++;
            if (spiRead((u8*)targetBuf, i, SPI_MAX_BUF_SIZE))
            {
                break;
            }
            else if(count>3)
    ...
    if (check == FALSE)
    {
        DEBUG_ISP("UI Setting Check Fail, Set UI Setting Backup Check!\n");
        spiReadUIBackup();
    }
#else
    if (check == FALSE)
    {
        DEBUG_ISP("UI Setting Check Fail, Set Default Value!\n");
        uiSetDefaultSetting();
#if (SW_APPLICATION_OPTION == MR8100_BABYMONITOR)//Lucian1113
        for(i=0; i<4; i++)
            iconflag[UI_MENU_SETIDX_CH1_ON+i] = (u8)(uiRFSTAT[i] & 0x01);
#endif
    }
#endif
}
...
s32 spiWriteUI(void)
{
    ...
    if (!spiSectorErase(startAddr))
    {
        DEBUG_ISP("Error! Block Erase Fail in spiWriteUI.\n");
        return 0;
    }
    uiWriteSettingToFlash(targetBuf);
    for (unAddr=startAddr; unTotalSize>0; unAddr+= spiPageSize, unTotalSize-=spiPageSize, targetBuf+= spiPageSize)
    {
        ...
        if (!spiWrite(unAddr, targetBuf, SPI_MAX_BUF_SIZE))
        {
            DEBUG_ISP("Error! SPI Write Error in UI Config Update.\n");
            return 0;
        }
    }
    DEBUG_ISP("SPI Write UI Config Finish.\n");
    return 1;
}
...
s32 spiWriteNet(void)
{
    ...
    if (!spiSectorErase(spiNetwrokStartAddr))
    {
        DEBUG_ISP("Error! Block Erase Fail in spiWriteUI.\n");
        return 0;
    }
    uiWriteNetworkIDFromFlash(targetBuf);
    for (unAddr=spiNetwrokStartAddr; unTotalSize>0; unAddr+= spiPageSize, unTotalSize-=spiPageSize, targetBuf+= spiPageSize)
    {
        WDT_Reset_Count();
        // Wrong place operation detect
...
s32 spiWriteHomeRF(u8 typeIdx)
{
    ...
    for( i=0; i< unTotalSize/0x1000; i++)
    {
        ...
        {
            DEBUG_ISP("Error! Block(0x%x) Erase Fail in spiWriteUI.\n",targetAddr);
            OSFlagPost(gUiStateFlagGrp, FLAGUI_UI_USE_FLASH, OS_FLAG_CLR, &err);
            return 0;
        }
    }

    homeRFWriteToFlash(targetBuf, typeIdx);
    ...
    DEBUG_ISP("SPI Write HOMERF Finish. %d %x %x\n",typeIdx, targetAddr, unTotalSize);
...
//    DEBUG_ISP("FileCount %d\r\n",FileCount);
    ...
    if (FileCount != uiFileListCharCount[25].stCount+uiFileListCharCount[25].stStart)
    {
        ...
        DEBUG_ISP("uiFileListCharCount read error in spiReadFBSetting\r\n");
        return;
    }
...
s32 spiGet_UI_FB_Index(u8* targetStr)
{
    ...
    if(targetStr == NULL )
    {
        //DEBUG_GREEN("targetStr Null Return \n");
        return 0;
    }
...
//        DEBUG_SPI("cmp MainName %s and %s\r\n", MainName,frame_buf_obj[index].stFileName);
    ...
    //DEBUG_MAIN_ERR("get file %s from uiFileListCharCount fail\r\n",targetStr);
    return -1;
}
...
Note:
    ...
    and set sysSPI_Disable() after call this function
*/
#if 0//((CHIP_OPTION == CHIP_A1013A) || (CHIP_OPTION == CHIP_A1013B) || (CHIP_OPTION == CHIP_A1016A) || (CHIP_OPTION == CHIP_A1016B) || (CHIP_OPTION == CHIP_A1018A) || (CHIP_OPTION == CHIP_A1018B)||(CHIP_OPTION == CHIP_A1026A) )
u8 spiWeb_OpenFB_ByIndex(u8 *pucDstAddr, s32 fb_index)
{
    ...
    CurReadSize = (frame_buf_obj[fb_index].stFileLen);
    data_size = CurReadSize;
    spiRead(pucDstAddr, frame_buf_obj[fb_index].stPageStartAddr, CurReadSize);
    return 1;
}
#endif
...
{
    u32 i, j;
    u32	*punWriteBuf = (u32*)spiWriteBuf;
    ...
    /* set write data */
    for (i = 0; i < (SPI_MAX_BUF_SIZE/4); i++)
        *punWriteBuf++= i;

    /* Block accessing test */
    if (spiBlockEraseTest(unAddrForTest) == 1)
        DEBUG_SPI("Block Erase Test Pass!\n");
    else
    {
        DEBUG_SPI("Block Erase Test Failed!\n");
        return 0;
    }
