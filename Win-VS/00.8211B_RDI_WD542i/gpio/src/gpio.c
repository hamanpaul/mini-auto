 ...
 */
  ...
  GPIO_IIC_CFG GPIO_IIC_CFG_SD2068   = {GPIO_GROUP_I2C_SCK, GPIO_GROUP_I2C_SDA, GPIO_BIT_I2C_SCK, GPIO_BIT_I2C_SDA};  
#endif
...
s32 gpioInit(void)
{
    ...
    for (i = 0; i < GPIO_GROUP_COUNT; i++)
    {
        for (j = 0; j < GPIO_PIN_COUNT; j++)        
        {
            ...
            else if (i==1)
            {
                ...
                #elif ( (CHIP_OPTION == CHIP_A1016B) || (CHIP_OPTION == CHIP_A1018B) || (CHIP_OPTION == CHIP_A1020A) ||(CHIP_OPTION == CHIP_A1026A)) // A1020DIFF1016
                    gpioLvTrgIntCfg(i, j, GPIO1_LVTRG_CFG[j]);
                #endif
            }            
            ...
            else if (i==2)
            {
                ...
                gpioLvTrgIntCfg(i, j, GPIO2_LVTRG_CFG[j]);
            }
            else if (i==3)
            {
                gpioIntConfig(i, j, &gpio3IntCfg[j]);
                gpioLvTrgIntCfg(i, j, GPIO3_LVTRG_CFG[j]);
            }
            #endif
        #endif
    ...
    (CHIP_OPTION == CHIP_A1018A) || (CHIP_OPTION == CHIP_A1018B) || (CHIP_OPTION == CHIP_A1020A)||\
    (CHIP_OPTION == CHIP_A1026A))    
    marsIntIRQEnable(INT_IRQMASK_GPIO_0 | INT_IRQMASK_GPIO_1);
#else
    marsIntIRQEnable(INT_IRQMASK_GPIO_0);
#endif
...
#if ( (CHIP_OPTION == CHIP_A1013A) || (CHIP_OPTION == CHIP_A1013B) || (CHIP_OPTION == CHIP_A1016A)|| (CHIP_OPTION == CHIP_A1018A) )
    if(group > 1)
        return 0;
#elif ( (CHIP_OPTION == CHIP_A1016B) || (CHIP_OPTION == CHIP_A1018B) || (CHIP_OPTION == CHIP_A1020A) || (CHIP_OPTION == CHIP_A1026A) ) // A1020DIFF1016
    if(group > 4)
...
s32 gpioLvTrgIntCfg(u8 group, u8 pin, GPIO_LVTRG_SATAUS cfg)
{
    ...
    if(group == 1)
    {
        if (pin >= 16)
        {
            DEBUG_GPIO("Error: gpioLvTrgIntCfg out of range %d_%d\r\n",group, pin);
            return 0;
        }
        ...
        switch(cfg)
        {
            ...
            default:
                DEBUG_GPIO("gpioLvTrgIntCfg error set %d_%d %d\r\n",group, pin, cfg);
                return 0;
