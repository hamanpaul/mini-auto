...
static u32 Thr_PixelDiff ;       //(10*4)   //  sum(YYCbCr1-YYCbCr2) > THR_PIXELDIFF ==> 則判斷該點為有改變.
static u32 Thr_BlockMargin;
static u32 gMD_Win_Height;
static u32 gMD_Win_Width;

u8 nightVMDadd = 0; // Amon 160817
#if VMDSW
...
s32 MD_DiffVal[2]={0,0};

u8  MD_level_DeIntlac=0;
u8  MD_level_MdTrig=0;
u8  MD_Period_Sel=0;
u8  MD_DeInt_AlwaysOn=0;
u8  MD_SenLevel=0;

/*
//put  it in "MotionDetect_API.h"

#define MD_PARID_THR_PIXDIFF			0
#define MD_PARID_THR_BLKMARG			1

extern void MotionDetect_GetParm(u16 ParId,void* pParm);
extern void MotionDetect_SetParm(u16 ParId,void* pParm);
*/

u32* pMD_Thr_PixelDiff=&Thr_PixelDiff;
u32* pMD_Thr_BlockMargin=&Thr_BlockMargin;
...
//{PixelDiff,BlockMargin}
  ...
  #elif ((HW_BOARD_OPTION == MR8200_RX_TRANWO_BOX) || (HW_BOARD_OPTION == MR8120_TX_TRANWO2)||\
  	(HW_BOARD_OPTION == MR8200_RX_TRANWO_D8593_HA)|| (HW_BOARD_OPTION == MR8200_RX_TRANWO_D2505)||\
  	(HW_BOARD_OPTION == MR8120_TX_TRANWO_VM2505) || (HW_BOARD_OPTION == MR8200_RX_TRANWO_SMH101) ||\
  	(HW_BOARD_OPTION == MR8200_RX_TRANWO_SMH101_HA) || (HW_BOARD_OPTION == MR8120_TX_TRANWO3))
    u8 MD_SensitivityConfTab[MD_SENSITIVITY_LEVEL][2]={
        {25,10},
        {33,16},    
        {40,22}
    ...
    #if(UI_PROJ_OPT == 10) /* Carrot */ 
    u8 MD_SensitivityConfTab[MD_SENSITIVITY_LEVEL][2]={
        {15,5},
        {25,10},    
        {35,20}
    };
    #elif((UI_PROJ_OPT == 14) || (UI_PROJ_OPT == 15)) /* SWANN_1RF*/ 
    u8 MD_SensitivityConfTab[MD_SENSITIVITY_LEVEL][2]={
        {20,7},
        {25,10},    
        {33,16}
    };
    #elif((UI_PROJ_OPT == 11 ) || (UI_PROJ_OPT ==12))
    u8 MD_SensitivityConfTab[MD_SENSITIVITY_LEVEL][2]={
        {10,1},
        {20,5},    
        {30,10}
    ...
    u8 MD_SensitivityConfTab[MD_SENSITIVITY_LEVEL][2]={
    #if( (HW_DERIV_MODEL==HW_DEVTYPE_CDVR_AFN)||(HW_DERIV_MODEL==HW_DEVTYPE_CDVR_AFN720PSEN) )
        {10, 1},
        {15, 3},//{20, 5},
        {30,10}        
    #else
    ...
    u8 MD_SensitivityConfTab_New[MD_SENSITIVITY_LEVEL][2]={
        {0 , 1},    //VMDSW
        {0 , 2},    //VMDSW
        {0 , 5}     //VMDSW
    };
    u8 MD_SensitivityConfTab_CL692_150803[MD_SENSITIVITY_LEVEL][2]={
        {0 , 127},    //VMDSW
        {0 , 127},    //VMDSW
        {0 , 127}     //VMDSW
    };
...
void VMD_Interpolation();
void VMDMethodProcess();
...
void MotionDetect_API(int ID)
{
    ...
    DrawRectOn = DRAW_RECT_ON;
        ...
        MD_Diff = mduMotionDetect(ID,DetectWidth,DetectHeight,0);
     #if 0
        if(MD_Diff>0)
    ...
    else
    {
        ...
        if(MD_Diff>10)
        {
        	extern void UI_Motion_Record_Process(void);
        	UI_Motion_Record_Process();
        }
    #endif
    }
  #endif
  ...
  #if (MULTI_CHANNEL_SUPPORT && MULTI_CHANNEL_VIDEO_REC)  // multichannl
    #if(HW_BOARD_OPTION == MR9670_WOAN)
    if(MD_Diff > 10)
    {
    ...
    {
        if(VideoClipOption[ID].video_double_field_flag == 0)
            DEBUG_CIU("71UM Ch%d video_double_field_flag = 1\n", ID);
        VideoClipOption[ID].video_double_field_flag = 1;
        MD_trigger = 1;
    //    if(VideoClipOption[ID].asfCaptureMode & ASF_CAPTURE_EVENT_MOTION_ENA)
   //         VideoClipOption[ID].MD_Diff             = 1;
        ...
        if(VideoClipOption[ID].video_double_field_flag == 1)
            DEBUG_CIU("71UM Ch%d video_double_field_flag = 0\n", ID);
        VideoClipOption[ID].video_double_field_flag = 0;
        MD_trigger = 0;
    }
	#elif(HW_BOARD_OPTION == MR6730_AFN) 
	...
	if(MD_DeInt_AlwaysOn)
	{
		//always on
		if(VideoClipOption[ID].video_double_field_flag==0)
		{
			VideoClipOption[ID].video_double_field_flag = 1;
			//DEBUG_CIU("B-CH%d-DI=1\n",ID);//DEBUG_CIU("Ch%d DeIntlac = 1\n",ID);			
		}

		//Motion Triggered
		if(MD_Diff > MD_level_MdTrig)//MDLV_MDTRIG_THRES
		{
			MD_trigger = 1;
			if((VideoClipOption[ID].asfCaptureMode & ASF_CAPTURE_EVENT_MOTION_ENA))
			{
			#if 0 
				VideoClipOption[1].MD_Diff = VideoClipOption[2].MD_Diff = 1;//arm trigger together
			#else
				VideoClipOption[ID].MD_Diff = 1;//arm trigger
			#endif 
			}		
		}
		else
	...
	{	
		...
		//de-interlace
		if(MD_Diff > MD_level_DeIntlac)//MDLV_DEINTLAC_THRES
		{
			#if 1
			//show once
			if(VideoClipOption[ID].video_double_field_flag == 0)
		...
		else
		{
			...
			//show once
			 if(VideoClipOption[ID].video_double_field_flag == 1)
			 {
				//DEBUG_CIU("B-CH%d-DI=0\n",ID);//DEBUG_CIU("Ch%d DeIntlac = 0\n",ID);
			 }
			#endif 
			VideoClipOption[ID].video_double_field_flag = 0;
		}
...
void mduMotionDetect_init()
{
    ...
    #if VMDSW
        #if INTERPOLATION
        memset_hw((void*)VMDMeanBuf1, 0, VMD_BUF_SIZE_INTER);
        memset_hw((void*)VMDMeanBuf2, 0, VMD_BUF_SIZE_INTER);
        #else
        memset_hw((void*)VMDMeanBuf1, 0, VMD_BUF_SIZE);
        memset_hw((void*)VMDMeanBuf2, 0, VMD_BUF_SIZE);
        #endif
    #endif
    #if (sysPIPMain == PIP_MAIN_CH1)
	...
	//for AFN series
		...
		DEBUG_CIU("mduMotionDetect_init(preview=%d,video=%d)\n",MD_period_Preview,MD_period_Video);


		#if HW_MD_SUPPORT
			mduMotionDetect_Sensitivity_Config(MD_SenLevel);
			//DEBUG_CIU("mduMotionDetect_Sensitivity_Config(%d)\n",MD_SenLevel);
			DEBUG_CIU("mduMotionDetect_Sensitivity_Config(%d),{%d,%d}\n",MD_SenLevel,Thr_PixelDiff,Thr_BlockMargin);
    	#endif
...
{
    int i;
    unsigned char *F_curr;
    int Diff,WholeScreenDiff,BlockNum;
    ...
    if(gMD_Win_Width <=360)
    {
       ...
       mduRegConfig(ID,Thr_PixelDiff,F_curr,BlockNum,MDWinWidth,1);
    }
    else if(gMD_Win_Width <=720)
    {
       ...
       mduRegConfig(ID,Thr_PixelDiff,F_curr,BlockNum,MDWinWidth,2);
    }
    else if(gMD_Win_Width <=1280)
    {
       ...
       mduRegConfig(ID,Thr_PixelDiff,F_curr,BlockNum,MDWinWidth,4);
    }
    ...
    for(i=0;i<BlockNum;i++)
    {
       ...
       {
          WholeScreenDiff ++;
          if(gMD_Win_Width <=720)
          {
              if(MD_blk_Mask_VGA[ID][i]==0)
              {
                MD_blk_Wnum[ID][i]=1;
                Diff ++;
              }
...
#if 0 //VMDSW
    if(SalientCnt)
        DEBUG_CIU("VMDSW: %2d,%2d,%2d,%d,%d,%d,%d,%d(%d) \n",SalientCnt,FilterDiffCnt,VMDDiffCnt,Thr_BlockMargin,VMDSWSense,BlockSize,WindowSize,PeriodThr,MDTxNewVMDSupport);
#endif

    if(Diff >= Thr_BlockMargin)
    {
    #if MD_DEBUG_ENA   
       DEBUG_CIU("MDNum=%d,%d,%d\n",Diff,Thr_BlockMargin,Thr_PixelDiff);
    #endif
...
void mdu_TestSensitivity(char *cmd)
{
    ...
    Thr_PixelDiff=Diff;
    Thr_BlockMargin=BlockMargin;

    DEBUG_CIU("Thr_PixelDiff = %d,Thr_BlockMargin=%d\n", Thr_PixelDiff,Thr_BlockMargin);
}
...
void mduMotionDetect_Sensitivity_Config(u8 SenseLevel)
{
   ...
   {
      Thr_PixelDiff=MD_SensitivityConfTab[SenseLevel][0];
      #if ((HW_BOARD_OPTION == MR8120_TX_RDI_CL692) && (UI_PROJ_OPT == 3))
      Thr_BlockMargin=MD_SensitivityConfTab[SenseLevel][1]+nightVMDadd; // Amon 160817
      #else
      Thr_BlockMargin=MD_SensitivityConfTab[SenseLevel][1];
	  #endif
...
//void VMDMotionExtraction2(u8 mean_cur[MEAN_Height][MEAN_Width], u8 mean_prev[MEAN_Height][MEAN_Width], u32 PosCnt, u8 i, u8 j, s8 *dx_temp, s8 *dy_temp )
void VMDMotionExtraction2(u8 *mean_cur, u8 *mean_prev, u32 PosCnt, u8 i, u8 j, s8 *dx_temp, s8 *dy_temp )
{
    ...
    else
    {
        ...
        i = i*2;
        j = j*2;
        MeanWidthTemp = MEAN_Width_INTER;
        MeanHeightTemp = MEAN_Height_INTER;
        #else
        MeanWidthTemp = MEAN_Width;
        MeanHeightTemp = MEAN_Height;
        #endif
...
//void VMDTemporalDiff(u8 mean_cur[MEAN_Height][MEAN_Width], u8 mean_prev[MEAN_Height][MEAN_Width], u32 PosCnt ,u8 i, u8 j)
...
void VMD_Interpolation()
{
    ...
    for(j = 0; j<(MEAN_Height_INTER) ; j+=2)
    {
        for(i = 1; i<(MEAN_Width_INTER) ; i+=2)
        {
            Pos  = (j*MEAN_Width_INTER + i);
            *(mean_cur_ptr+ Pos)= (*(mean_cur_ptr+ Pos-1) + *(mean_cur_ptr+ Pos+1) )/2;
        }
    }
    for(j = 1; j<(MEAN_Height_INTER) ; j+=2)
    {
        for(i = 0; i<(MEAN_Width_INTER) ; i+=2)
        {
            Pos  = (j*MEAN_Width_INTER + i);
            *(mean_cur_ptr+ Pos)= (*(mean_cur_ptr+ Pos-MEAN_Width_INTER) + *(mean_cur_ptr+ Pos+MEAN_Width_INTER) )/2;
        }
    }
    for(j = 1; j<(MEAN_Height_INTER) ; j+=2)
    {
        ...
        {
            Pos  = (j*MEAN_Width_INTER + i);
              *(mean_cur_ptr+ Pos)= (*(mean_cur_ptr+ Pos-1) + *(mean_cur_ptr+ Pos+1) + *(mean_cur_ptr+ Pos-MEAN_Width_INTER) + *(mean_cur_ptr+ Pos+MEAN_Width_INTER) )/4;
//            if(Pos == 446)
//            {
...
void VMDSW_TestSensitivity(char *cmd)
{
    ...
    s8 window;
    u8 perthr;
    sscanf((char*)cmd, "%d %d %d", &block,&window,&perthr);
    BlockSize=block;
    WindowSize=window;
    PeriodThr=perthr;
    VMD_CNT =0;
    DEBUG_CIU("BlockSize = %d,WindowSize=%d PeriodThr=%d\n", BlockSize,WindowSize,PeriodThr);
}

#if 0
s8 getPrevDisplacement(u8* MotionBuf[10], u8 FrameCnt, u32 PosCnt)
{
    s8 PrevDisplacement;
    
    PrevDisplacement = (s8) *(MotionBuf[FrameCnt] + PosCnt);
//    if (PosCnt == 150)
//        DEBUG_CIU("\t(%d) ,%d ,%2d  \n",VMD_CNT-PeriodTime,CNT_temp ,*(MotionBuf[CNT_temp] + PosCnt) );
...
void VMDMotionSaveCnt(u32 PosCnt,u8 i, u8 j ,s8 dx, s8 dy)
{
    u8  BitCnt;
    u32 ByteCnt;
...
#if 0
    if (i==10 && j==7 )
//        DEBUG_CIU("%2x,%2x,%2d,%2d,%2d\n",*(VMDPositiveBuf_X[VMD_CNT%10]  + shiftCnt),*(VMDPositiveBuf_Y[VMD_CNT%10]  + shiftCnt),rx,PNXYshift[bitcnt],shiftCnt);
//        DEBUG_CIU("%x \n",((u32)VMDPositiveBuf_X) );
        DEBUG_CIU("%d, 0x%2x\n", VMD_CNT, *(VMDPositiveBuf_X[VMD_CNT%10] + ByteCnt));
#endif
...
void VMDMotionExtraction(u8 i, u8 j, s8 *dx_temp, s8 *dy_temp , u8 mean_cur[MEAN_Height][MEAN_Width], u8 mean_prev[MEAN_Height][MEAN_Width])
{
    u8 x,y;
    u8 mx ,my ;
    s8 shift_x,shift_y;
...
// first check
        minmse = 999999;
        mse=0;
        if(my+BlockSize<=MEAN_Height && mx+BlockSize<=MEAN_Width)
        {
            for(y=0;y<BlockSize;y++)
            {
        	 	for(x=0;x<BlockSize;x++)
        		{
        		    mse+=abs((mean_cur[my+y][mx+x]-mean_prev[my+y+0][mx+x+0]));
        		}
            }
            if(mse<minmse)
            {
...
// search window check
        for(shift_y = (-WindowSize); shift_y<=(WindowSize);shift_y++)
        {
            ...
            {
                mse=0;
                if(my+shift_y>=0 && mx+shift_x>=0 && my+shift_y+BlockSize<=MEAN_Height && mx+shift_x+BlockSize<=MEAN_Width)
                {
                    for(y=0;y<BlockSize;y++)
                    {
    				 	for(x=0;x<BlockSize;x++)
    					{
    					    mse+=abs((mean_cur[my+y][mx+x]-mean_prev[my+y+shift_y][mx+x+shift_x]));
                            //DEBUG_CIU("(%d,%d)",mean_cur[my+y][mx+x],mean_prev[my+y+shift_y][mx+x+shift_x]);
    					}
                    }
                    //DEBUG_CIU("(%d,%d)",mse,minmse);
                    if(mse<minmse && (minmse-mse) >2)
...
/*
        //DEBUG_CIU("(%2d,%2d)",*dx_temp,*dy_temp);
        
        if ( i==4 && j==4)
            //DEBUG_CIU("(%2d,%2d)\n",*dx_temp,*dy_temp);
            DEBUG_CIU("(%2d,%2d)\n",*dx_temp,mean_cur[my][mx]);
*/
    }
...
void VMDMethodProcess()
{
    ...
    VMDDiffCnt =0;
    FilterDiffCnt = 0;
    for(j = 0; j<(MEAN_Height-BlockSize+1); j++)
    {
        for (i =0; i<(MEAN_Width-BlockSize+1); i++)
        {
            PosCnt  = (j*MEAN_Width + i);
        #if VMDSW2 //method 2
            // step 1
...
//                DEBUG_CIU("%8d,%2d,%2d,%2d,%2d\n",VMD_CNT,dx,dy,*(VMDMotionPos_X + PosCnt),*(VMDMotionPos_Y + PosCnt));
        ...
        #else 
            // step 2
            VMDMotionExtraction(i, j, &dx, &dy, mean_cur, mean_prev);
            // save the positive and negative count
            VMDMotionSaveCnt(PosCnt,i, j, dx, dy);
            // step 3
            VMDTemporalFilter(PosCnt,i,j);
    ...
    VMD_CNT++;
    if(VMDDiffCnt > 80) // 解決 sensor 自身亮度變化
        VMDSW_Reset();
//    if(VMD_CNT%20 == 0 )
//        DEBUG_CIU("\n");
}
