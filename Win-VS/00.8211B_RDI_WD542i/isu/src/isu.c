...
u32 guiCurIsuPKIADDR0, guiCurIsuPKIADDR1, guiCurIsuPKIADDR2;
#endif
u8  SRCBuf0[2];
...
u8  FieldTypeID   = 0;  // 1: odd field, 0: even field
    ...
    (HW_BOARD_OPTION == MR9670_WOAN) || (HW_BOARD_OPTION == MR6730_AFN)
...
s32 old_ISU_isu_idufrmcnt;
s32 old_FID_isu_idufrmcnt;
s32 ISU_int_time;
s32 FID_int_time;
s32 Timer3_int_time;
s32 display_idx_timeoffset;
#endif

#if(MONITOR_ISU_CAPTURE_BUFFER_IDX)
s32 old_ISU_isu_avifrmcnt;
s32 old_FID_isu_avifrmcnt;
#endif
/*
 *********************************************************************************************************
 * Extern Variables
...
#if(HW_BOARD_OPTION==MUSTEK_DVRBOX)
u32 X_offset = 160*2;  //Lsk 090707 : put QVGA int center, width*2
u32 Y_offset =  120;
#elif(HW_BOARD_OPTION==PROJECT_DW950_REAL)
u32 X_offset = 80*2;
u32 Y_offset =  0;
#elif((HW_BOARD_OPTION==SEENEDGE_CARDVR))
u32 X_offset = 0;  //Lsk 090707 : put QVGA int center, width*2
u32 Y_offset = 0;
#elif((HW_BOARD_OPTION==SUPER_POWER))
u32 X_offset = 0;  //Lsk 090707 : put QVGA int center, width*2
u32 Y_offset = 0;
#else
  ...
  if( sysVideoInSel == VIDEO_IN_TV) //TV-in
  {
    ...
    else //sensor-in
    {
		...
		else
		{
            ...
            isu_avifrmcnt++;
        #if(HW_BOARD_OPTION ==  D010_CARDVR) //Lucian: QVGA 60 fps, preview 只呈現30 fps. 特例處理
            if(VideoRecFrameRate == MPEG4_VIDEO_FRAMERATE_60)
            {
               if(isu_avifrmcnt & 0x01)
               {
                  isu_idufrmcnt++;
               }
            }
...
void isuIntProcess_MPEGAVI(void)
{
  ...
  if( sysVideoInSel == VIDEO_IN_TV) //TV-in
  {
                ...
                {   // odd field, top field.
                    ...
                    #if ISU_OVERLAY_ENABLE
                      #if(HW_BOARD_OPTION == KONLIKA_DVRBOX)
                      if(TimeStampEnable)
                      {
   ...
   if( sysVideoInSel == VIDEO_IN_TV) //TV-in
   {
          ...
          //Do nothing...
            ...
            {    // odd field, Top field.
                if(sysTVOutOnFlag) //TV-out
                {
                #if ( ISUCIU_PREVIEW_PNOUT && ((CHIP_OPTION == CHIP_A1013A) || (CHIP_OPTION == CHIP_A1013B) ||(CHIP_OPTION == CHIP_A1016A) || (CHIP_OPTION == CHIP_A1016B)|| (CHIP_OPTION == CHIP_A1020A) || (CHIP_OPTION == CHIP_A1018A) ||(CHIP_OPTION == CHIP_A1018B) ||(CHIP_OPTION == CHIP_A1026A)) )
                   /*
...
void isuIntProcess_CAP_PREVIEW(void)
{
   if( sysVideoInSel == VIDEO_IN_TV) //TV-in
   {
        ...
        {    // odd field, Top field.
			...
			#if ((HW_BOARD_OPTION==WINNIN_IRDVR)||(HW_BOARD_OPTION==KONLIKA_DVRBOX))
                #if ISU_OVERLAY_ENABLE
                    #if(ISU_OVL_TYPE == ISU_OVL_TYPE_16Bit)
                    IsuOVL_IADDR    = (u32)ScalarOverlayImage + 1280;
                    #elif(ISU_OVL_TYPE == ISU_OVL_TYPE_1Bit)
...
void isuOutputAddrArrange_TV(void)
{
    ...
    if ( (siuOpMode == SIUMODE_MPEGAVI) && (sysCameraMode == SYS_CAMERA_MODE_PREVIEW) )
    {
        ...
        {   // even field, bottom field.
        ...
        #if ( (CHIP_OPTION == CHIP_A1013A) || (CHIP_OPTION == CHIP_A1013B) || (CHIP_OPTION == CHIP_A1016A) || (CHIP_OPTION == CHIP_A1020A) || (CHIP_OPTION == CHIP_A1016B) ||(CHIP_OPTION == CHIP_A1018A) || (CHIP_OPTION == CHIP_A1018B)||(CHIP_OPTION == CHIP_A1026A))
            isu_idufrmcnt++;
            IsuSCA_MODE = (IsuSCA_MODE & (~(ISU_PKWSEL_MASK|ISU_PNWSEL_MASK))) | ((isu_idufrmcnt % 3) << ISU_PKWSEL_SHFT);
        #else
            IsuSCA_MODE = (IsuSCA_MODE & (~(ISU_PKWSEL_MASK|ISU_PNWSEL_MASK))) | (( (isu_idufrmcnt+1) % 3) << ISU_PKWSEL_SHFT);
            #if(MONITOR_ISU_DISPLAY_BUFFER_IDX)
            if((isu_idufrmcnt!=old_FID_isu_idufrmcnt)&&(isu_idufrmcnt>10))
                DEBUG_ISU("2.<%d,%d>\n",isu_idufrmcnt+1,old_FID_isu_idufrmcnt);
            old_FID_isu_idufrmcnt = isu_idufrmcnt+1;
            #endif
        #endif
           ...
           #if(MONITOR_ISU_CAPTURE_BUFFER_IDX)
           if((isu_avifrmcnt!=old_FID_isu_avifrmcnt)&&(isu_avifrmcnt>10))
               DEBUG_ISU("1.{%d,%d}\n",isu_avifrmcnt+1,old_FID_isu_avifrmcnt);
           old_FID_isu_avifrmcnt = isu_avifrmcnt+1;
           #endif
    ...
    else if (siuOpMode == SIUMODE_PREVIEW)
    {
        ...
        {   // even field, bottom field.
            ...
            #else //for small pannel: only one field reserved
                #if DROP_BOTTOMFIELD
                   #if ( ISUCIU_PREVIEW_PNOUT && ((CHIP_OPTION == CHIP_A1013A) || (CHIP_OPTION == CHIP_A1013B) ||(CHIP_OPTION == CHIP_A1016A) || (CHIP_OPTION == CHIP_A1020A) || (CHIP_OPTION == CHIP_A1016B) || (CHIP_OPTION == CHIP_A1018A) ||(CHIP_OPTION == CHIP_A1018B)||(CHIP_OPTION == CHIP_A1026A)) )
                      IsuSCA_FUN  &= (~ISU_PNOUT_ENA);
                   #else
                      IsuSCA_FUN  &= (~ISU_PKOUT_ENA);
                   #endif
            ...
            isu_idufrmcnt++;
            #if(MONITOR_ISU_DISPLAY_BUFFER_IDX)
            if((isu_idufrmcnt!=old_ISU_isu_idufrmcnt+1)&&(isu_idufrmcnt>10))
                DEBUG_ISU("2.<%d,%d,%d>\n",isu_idufrmcnt,old_ISU_isu_idufrmcnt,isuFrameTime);
            old_ISU_isu_idufrmcnt = isu_idufrmcnt;
            timerCountRead(1, (u32*) &FID_int_time);
            FID_int_time    = FID_int_time / 100;
            #endif

        #if ( ISUCIU_PREVIEW_PNOUT && ((CHIP_OPTION == CHIP_A1013A) || (CHIP_OPTION == CHIP_A1013B) || (CHIP_OPTION == CHIP_A1016A) ||\
            (CHIP_OPTION == CHIP_A1016B) || (CHIP_OPTION == CHIP_A1020A) || (CHIP_OPTION == CHIP_A1018A) || (CHIP_OPTION == CHIP_A1018B)||\
...
void isuOutputAddrArrange_Sensor(void)
{
    ...
    if ( (siuOpMode == SIUMODE_MPEGAVI) && (sysCameraMode == SYS_CAMERA_MODE_PREVIEW) )
    {
        ...
        #if TV_DISP_BY_IDU
           //Display switch by IDU interrrupt
        #else
           #if MULTI_CHANNEL_SUPPORT
                if(sysVideoInCHsel == 0x00)
                {
    ...
    (CHIP_OPTION == CHIP_A1016B) || (CHIP_OPTION == CHIP_A1020A) || (CHIP_OPTION == CHIP_A1018A) ||\
    (CHIP_OPTION == CHIP_A1018B)||(CHIP_OPTION == CHIP_A1026A))
...
{   //Lucian: Only Capture VGA(640x480) size.
    ...
    // 0x0008
#if((HW_BOARD_OPTION==WINNIN_IRDVR)||(HW_BOARD_OPTION==KONLIKA_DVRBOX)||(HW_BOARD_OPTION==KD_DVRBOX)||(HW_BOARD_OPTION==AV9AV_DVR)||(HW_BOARD_OPTION==SEENEDGE_CARDVR))
	if (gu8TimeStamp==1)
	{
		IsuSCA_FUN =	ISU_PKOUT_ENA|
						ISU_PNOUT_DISA|
    ...
    //Keep Burst Request for Packet Channel:Disable
    //Reason:sensor will be overflow, scaler will be underrun
#if ( (CHIP_OPTION == CHIP_A1013A) || (CHIP_OPTION == CHIP_A1013B) || (CHIP_OPTION == CHIP_A1016A) || (CHIP_OPTION == CHIP_A1020A) || (CHIP_OPTION == CHIP_A1016B) ||\
    (CHIP_OPTION == CHIP_A1018A) || (CHIP_OPTION == CHIP_A1018B)||(CHIP_OPTION == CHIP_A1026A))
    IsuFIFO_TH = 0x5377777f;
#else
    IsuFIFO_TH = 0x53535353;
#endif
    	...
    	{
    		IsuOVL_IADDR	= (u32)ScalarOverlayImage;
     #if ((ISU_OVERLAY_LARGE_FONT_TYPE == ASCII_FONT_20x20x95) || (ISU_OVERLAY_LARGE_FONT_TYPE == ASCII_FONT_20x20x95_25))
            IsuOVL_STRIDE   = 1280 * 2;
    		IsuOVL_WSP		= 305 | (209 << 16);
    		IsuOVL_WEP		= 624 | (228 << 16);
     #elif (ISU_OVERLAY_LARGE_FONT_TYPE == ASCII_FONT_20x20x95_2BIT || ISU_OVERLAY_LARGE_FONT_TYPE == ASCII_FONT_20x20x95_DOUBLE_FIELD_2BIT || ISU_OVERLAY_LARGE_FONT_TYPE == ASCII_FONT_20x20x95_2BIT_WITH_LIGHT)
            #if ((HW_BOARD_OPTION==AV9AV_DVR)||(HW_BOARD_OPTION==SEENEDGE_CARDVR))
            //printf("isuCapturePreviewImg()\n");
            IsuOVL_STRIDE   = 160 * 2;
            /*
            IsuOVL_WSP      = 245 | (213 << 16);
            IsuOVL_WEP      = 624 | (232 << 16);
            IsuOVLPAL1      = 0x006bff1d;   // Blue
            IsuOVLPAL2      = 0x00FF554C;
            IsuOVLPAL3      = 0x00000000;
            */
            IsuOVL_WSP      =  25 | (  9 << 16);
            IsuOVL_WEP      = 624 | ( 28 << 16);
            //IsuOVLPAL1      = 0x008080ff;   // white
            IsuOVLPAL1      = 0x006bff1d;   // Blue
            IsuOVLPAL2      = 0x009400e1;   // yellow
            IsuOVLPAL3      = 0x00ff544c;   // red
            #elif(HW_BOARD_OPTION == KD_DVRBOX)
            //printf("isuCapturePreviewImg()\n");
            IsuOVL_STRIDE   = 160 * 2;
            /*
            IsuOVL_WSP      = 245 | (213 << 16);
...
s32 isuPreviewZoom(s8 zoomFactor)
{
    ...
    #if(HW_BOARD_OPTION==PROJECT_DW950_REAL)
    guiCurIsuPKIADDR0 = IsuPK_IADDR0;
    guiCurIsuPKIADDR1 = IsuPK_IADDR1;
    guiCurIsuPKIADDR2 = IsuPK_IADDR2;
    #endif
...
#if ( (CHIP_OPTION == CHIP_A1013A) || (CHIP_OPTION == CHIP_A1013B) || (CHIP_OPTION == CHIP_A1016A) ||\
    (CHIP_OPTION == CHIP_A1016B) || (CHIP_OPTION == CHIP_A1020A) || (CHIP_OPTION == CHIP_A1018A) || (CHIP_OPTION == CHIP_A1018B)||(CHIP_OPTION == CHIP_A1026A))
    IsuFIFO_TH = 0x5377777f;
#else
    IsuFIFO_TH = 0x53535353;
#endif
...
void isuOverlayImgConfig()
{
    if( sysVideoInSel == VIDEO_IN_TV) //TV-in
    {
        if( (mpeg4Width == 320) || (mpeg4Width == 352) ) //Lsk 090703 : QVGA frames
        {
            ...
            IsuOVL_STRIDE   = 640;
            IsuOVL_WSP      =   5 | ( 13 << 16);
            IsuOVL_WEP      = 316 | ( 48 << 16);
    		#elif(HW_BOARD_OPTION==WINNIN_IRDVR)
            IsuOVL_STRIDE   = 640;
            IsuOVL_WSP      =  95 | (199 << 16);
            IsuOVL_WEP      = 310 | (222 << 16);
    		#else
            IsuOVL_STRIDE   = 640;
            ...
            IsuOVL_WEP      = 310 | (234 << 16);
                ...
                IsuOVLPAL2      = 0x00FF554C;
                IsuOVLPAL3      = 0x00000000;
            #elif(HW_BOARD_OPTION==VEISE_CARDVR)
                IsuOVL_STRIDE   = 80;
                IsuOVL_WSP      =  75 | (179 << 16);
                IsuOVL_WEP      = 290 | (202 << 16);
                IsuOVLPAL1      = 0x008080ff;
                IsuOVLPAL2      = 0x00FF554C;
                IsuOVLPAL3      = 0x00000000;
            #elif(HW_BOARD_OPTION==JSY_DVRBOX)
                IsuOVL_STRIDE   = 80;
                IsuOVL_WSP      =  3 | (211 << 16);
                IsuOVL_WEP      =310 | (234 << 16);
                IsuOVLPAL1      = 0x008080ff;
                IsuOVLPAL2      = 0x00FF554C;
                IsuOVLPAL3      = 0x00000000;
            #elif(HW_BOARD_OPTION==AV9AV_DVR)
                IsuOVL_STRIDE   = 80;
                IsuOVL_WSP      =   5 | ( 13 << 16);
                IsuOVL_WEP      = 316 | ( 48 << 16);
                //IsuOVLPAL1      = 0x008080ff;
            ...
            #elif(HW_BOARD_OPTION==AOYA_DVRBOX)
            if(TvOutMode == SYS_TV_OUT_NTSC)
            {
                IsuOVL_STRIDE   = 80;
                IsuOVL_WSP      = 84 | (223 << 16);
                IsuOVL_WEP      =310 | (234 << 16);
            }
            else if(TvOutMode == SYS_TV_OUT_PAL)
            {
                IsuOVL_STRIDE   = 80;
                IsuOVL_WSP      = 87 | (263 << 16);
                IsuOVL_WEP      =310 | (274 << 16);
            }
            IsuOVLPAL1      = 0x008080ff;
            IsuOVLPAL2      = 0x00FF554C;
            IsuOVLPAL3      = 0x00000000;
       ...
       #if ((HW_BOARD_OPTION==DTY_IRDVR)||(HW_BOARD_OPTION==WINNIN_IRDVR))
        else if( (mpeg4Width == 704) || (mpeg4Width == 720) ) //Lsk 090703 : QVGA frames
        {
            IsuOVL_IADDR    = (u32)ScalarOverlayImage;
            ...
            IsuOVL_WEP      = 624 | (228 << 16);
            #endif
       #elif (ISU_OVERLAY_LARGE_FONT_TYPE == ASCII_FONT_20x20x95_2BIT || ISU_OVERLAY_LARGE_FONT_TYPE == ASCII_FONT_20x20x95_DOUBLE_FIELD_2BIT)
            #if (HW_BOARD_OPTION==DTY_IRDVR)
            IsuOVL_STRIDE   = 160 * 2;
            IsuOVL_WSP      = 31 |  ( 15 << 16);
            IsuOVL_WEP      = 650 | ( 44 << 16);
            ...
            if (TvOutMode == SYS_TV_OUT_NTSC)
            {
                IsuOVL_WSP      = 225 | (189 << 16);
                IsuOVL_WEP      = 604 | (208 << 16);
            }
            else
            {
                IsuOVL_WSP      = 225 | (234 << 16);
                IsuOVL_WEP      = 604 | (253 << 16);
            }
            #else
            IsuOVL_STRIDE   = 160 * 2;
            IsuOVL_WSP      = 245 | (219 << 16);
        ...
        else if (TvOutMode == SYS_TV_OUT_NTSC)
        {
            ...
            #elif(HW_BOARD_OPTION==ITS_CARDVR)
            IsuOVL_WSP      =  25 | ( 14 << 16);
            IsuOVL_WEP      = 624 | ( 42 << 16);
            #elif(HW_BOARD_OPTION==SUPER_POWER)
            IsuOVL_WSP      =  25 | ( 14 << 16);
            IsuOVL_WEP      = 624 | ( 42 << 16);
            #elif (HW_BOARD_OPTION==GOS_DVRBOX)
            IsuOVL_WSP      =  25 | (  9 << 16);
            IsuOVL_WEP      = 624 | ( 18 << 16);
            #elif (HW_BOARD_OPTION==VER100_CARDVR)
            IsuOVL_WSP      =  25 | (  9 << 16);
            IsuOVL_WEP      = 624 | ( 18 << 16);
            #elif (HW_BOARD_OPTION==DTY_IRDVR)
            IsuOVL_WSP      = 11 |  ( 15 << 16);
            IsuOVL_WEP      = 630 | ( 44 << 16);
    		#elif(HW_BOARD_OPTION==WINNIN_IRDVR)
            IsuOVL_WSP      = 245 | (209 << 16);
            IsuOVL_WEP      = 624 | (228 << 16);
            #elif(HW_BOARD_OPTION==JSW_DVRBOX)
            IsuOVL_WSP      = 165 | (219 << 16);
            IsuOVL_WEP      = 624 | (228 << 16);
            #elif(HW_BOARD_OPTION==JSY_DVRBOX)
            IsuOVL_WSP      = 165 | (219 << 16);
            IsuOVL_WEP      = 624 | (228 << 16);
    		#elif (HW_BOARD_OPTION==AURUM_DVRBOX)
            IsuOVL_WSP      =  15 | ( 19 << 16);
            IsuOVL_WEP      = 394 | ( 28 << 16);
            #else
            IsuOVL_WSP      = 245 | (219 << 16);
            IsuOVL_WEP      = 624 | (228 << 16);
            #endif
        ...
        #if ((HW_BOARD_OPTION==ELEGANT_KFCDVR)||(HW_BOARD_OPTION==SUNIN_CARDVR)||(HW_BOARD_OPTION==SUNIN1_CARDVR))
            IsuOVL_WSP      =  25 | (  9 << 16);
            #if UART_GPS_COMMAND    // Three line overlay string
            IsuOVL_WEP      = 624 | ( 38 << 16);
            #else                   // Two line overlay string
            IsuOVL_WEP      = 624 | ( 28 << 16);
            #endif
        #elif(HW_BOARD_OPTION==ITS_CARDVR)
            IsuOVL_WSP      =  25 | (  14 << 16);
            #if UART_GPS_COMMAND    // Three line overlay string
            IsuOVL_WEP      = 624 | ( 57 << 16);
            #else                   // Two line overlay string
            IsuOVL_WEP      = 624 | ( 42 << 16);
            #endif
            IsuOVLPAL1      = 0x008080ff;   // white
            ...
            IsuOVL_WEP      = 624 | ( 28 << 16);
            //IsuOVLPAL1      = 0x008080ff;   // white
            //IsuOVLPAL1      = 0x00c06a26;   // 暗紅色
            IsuOVLPAL1      = 0x00152c96;   // Green
            IsuOVLPAL2      = 0x009400e1;   // yellow
            IsuOVLPAL3      = 0x00ff544c;   // red
            #elif(HW_BOARD_OPTION==AV9AV_DVR)
            IsuOVL_WSP      =  25 | (  9 << 16);
            ...
            IsuOVLPAL2      = 0x009400e1;   // yellow
            IsuOVLPAL3      = 0x00ff544c;   // red
            #elif ((HW_BOARD_OPTION == KD_DVRBOX)||(HW_BOARD_OPTION==VER100_CARDVR))
            IsuOVL_WSP      =  25 | (  9 << 16);
            IsuOVL_WEP      = 624 | ( 28 << 16);
            IsuOVLPAL1      = 0x008080ff;   // white
            IsuOVLPAL2      = 0x009400e1;   // yellow
            ...
            IsuOVLPAL3      = 0x00ff544c;   // red
            #elif(HW_BOARD_OPTION==AOYA_DVRBOX)
            IsuOVL_WSP      = 213 | (219 << 16);
            IsuOVL_WEP      = 624 | (228 << 16);
            IsuOVLPAL1      = 0x008080ff;   // white
            IsuOVLPAL2      = 0x009400e1;   // yellow
            IsuOVLPAL3      = 0x00ff544c;   // red
            #elif(HW_BOARD_OPTION==JSY_DVRBOX)
            IsuOVL_WSP      =  10  | (219 << 16);
            IsuOVL_WEP      = 629  | (228 << 16);
            IsuOVLPAL1      = 0x008080ff;   // white
            IsuOVLPAL2      = 0x009400e1;   // yellow
            IsuOVLPAL3      = 0x00ff544c;   // red
            #elif (HW_BOARD_OPTION==DTY_IRDVR)
            IsuOVL_WSP      = 10 |  ( 15 << 16);
            IsuOVL_WEP      = 629 | ( 44 << 16);
            IsuOVLPAL1      = 0x008080ff;   // white
            IsuOVLPAL2      = 0x009400e1;   // yellow
            IsuOVLPAL3      = 0x00ff544c;   // red
            #elif(HW_BOARD_OPTION==WINNIN_IRDVR)
            ...
            #elif(HW_BOARD_OPTION==VEISE_CARDVR)
            IsuOVL_WSP      = 205 | (169 << 16);
            IsuOVL_WEP      = 584 | (188 << 16);
            IsuOVLPAL1      = 0x008080ff;   // white
            IsuOVLPAL2      = 0x009400e1;   // yellow
            IsuOVLPAL3      = 0x00ff544c;   // red
            #elif((HW_BOARD_OPTION==EVERSPRING_DVRBOX)||(HW_BOARD_OPTION==ES_LIGHTING))
            IsuOVL_WSP      = 245 | (219 << 16);
            IsuOVL_WEP      = 624 | (228 << 16);
            IsuOVLPAL1      = 0x008080ff;   // white
            IsuOVLPAL2      = 0x009400e1;   // yellow
            IsuOVLPAL3      = 0x00ff544c;   // red
            #elif((HW_BOARD_OPTION==RDI_DOORPHONE_774)||(HW_BOARD_OPTION==STONE_DVRBOX))
            IsuOVL_WSP      = 237 | (216 << 16);
            IsuOVL_WEP      = 624 | (235 << 16);
            IsuOVLPAL1      = 0x008080ff;   // white
            IsuOVLPAL2      = 0x009400e1;   // yellow
            IsuOVLPAL3      = 0x00ff544c;   // red
            #elif(HW_BOARD_OPTION==SKYBEST_DVRBOX)
            IsuOVL_WSP      = 241 | (199 << 16);
            IsuOVL_WEP      = 624 | (238 << 16);
            IsuOVLPAL1      = 0x008080ff;   // white
            IsuOVLPAL2      = 0x009400e1;   // yellow
            IsuOVLPAL3      = 0x00ff544c;   // red
            #elif(HW_BOARD_OPTION==SUPER_POWER)
        ...
        else    /*PAL*/
        {
    		...
    		#if (HW_BOARD_OPTION==DTY_IRDVR)
            IsuOVL_WSP      = 17 |  ( 15 << 16);
            IsuOVL_WEP      = 636 | ( 44 << 16);
    		#elif(HW_BOARD_OPTION==WINNIN_IRDVR)
            IsuOVL_WSP      = 245 | (254 << 16);
            IsuOVL_WEP      = 624 | (273 << 16);
            #elif(HW_BOARD_OPTION==JSW_DVRBOX)
            IsuOVL_WSP      = 165 | (250 << 16);
            IsuOVL_WEP      = 624 | (259 << 16);
            #elif(HW_BOARD_OPTION==JSY_DVRBOX)
            IsuOVL_WSP      = 165 | (250 << 16);
            IsuOVL_WEP      = 624 | (259 << 16);
    		#else
            ...
            IsuOVLPAL2      = 0x009400e1;   // yellow
            IsuOVLPAL3      = 0x00ff544c;   // red
            #elif ((HW_BOARD_OPTION==AV9AV_DVR)||(HW_BOARD_OPTION==SEENEDGE_CARDVR))
            IsuOVL_WSP      =  25 | (  9 << 16);
            IsuOVL_WEP      = 624 | ( 28 << 16);
            //IsuOVLPAL1      = 0x008080ff;   // white
            IsuOVLPAL1      = 0x006bff1d;   // Blue
            ...
            IsuOVLPAL3      = 0x00ff544c;   // red
            #elif(HW_BOARD_OPTION==JSW_DVRBOX)
            IsuOVL_WSP      = 165 | (267 << 16);
            IsuOVL_WEP      = 624 | (276 << 16);
            IsuOVLPAL1      = 0x008080ff;   // white
            IsuOVLPAL2      = 0x009400e1;   // yellow
            IsuOVLPAL3      = 0x00ff544c;   // red
            #elif(HW_BOARD_OPTION==AOYA_DVRBOX)
            IsuOVL_WSP      = 225 | (267 << 16);
            IsuOVL_WEP      = 624 | (276 << 16);
            IsuOVLPAL1      = 0x008080ff;   // white
            IsuOVLPAL2      = 0x009400e1;   // yellow
            IsuOVLPAL3      = 0x00ff544c;   // red
            ...
            IsuOVLPAL3      = 0x00ff544c;   // red
            #elif((HW_BOARD_OPTION==RDI_DOORPHONE_774)||(HW_BOARD_OPTION==STONE_DVRBOX))
            IsuOVL_WSP      = 237 | (252 << 16);
            IsuOVL_WEP      = 624 | (271 << 16);
            IsuOVLPAL1      = 0x008080ff;   // white
            IsuOVLPAL2      = 0x009400e1;   // yellow
            IsuOVLPAL3      = 0x00ff544c;   // red
            #elif((HW_BOARD_OPTION==SKYBEST_DVRBOX))
            IsuOVL_WSP      = 249 | (263 << 16);
            IsuOVL_WEP      = 624 | (282 << 16);
            IsuOVLPAL1      = 0x008080ff;   // white
            IsuOVLPAL2      = 0x009400e1;   // yellow
            IsuOVLPAL3      = 0x00ff544c;   // red
             #elif(HW_BOARD_OPTION==SUPER_POWER)
            ...
            IsuOVLPAL2      = 0x009400e1;   // yellow
            IsuOVLPAL3      = 0x00ff544c;   // red
            #elif(HW_BOARD_OPTION==CWELL_DVRBOX)
            IsuOVL_WSP      = 249 | (263 << 16);
            IsuOVL_WEP      = 624 | (282 << 16);
            IsuOVLPAL1      = 0x008080ff;   // white
            IsuOVLPAL2      = 0x009400e1;   // yellow
    ...
    else  //Sensor-in
    {
        ...
        if( (mpeg4Width == 320) || (mpeg4Width == 352) ) //Lsk 090703 : QVGA frames
        {
            ...
            IsuOVL_WEP      = 310 | (234 << 16);
                ...
                IsuOVL_STRIDE   = 80;
                IsuOVL_WSP      =  4 | ( 13 << 16);
                IsuOVL_WEP      =315 | ( 48 << 16);
            #elif(HW_BOARD_OPTION==WINNIN_IRDVR)
                IsuOVL_STRIDE   = 80;
                IsuOVL_WSP      = 96 | (199 << 16);
                IsuOVL_WEP      =311 | (222 << 16);
            #elif(HW_BOARD_OPTION==VEISE_CARDVR)
                IsuOVL_STRIDE   = 80;
                IsuOVL_WSP      = 96 | (199 << 16);
                IsuOVL_WEP      =211 | (222 << 16);
            #else
                IsuOVL_STRIDE   = 80;
                IsuOVL_WSP      = 83 | (223 << 16);
                IsuOVL_WEP      =310 | (234 << 16);
        ...
        else
        {
            ...
            IsuOVL_WSP      =  25 | (  9<< 16);
            IsuOVL_WEP      = 624 | ( 38 << 16);
            #elif ((HW_BOARD_OPTION==SUNIN_CARDVR)||(HW_BOARD_OPTION==SUNIN1_CARDVR))
            IsuOVL_WSP      =  25 | (  9 << 16);
            IsuOVL_WEP      = 624 | ( 48 << 16);
            #elif (HW_BOARD_OPTION==ITS_CARDVR)
            IsuOVL_WSP      =  25 | (  14 << 16);
            IsuOVL_WEP      = 624 | ( 57 << 16);
            IsuOVLPAL1      = 0x008080ff;   // white
            ...
            IsuOVL_STRIDE   = 160;
            #if (HW_BOARD_OPTION==SUNIN1_CARDVR)
            IsuOVL_WSP      =  25 | (441 << 16);
            IsuOVL_WEP      = 624 | (456 << 16);
            IsuOVLPAL1      = 0x009400e1;   // yellow
            IsuOVLPAL2      = 0x008080ff;   // white
            IsuOVLPAL3      = 0x00ff544c;   // red
            #endif
        #endif
        }
...
s32 isuVideoZoom(s32 zoomFactor)
{
    ...
    if(sysTVOutOnFlag) //TV out
    {
        ...
        // 0x0008
#if((HW_BOARD_OPTION==WINNIN_IRDVR)||(HW_BOARD_OPTION==KONLIKA_DVRBOX)||(HW_BOARD_OPTION==KD_DVRBOX)||(HW_BOARD_OPTION==AV9AV_DVR)\
    ||(HW_BOARD_OPTION==SEENEDGE_CARDVR) ||(HW_BOARD_OPTION==SUNIN1_CARDVR))
		if (gu8TimeStamp==1)
		{
			IsuSCA_FUN =
				ISU_PKOUT_ENA|
...
#elif((HW_BOARD_OPTION==ELEGANT_KFCDVR)||(HW_BOARD_OPTION==GOS_DVRBOX)||(HW_BOARD_OPTION == VER100_CARDVR)||(HW_BOARD_OPTION==SUNIN_CARDVR)||(HW_BOARD_OPTION==ITS_CARDVR))
		if (ShowOverlayString)
		{
			IsuSCA_FUN =
                ISU_PKOUT_ENA|
                ISU_PNOUT_ENA|
    ...
    else //Pannel out (only for small size pannel)
    {
        ...
        #if((HW_BOARD_OPTION == KONLIKA_DVRBOX)||(HW_BOARD_OPTION==KD_DVRBOX)||(HW_BOARD_OPTION==AV9AV_DVR)||(HW_BOARD_OPTION==SEENEDGE_CARDVR) )
		if (gu8TimeStamp==1)
		{
			IsuSCA_FUN =
				ISU_PKOUT_ENA|
     ...
     (CHIP_OPTION == CHIP_A1026A))
       ...
       #endif
	#else
        IsuFIFO_TH = 0x53535353; //Lucian: 避免Video clip時,LCD 上有雜點. 2008/2/25
    #endif
    }
...
/*BJ 0821 E*/
    ...
    */
    ...
    #endif
	#if (HW_BOARD_OPTION==DTY_IRDVR)
		DEBUG_ISU("Clear Overlay buf\n");
	    memset(ScalarOverlayImage, 0, 640 * 60 * 2);
	#else
		memset(ScalarOverlayImage, 0, 640 * 48 * 2);
	#endif
    #if CDVR_LOG
...
void isu_FCINTE_ena(void)
{
   IsuSCA_INTC |= ISU_FCINTE_ENA; //Lucian: Fixed HW bug, 此時再打開
}

void isu_FCINTE_disa(void)
{
...
s32 isuGenPauseFrame(u8 *Srcbuf , u8 *Dstbuf , u16 sizeX, u16 sizeY)
{
    ...
    */
    ...
    #if(HW_BOARD_OPTION==PROJECT_DW950_REAL)
        IsuPK_IADDR0    =   (u32)Dstbuf+( (480-Disp_X) );
        IsuPK_STRIDE    =   (PANNEL_X<<1);
    #endif
