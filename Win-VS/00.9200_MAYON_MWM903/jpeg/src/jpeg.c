...
#define JPEG_TIMEOUT            20

#define JPEG_TIMEOUT_S           3
#define JPEG_TIMEOUT_M           8
#define JPEG_TIMEOUT_L          13
#else
#define JPEG_TIMEOUT            100  /*CY 1023*/

#define JPEG_TIMEOUT_S           3
#define JPEG_TIMEOUT_M          15
#define JPEG_TIMEOUT_L          50
#endif
...
s32 jpegInit(void)
{
	#if 1//(HW_BOARD_OPTION==MR9300_216M_EVB || HW_BOARD_OPTION==MR9300_RX_RDI)
	//Lsk: avoid interrupt timeout
	SYS_CLK1    = (SYS_CLK1 & ~0x0c000000) | 0x0C000000; //JPEG VLC Clock Divisor. The clock divisor value = Divisor + 1.
	#else
...
void jpegIntHandler(void)
{
    ...
    else if((intStat & 0x00000080) && (jpegSemDec_flag & 2))   // JPEG decoder block mode interrupt
    {
		...
		printf("JJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJJ\n");
	#endif
/*BJ 0523 E*/
}
...
#if ADDAPP3TOJPEG
//
s32 jpegCaptureAPP3Image(u8* pBuf, s16 JpegOpMode)
{
    u32 MCU_no;
    ...
    */
    ...
    JPEG_Enc( 320,                          // Image width
              240,                          // Image height
              0,                            // Bitstream format, 0: YUV422, 1: YUV420
              jpegRestart,                  // Restart interval
              JpegOpMode,                   // 1: Slice mode, 0: frame mode
              PKBuf0,                       // Image address 0, for both frame mode and slice mode, must align 32. Y data, if DataType == YUV420.
              PKBuf1,                       // Image address 1, for slice mode only, must align 32. UV data, if DataType == YUV420.
              PKBuf2,                       // Image address 2, for slice mode only, must align 32.
              pBuf,                         // Bitstream address, must align 4
              0,                            // for A1018, image data type, 0:YUV422, 1:YUV420.
              320 * 2);                     // for A1018, stride in bytes.
...
s32 jpegDecompression(u8* pBuf , u8* pResult)
{
    ...
    if(splitmenu==1)
    {
		...
		pucSrcAddr = (u32*)pResult;
        pucDstAddr = (u32*)PKBuf0;
...
#if ( (CHIP_OPTION == CHIP_A1018B)  || (CHIP_OPTION == CHIP_A1019A) ||(CHIP_OPTION == CHIP_A1025A) || (CHIP_OPTION == CHIP_A1021A)  )// for JPEG_BRI_DAT_TYPE_YUV422
/*
                                        ...
                                        u32 Stride)     // for A1018, stride in bytes. Only valid when DataType == YUV420.
{
    ...
    case JPEG_IMG_420:
        ...
        break;
    case JPEG_IMG_440:
        DEBUG_JPEG("Error: Error A1018B jpeg YUV440\n");
        JPEG_BRI_CTRL   = JPEG_BRI_MODE_DEC |
                          (DataType << 2) |
                          JPEG_BRI_DAT_FMT_YUV440;
        break;
    case JPEG_IMG_444:
        DEBUG_JPEG("Error: Error A1018B jpeg YUV444\n");
        JPEG_BRI_CTRL   = JPEG_BRI_MODE_DEC |
                          (DataType << 2) |
                          JPEG_BRI_DAT_FMT_YUV444;
        break;
	...
	McuNumber = ((JpegCtrl&0xffff0000)>>16)|((JpegCtrl&0x00003f00)<<8);

	if(McuNumber < 100)
		timeout = JPEG_TIMEOUT_S;
	else if(McuNumber < 500)
		timeout = JPEG_TIMEOUT_M;
	else	
		timeout = JPEG_TIMEOUT_L;
	
	OSSemPend(jpegSemEvt, timeout, &err);	
    //OSSemPend(jpegSemEvt, JPEG_TIMEOUT, &err);  //Lsk: Jpeg decoder may lose interrupt.
    OSSemPost(mpeg4ReadySemEvt);
...
s32 jpegRAW420(u8* pBuf,       // Bitstream address, must align 4
                    u8* pResultY,   // Output image address, must align 32. Y data, if DataType == YUV420. YUV422 data, if DataType == YUV422.
                    u8* pResultUV,  // Output image address, must align 32. UV data, if DataType == YUV420. Invalid when DataType == YUV422.
                    u16 scaler_w ,
                    u16 scaler_h,
                    u32 Stride)
{
    ...
    {
        addrY = pBuf + j*scaler_w;
        addrC = pBuf+(scaler_w*scaler_h)+ j*scaler_w;
        dateY = pResultY + j*Stride;
        dataC = pResultUV+ j*Stride;
        memcpy(dateY ,addrY, (u32) (scaler_w));
        if (j < scaler_h/2)
            memcpy(dataC ,addrC, (u32) (scaler_w));
    }
