...
#if REAL_CHIP
#if SDC_CMD_TIMEOUT_LONG
#define SDC_TIMEOUT        600
#else
...
#if SDC_CMD_TIMEOUT_LONG
#define SDC_MAX_ACMD_LOOP        100000
#else
#define SDC_MAX_ACMD_LOOP        8192
#endif
...
OS_STK sdcTaskStack[SDC_DEVICE_TASK_STACK_SIZE];
SDC_INT_EVT sdcIntEvt;

OS_EVENT *sdcSemTaskEvt;
OS_EVENT *sdcSemTaskHandleEvt;
#endif
...
#if SD_TASK_INSTALL_FLOW_SUPPORT
u32 sdcLastInsertedStatus = SDC_CD_OFF;
#endif
...
int SDInsertedDetect(void)
{
#if SDC_SDCD_ENA
    if(SdcStat & SDC_DETECT)
        return SDC_CD_OFF;
    else
#endif
        return SDC_CD_IN;
...
#else
    guiSDReadDMAId = marsDMAReq(DMA_REQ_SD2_READ, &dmaCfg);
#endif
...
#if !SD1_SWITCH_SD2
    guiSDReadDMAId = marsDMAReq(DMA_REQ_SD_READ_FAST, &dmaCfg);
#else
    guiSDReadDMAId = marsDMAReq(DMA_REQ_SD2_READ_FAST, &dmaCfg);
#endif
...
#if !SD1_SWITCH_SD2
    guiSDWriteDMAId = marsDMAReq(DMA_REQ_SD_WRITE_FAST, &dmaCfg);
#else
    guiSDWriteDMAId = marsDMAReq(DMA_REQ_SD2_WRITE_FAST, &dmaCfg);
#endif
...
s32 sdcCardIdentificationMode2Dot0(void)
{
    ...
    u32 count;
    //u8 ucTestHighCap = 0;	it never used
...
s32 sdcGetCsd2Dot0(void)
{
    ...
    {
        dcfStorageSize[STORAGE_MEMORY_SD_MMC] = FS_MEDIA_SD_512GB;
    }
    else if (capacity > 252046848) /* 4GB tolerance for 256GB */
    {
        dcfStorageSize[STORAGE_MEMORY_SD_MMC] = FS_MEDIA_SD_256GB;
    }
    else if (capacity > 125023424) /* 4GB tolerance for 128GB */
    {
        dcfStorageSize[STORAGE_MEMORY_SD_MMC] = FS_MEDIA_SD_128GB;
    }
    else if (capacity > 61962087) /* 2048MB tolerance for 64GB */
    {
        dcfStorageSize[STORAGE_MEMORY_SD_MMC] = FS_MEDIA_SD_64GB;
    }
    else if (capacity > 31459280) /* 2048MB tolerance for 32GB */
    {
        dcfStorageSize[STORAGE_MEMORY_SD_MMC] = FS_MEDIA_SD_32GB;
    }
    else if (capacity > 14680064) /* 2048MB tolerance for 16GB */
    {
        dcfStorageSize[STORAGE_MEMORY_SD_MMC] = FS_MEDIA_SD_16GB;
    }
    else if (capacity > 7562500) /* 256MB tolerance for 8GB */
    {
        dcfStorageSize[STORAGE_MEMORY_SD_MMC] = FS_MEDIA_SD_8GB;
    }
    else if (capacity > 3656250) /* 256MB tolerance for 4GB */
    {
        dcfStorageSize[STORAGE_MEMORY_SD_MMC] = FS_MEDIA_SD_4GB;
    }
    else if (capacity > 1703125) /* 256MB tolerance for 2GB */
    {
        dcfStorageSize[STORAGE_MEMORY_SD_MMC] = FS_MEDIA_SD_2GB;
    }
    else if (capacity > 726562) /* 256MB tolerance for 1GB */
    {
        dcfStorageSize[STORAGE_MEMORY_SD_MMC] = FS_MEDIA_SD_1GB;
    }
    else if (capacity > 375000) /* 128MB tolerance for 512MB */
    {
        dcfStorageSize[STORAGE_MEMORY_SD_MMC] = FS_MEDIA_SD_512MB;
    }
    else if (capacity > 187500) /* 64MB tolerance for 256MB */
    {
        dcfStorageSize[STORAGE_MEMORY_SD_MMC] = FS_MEDIA_SD_256MB;
    }
    else if (capacity > 93750) /* 32MB tolerance for 128MB */
    {
        dcfStorageSize[STORAGE_MEMORY_SD_MMC] = FS_MEDIA_SD_128MB;
    }
    else if (capacity > 46875) /* 16MB tolerance for 64MB */
    {
        dcfStorageSize[STORAGE_MEMORY_SD_MMC] = FS_MEDIA_SD_64MB;
    }
    else if (capacity > 23437) /* 8MB tolerance for 32MB */
    {
        dcfStorageSize[STORAGE_MEMORY_SD_MMC] = FS_MEDIA_SD_32MB;
    }
    else if (capacity > 11718) /* 4MB tolerance for 16MB */
    {
        dcfStorageSize[STORAGE_MEMORY_SD_MMC] = FS_MEDIA_SD_16MB;
    }
    else
    {
        dcfStorageSize[STORAGE_MEMORY_SD_MMC] = 0;
    }
    DEBUG_USB("==>SDC dcfStorageSize=%d,%d\n",dcfStorageSize[STORAGE_MEMORY_SD_MMC],capacity);
...
s32 sdcSwitchFunction(u32 param, u8* swStat)
{
    ...
    /* set read data dma */
    if(sdcSetReadStatusDma(swStat, SDC_SWITCH_LEN) != 1)
    {
...
s32 sdcDataTransferMode(u32 busWidth)
{
    ...
    DEBUG_SDC("Trace: Default mode %dHz,sdcTryInvertSDClk=%d\n", SYS_CPU_CLK_FREQ/(clkDiv+1),sdcTryInvertSDClk );
#endif
    //free(swStat);	// avoid warning message
    return SD_OK;
}
...
s32 sdcErase(u32 startAddr, u32 endAddr)
{
    ...
    if (sdcDetectCard() != SD_OK)
    {
        ...
        sysSetStorageStatus(tmpVal, SYS_V_STORAGE_NREADY);
        return -2;
    }
    /* check write protect */
#if SDC_WRPROT_ENA
...
s32 sdcInit(void)
{
...
#if REAL_CHIP
    SdcDmaTrig = SDC_DMA_INC16 | SDC_DMA_TRIG_16W16R; /* DMA trigger after 4 words in fifo */
#else
    SdcDmaTrig = SDC_DMA_INC16 | SDC_DMA_TRIG_16W16R; /* DMA trigger after 4 words in fifo */
#endif
#if SD_TASK_INSTALL_FLOW_SUPPORT
    ...
    if(sdcSemEvt != NULL)
    {
        ...
        if (err != OS_NO_ERR)
            DEBUG_SDC("[E] SDC Del sdcSemEvt failed: %d\n", err);
    }

    if(sdcSemMulTx_INT != NULL)
    {
        sdcSemMulTx_INT = OSSemDel(sdcSemMulTx_INT, OS_DEL_ALWAYS, &err);
        if (err != OS_NO_ERR)
            DEBUG_SDC("[E] SDC Del sdcSemMulTx_INT failed: %d\n", err);
    }

#if USE_TX_INT
    if(sdcSemTx_INT != NULL)
    {
        sdcSemTx_INT = OSSemDel(sdcSemTx_INT, OS_DEL_ALWAYS, &err);
        if(err != OS_NO_ERR)
            DEBUG_SDC("[E] SDC Del sdcSemTx_INT failed: %d\n", err);
    }
#endif
...
void sdcIntHandler(void)
{
    ...
    if (intStat & SDC_CARD_DETECT)
    {
        ...
        {
            DEBUG_SDC("SD Remove!!\n");
            sdcSetIntEvt(SDC_TASK_EVT_UNMOUNT);
        }
        else
        {
            DEBUG_SDC("SD Card In!!\n");
            sdcSetIntEvt(SDC_TASK_EVT_MOUNT);
        }
#else
        if(((SdcStat & SDC_DETECT) >> 2) == SDC_CD_OFF )
        {
        ...
        if(!uiSentKeyToUi(UI_KEY_SDCD))
            sysSetUIKeyRetry(SYS_V_STORAGE_SDC);
#endif
    }
...
s32 sdcPowerOnRest(void)
{
...
#if SDC_PWR_LOW_START
    gpioGetLevel(GPIO_POWEROFFGRP_SDC, GPIO_POWEROFFBIT_SDC, &Lvl);
    if(Lvl == 1) return 1;
#endif
    ...
    DEBUG_SDC("======= sd2PowerOnRest ===========\n");
    //Gpio-0: 19~23, 28~29, except: 29(CD)
    Gpio0Ena |= 0x10F80000;
    Gpio0Dir &= (~0x10F80000);
    Gpio0Level &= (~0x10F80000);
	...
	gpioSDPwrSwitch(0);
#else
	gpioSetLevel(GPIO_POWEROFFGRP_SDC, GPIO_POWEROFFBIT_SDC, Lvl);
#endif
	OSTimeDly(3);

#if(CHIP_OPTION == CHIP_A1025A)

	Gpio0Ena = gpio1_en & (~0x10F80000);
	Gpio0Dir = gpio1_dir;
...
#if SD_TASK_INSTALL_FLOW_SUPPORT
int sdcSetIntEvt(u8 Cause)
{
    u8 err;
    // check if cause is valid
    if(Cause >= SDC_TASK_EVT_UNDEF)
    {
        DEBUG_SDC("[W] SDC SEvt(%d) fail.\n", Cause);
        return 0;
    }

    OSSemPend(sdcSemTaskHandleEvt, OS_IPC_WAIT_FOREVER, &err);
    sdcIntEvt.cause[sdcIntEvt.idxSet] = Cause;
    //DEBUG_SDC("[I] SDC SEvt %d idx %d\n", Cause, sdcIntEvt.idxSet);

    sdcIntEvt.idxSet = (sdcIntEvt.idxSet + 1) & (SDC_DEV_MAX_INT_EVT - 1);
    OSSemPost(sdcSemTaskHandleEvt);

    // check if event queue is full
    if(sdcIntEvt.idxSet == sdcIntEvt.idxGet)
    {
        DEBUG_SDC("[W] SDC Task evt full.\n");
        return 0;
    }

    OSSemPost(sdcSemTaskEvt);
    return 1;
}

int sdcGetIntEvt(u8 *pCause)
{
    u8 err;
    // check if event queue is empty
    if(sdcIntEvt.idxGet == sdcIntEvt.idxSet)
    {
        DEBUG_SDC("[I] SDC GEvt empty.\n");
        return 0;
    }

    OSSemPend(sdcSemTaskHandleEvt, OS_IPC_WAIT_FOREVER, &err);
    *pCause = sdcIntEvt.cause[sdcIntEvt.idxGet];
    //DEBUG_SDC("[I] SDC GEvt %d idx %d\n", *pCause, sdcIntEvt.idxGet);

    sdcIntEvt.idxGet = (sdcIntEvt.idxGet + 1) & (SDC_DEV_MAX_INT_EVT - 1);
    OSSemPost(sdcSemTaskHandleEvt);

    // check if cause is valid
    if(*pCause >= SDC_TASK_EVT_UNDEF)
        return 0;
...
int sdcMountSeq(void)
{
    ...
    if(sdcLastInsertedStatus == SDC_CD_IN)
    {
        DEBUG_SDC("[I] SDC Multi int happened.\n");
        return 1;
    }
    sdcLastInsertedStatus = SDC_CD_IN;
    ...
    while((Status < 0) && (Cnt <= 3));

    if(Status == SD_OK)
        sysSentMountSeq(SYS_V_STORAGE_SDC);

    return 1;
}

int sdcUnmountSeq(void)
{
    int Cnt, Status;
    ...
    if(sdcLastInsertedStatus == SDC_CD_OFF)
    {
        DEBUG_SDC("[I] SDC Multi int happened.\n");
        return 1;
    }
    sdcLastInsertedStatus = SDC_CD_OFF;

    sysSentMountSeq(SYS_V_STORAGE_SDC);
    //sysSentUnMountSeq(SYS_V_STORAGE_SDC);
...
int (*sdcIntEvtFunc[])(void) =
{
    sdcMountSeq,
    sdcUnmountSeq,
};

void sdcTask(void *pData)
{
    u8 err, cause;

    while(1)
    {
        if(sdcGetIntEvt(&cause))
        {
            //DEBUG_YELLOW("W: cause: %d\n", cause);
            (*sdcIntEvtFunc[cause])();
            //DEBUG_GREEN("P: cause: %d\n", cause);
        }
        else
        {
            OSSemPend(sdcSemTaskEvt, OS_IPC_WAIT_FOREVER, &err);
            if(err != OS_NO_ERR)
            {
                DEBUG_SDC("[E] sdcSemTaskEvt: %d.\n", err);
                return ;
            }
        }
    }
...
int sdcTaskInit(void)
{
    sdcInit();

    sdcSemTaskEvt = OSSemCreate(0);
    sdcSemTaskHandleEvt = OSSemCreate(1);

    memset((void *)&sdcIntEvt, 0, sizeof(SDC_INT_EVT));
    if(0 == OSTaskCreate(SDC_TSAK, SDC_TASK_PARAMETER, SDC_DEVICE_TASK_STACK, SDC_TASK_PRIORITY))
        DEBUG_SDC("[I] SDC Task is OK.\n");

    if(SDInsertedDetect() == SDC_CD_IN)
        sdcSetIntEvt(SDC_TASK_EVT_MOUNT);
    return 1;
}
#endif
