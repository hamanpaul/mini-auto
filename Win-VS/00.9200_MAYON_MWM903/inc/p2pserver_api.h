...
#define NET_LIST_ITEM_NUMBER    80
#define NET_MORE_EVENT_LIMIT   160

enum
{
    NET_FILE_SYSTEM_ACCESS_BY_IDLE = 0,
    NET_FILE_SYSTEM_ACCESS_BY_APP,
    NET_FILE_SYSTEM_ACCESS_BY_RX,
};
...
#if LWIP2_SUPPORT
#define MAX_NTP_NUM 3
#define NTP_SERV (char*[MAX_NTP_NUM])   {   "0.pool.ntp.org",\
                                            "0.ca.pool.ntp.org",\
                                            "time.stdtime.gov.tw"}
#endif
/*Event Define*/
...
/* Function prototype */
extern int StreamingCH_by_APP(void); /*Return which channel was streaming by APP.*/
extern void Check_P2P_info(unsigned int *p2p_info);/*if p2p_info equals P2P_STATUS_OK(0x07), means the Device have connected to the P2P server.*/
extern s32 P2PSendEvent(u32 camidx,u32 eventType);
...
extern s32 Upgrade_fw_net(u32 dummy, u32 dummy2);/*Upgrade firmware via internet.*/
extern void NTP_Switch(int OnOff);/*Enable or disable get NTP time,ENABLE=1,DISABLE=0).*/
#if LWIP2_SUPPORT
extern void ntpdate(void);/*Get current time from NTP server.*/
#else
extern s32 ntpdate(u32 dummy, u32 dummy2);
#endif
...
extern void SetNetworkInfo(struct NetworkInfo *info);
extern void GetNetworkInfo(struct NetworkInfo *info);

#if ISP_NEW_UPGRADE_FLOW_SUPPORT
extern void netSetFirmwareName(char *Version, u32 Length);
extern char *netGetFirmwareName(void);
extern int netDoDownloadFile(char *pPath, u8 *BufAddr);
extern int netGetVersionFileInfo(u8 *pSrcBuf, u32 LenOfBuf, u32 *ReadSize);
#endif

#if (NET_STATUS_POLLING && NIC_SUPPORT)
extern void phyNETRunPerSec(void);
#endif
