...
#include "H264api.h"
#include "../VideoCodec/H264/inc/Parset.h"
#endif
...
typedef struct _VIDEO_CLIP_OPTION
{
    ...
    u8 				ShowDebugMsgFlag;  //Lsk: ToDebug 1. stop record for schedule. 2. pending
    u8 				TryErrCnt;  	   //Lsk: pending issue, workaround
    #endif
    u8              VideoChannelID;
    ...
    u32             CurrentVideoSize;   //only big stream for asf record
    u32             CurrentBufferSize;  //big/small stream in buffer
    ...
    u8              EncodeLineStripe; //default 320x240
    u8              EncodeDownSample;

    //step 1. LineStripe step 2. downsample
    u32             FrameWidth;
    u32             LineStripeFrameWidth;
    u32             H_addr;
    u32             V_addr;
...
//aher test
    ...
    INT32U          guiIISRecDMAId, guiIISPlayDMAId;
    ...
    u8              paddingBytes[ASF_HEADER_SIZE_CEIL]; //Lsk 090309
...
u8 MultiChannelCheckRecordChannel(void);
