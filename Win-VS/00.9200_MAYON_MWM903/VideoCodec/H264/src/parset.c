...
int GenerateSeq_parameter_set_rbsp (seq_parameter_set_rbsp_t *sps, u8 *rbsp, Bitstream *bitstream);
//int GenerateSeq_parameter_set_NALU (NALU_t *);
int GenerateSeq_parameter_set_NALU (NALU_t *n, seq_parameter_set_rbsp_t *sps, Bitstream *bitstream);

void GeneratePictureParameterSet(pic_parameter_set_rbsp_t *, seq_parameter_set_rbsp_t *);
//int GeneratePic_parameter_set_rbsp (pic_parameter_set_rbsp_t *, unsigned char *, Bitstream *bitstream);
int GeneratePic_parameter_set_rbsp (pic_parameter_set_rbsp_t *pps, unsigned char *rbsp, Bitstream *bitstream);
//int GeneratePic_parameter_set_NALU(NALU_t *);
int GeneratePic_parameter_set_NALU(NALU_t *n, pic_parameter_set_rbsp_t *pps, Bitstream *bitstream);
//////////////////////////////////////////////////////////
//
 ...
 *************************************************************************************
*/
int GenerateParameterSets_SW (H264_ENC_CFG* cfg, u8 *buffer, pic_parameter_set_rbsp_t *pps, seq_parameter_set_rbsp_t *sps, Bitstream *bitstream)
{ 
  ...
  GenerateSequenceParameterSet(cfg, sps);
  length = GenerateSeq_parameter_set_NALU(&n, sps, bitstream);
  ...
  */
  ...
  GeneratePictureParameterSet (pps, sps);
  length += GeneratePic_parameter_set_NALU(&n, pps, bitstream);
  /*	
 ...
 *************************************************************************************
*/
int Rfiu_GenerateParameterSets_SW (H264_ENC_CFG* cfg, u8 *buffer, NALU_t *n, pic_parameter_set_rbsp_t *pps, seq_parameter_set_rbsp_t *sps, Bitstream *bitstream)
{ 
...
{  
  ...
  RBSPlen = GenerateSeq_parameter_set_rbsp (sps, rbsp, bitstream);
...
int GeneratePic_parameter_set_NALU(NALU_t *n, pic_parameter_set_rbsp_t *pps, Bitstream *bitstream)
{
  ...
  RBSPlen = GeneratePic_parameter_set_rbsp (pps, rbsp, bitstream);
...
void GenerateSequenceParameterSet(H264_ENC_CFG* cfg, seq_parameter_set_rbsp_t *sps) 
{
    ...
    sps->constrained_set0_flag = 1;
    sps->constrained_set1_flag = 1;
    sps->constrained_set2_flag = 1;
    ...
    {
        //TO DO : H1 H264 IP don't support
        /*
    ...
    sps->log2_max_frame_num_minus4 = (16 - 4);             
    ...
    if (sps->vui_parameters_present_flag == TRUE)
    {
        ...
        if (sps->vui_seq_parameters.bitstream_restriction_flag == TRUE)
        {
            ...
            sps->vui_seq_parameters.log2_max_mv_length_vertical = 7;
...
void GeneratePictureParameterSet( pic_parameter_set_rbsp_t *pps, seq_parameter_set_rbsp_t *sps)
{
    ...
    pps->deblocking_filter_control_present_flag = 1; //Lsk note
...
int GenerateSeq_parameter_set_rbsp (seq_parameter_set_rbsp_t *sps, u8 *rbsp, Bitstream *bitstream)
{
    ...
    if(sps->vui_parameters_present_flag == TRUE)
    {
        //TO DO : CCU H264 IP don't support
     #if H1_264TEST_ENC   
        len+=encode_u_1 (sps->vui_seq_parameters.aspect_ratio_info_present_flag, bitstream);
        len+=encode_u_1 (sps->vui_seq_parameters.overscan_info_present_flag, bitstream);
        len+=encode_u_1 (sps->vui_seq_parameters.video_signal_type_present_flag, bitstream);
        len+=encode_u_1 (sps->vui_seq_parameters.chroma_location_info_present_flag, bitstream);
        len+=encode_u_1 (sps->vui_seq_parameters.timing_info_present_flag, bitstream);
        if(sps->vui_seq_parameters.timing_info_present_flag == TRUE)
        {
            len+=encode_u_1 (sps->vui_seq_parameters.num_units_in_tick, bitstream);
            len+=encode_ue_v (sps->vui_seq_parameters.time_scale, bitstream);
            len+=encode_u_1 (sps->vui_seq_parameters.fixed_frame_rate_flag, bitstream);
        }
        len+=encode_u_1 (sps->vui_seq_parameters.nal_hrd_parameters_present_flag, bitstream);
        len+=encode_u_1 (sps->vui_seq_parameters.vcl_hrd_parameters_present_flag, bitstream);
        len+=encode_u_1 (sps->vui_seq_parameters.video_signal_type_present_flag, bitstream);
        len+=encode_u_1 (sps->vui_seq_parameters.bitstream_restriction_flag, bitstream);
        if (sps->vui_seq_parameters.bitstream_restriction_flag == TRUE)
        {
            len+=encode_u_1 (sps->vui_seq_parameters.motion_vectors_over_pic_boundaries_flag, bitstream);
            len+=encode_u_1 (sps->vui_seq_parameters.max_bytes_per_pic_denom, bitstream);
            len+=encode_u_1 (sps->vui_seq_parameters.max_bits_per_mb_denom, bitstream);
            len+=encode_ue_v (sps->vui_seq_parameters.log2_max_mv_length_vertical, bitstream);
            len+=encode_ue_v (sps->vui_seq_parameters.log2_max_mv_length_horizontal, bitstream);
            len+=encode_u_1 (sps->vui_seq_parameters.num_reorder_frames, bitstream);
            len+=encode_u_1 (sps->vui_seq_parameters.max_dec_frame_buffering, bitstream);
        }
        #endif
    }
...
int ReadVUI(Bitstream *p, seq_parameter_set_rbsp_t *sps)
{
  Bitstream *s = p;
  if (sps->vui_parameters_present_flag)
  {
    sps->vui_seq_parameters.aspect_ratio_info_present_flag = decode_u_1  ("VUI: aspect_ratio_info_present_flag"   , s);
    if (sps->vui_seq_parameters.aspect_ratio_info_present_flag)
    {
      sps->vui_seq_parameters.aspect_ratio_idc             = decode_u_v  ( 8, "VUI: aspect_ratio_idc"              , s);
      if (255==sps->vui_seq_parameters.aspect_ratio_idc)
      {
        sps->vui_seq_parameters.sar_width                  = decode_u_v  (16, "VUI: sar_width"                     , s);
        sps->vui_seq_parameters.sar_height                 = decode_u_v  (16, "VUI: sar_height"                    , s);
      }
  }

    sps->vui_seq_parameters.overscan_info_present_flag     = decode_u_1  ("VUI: overscan_info_present_flag"        , s);
    if (sps->vui_seq_parameters.overscan_info_present_flag)
    {
      sps->vui_seq_parameters.overscan_appropriate_flag    = decode_u_1  ("VUI: overscan_appropriate_flag"         , s);
    }

    sps->vui_seq_parameters.video_signal_type_present_flag = decode_u_1  ("VUI: video_signal_type_present_flag"    , s);
    if (sps->vui_seq_parameters.video_signal_type_present_flag)
    {
      sps->vui_seq_parameters.video_format                    = decode_u_v  ( 3,"VUI: video_format"                      , s);
      sps->vui_seq_parameters.video_full_range_flag           = decode_u_1  (   "VUI: video_full_range_flag"             , s);
      sps->vui_seq_parameters.colour_description_present_flag = decode_u_1  (   "VUI: color_description_present_flag"    , s);
      if(sps->vui_seq_parameters.colour_description_present_flag)
      {
        sps->vui_seq_parameters.colour_primaries              = decode_u_v  ( 8,"VUI: colour_primaries"                  , s);
        sps->vui_seq_parameters.transfer_characteristics      = decode_u_v  ( 8,"VUI: transfer_characteristics"          , s);
        sps->vui_seq_parameters.matrix_coefficients           = decode_u_v  ( 8,"VUI: matrix_coefficients"               , s);
      }
    }
    sps->vui_seq_parameters.chroma_location_info_present_flag = decode_u_1  (   "VUI: chroma_loc_info_present_flag"      , s);
    if(sps->vui_seq_parameters.chroma_location_info_present_flag)
    {
      sps->vui_seq_parameters.chroma_sample_loc_type_top_field     = decode_ue_v  ( "VUI: chroma_sample_loc_type_top_field"    , s);
      sps->vui_seq_parameters.chroma_sample_loc_type_bottom_field  = decode_ue_v  ( "VUI: chroma_sample_loc_type_bottom_field" , s);
    }
    sps->vui_seq_parameters.timing_info_present_flag          = decode_u_1  ("VUI: timing_info_present_flag"           , s);
    if (sps->vui_seq_parameters.timing_info_present_flag)
    {
      sps->vui_seq_parameters.num_units_in_tick               = decode_u_v  (32,"VUI: num_units_in_tick"               , s);
      sps->vui_seq_parameters.time_scale                      = decode_u_v  (32,"VUI: time_scale"                      , s);
      sps->vui_seq_parameters.fixed_frame_rate_flag           = decode_u_1  (   "VUI: fixed_frame_rate_flag"           , s);
    }
    sps->vui_seq_parameters.nal_hrd_parameters_present_flag   = decode_u_1  ("VUI: nal_hrd_parameters_present_flag"    , s);
    if (sps->vui_seq_parameters.nal_hrd_parameters_present_flag)
    {
		DEBUG_H264("H264 not support nal_hrd_parameters_present_flag\n");	
      //ReadHRDParameters(p, &(sps->vui_seq_parameters.nal_hrd_parameters));
    }
    sps->vui_seq_parameters.vcl_hrd_parameters_present_flag   = decode_u_1  ("VUI: vcl_hrd_parameters_present_flag"    , s);
    if (sps->vui_seq_parameters.vcl_hrd_parameters_present_flag)
    {
		DEBUG_H264("H264 not support vcl_hrd_parameters_present_flag\n");	
    ...
    {
      sps->vui_seq_parameters.low_delay_hrd_flag             =  decode_u_1  ("VUI: low_delay_hrd_flag"                 , s);
    }
    sps->vui_seq_parameters.pic_struct_present_flag          =  decode_u_1  ("VUI: pic_struct_present_flag   "         , s);
    sps->vui_seq_parameters.bitstream_restriction_flag       =  decode_u_1  ("VUI: bitstream_restriction_flag"         , s);
    if (sps->vui_seq_parameters.bitstream_restriction_flag)
    {
      sps->vui_seq_parameters.motion_vectors_over_pic_boundaries_flag =  decode_u_1  ("VUI: motion_vectors_over_pic_boundaries_flag", s);
      sps->vui_seq_parameters.max_bytes_per_pic_denom                 =  decode_ue_v ("VUI: max_bytes_per_pic_denom"                , s);
      sps->vui_seq_parameters.max_bits_per_mb_denom                   =  decode_ue_v ("VUI: max_bits_per_mb_denom"                  , s);
      sps->vui_seq_parameters.log2_max_mv_length_horizontal           =  decode_ue_v ("VUI: log2_max_mv_length_horizontal"          , s);
      sps->vui_seq_parameters.log2_max_mv_length_vertical             =  decode_ue_v ("VUI: log2_max_mv_length_vertical"            , s);
      sps->vui_seq_parameters.num_reorder_frames                      =  decode_ue_v ("VUI: num_reorder_frames"                     , s);
      sps->vui_seq_parameters.max_dec_frame_buffering                 =  decode_ue_v ("VUI: max_dec_frame_buffering"                , s);
    }
  }
...
int InterpretSPS (seq_parameter_set_rbsp_t *sps) //Lsk : mapping spec 7.3.2.1
{
    ...
    if(sps->frame_cropping_flag)
    {
	DEBUG_H264("\t frame_cropping_rect_left_offset = %d\n"	,sps->frame_cropping_rect_left_offset	  );// 0
	DEBUG_H264("\t frame_cropping_rect_right_offset = %d\n"	,sps->frame_cropping_rect_right_offset	  );// 0
	DEBUG_H264("\t frame_cropping_rect_top_offset = %d\n"	,sps->frame_cropping_rect_top_offset	  );// 0
	DEBUG_H264("\t frame_cropping_rect_bottom_offset = %d\n",sps->frame_cropping_rect_bottom_offset	  );// 0
    }

	DEBUG_H264("vui_parameters_present_flag = %d\n"			,sps->vui_parameters_present_flag		  );// 0	 	
	
	DEBUG_H264("aspect_ratio_info_present_flag = %d\n"      ,sps->vui_seq_parameters.aspect_ratio_info_present_flag     );// 0
	DEBUG_H264("overscan_appropriate_flag = %d\n"           ,sps->vui_seq_parameters.overscan_appropriate_flag          );// 0
	DEBUG_H264("video_signal_type_present_flag = %d\n"      ,sps->vui_seq_parameters.video_signal_type_present_flag     );// 0
	DEBUG_H264("chroma_location_info_present_flag = %d\n"   ,sps->vui_seq_parameters.chroma_location_info_present_flag  );// 0
	DEBUG_H264("timing_info_present_flag = %d\n"            ,sps->vui_seq_parameters.timing_info_present_flag           );// 1
	if (sps->vui_seq_parameters.timing_info_present_flag)
	{
	    DEBUG_H264("\t num_units_in_tick = %d\n"               ,sps->vui_seq_parameters.num_units_in_tick                  );// 1
	    DEBUG_H264("\t time_scale = %d\n"                      ,sps->vui_seq_parameters.time_scale                         );// 60
	    DEBUG_H264("\t fixed_frame_rate_flag = %d\n"           ,sps->vui_seq_parameters.fixed_frame_rate_flag              );// 1
	}
	DEBUG_H264("nal_hrd_parameters_present_flag = %d\n"	    ,sps->vui_seq_parameters.nal_hrd_parameters_present_flag    );// 0
	DEBUG_H264("vcl_hrd_parameters_present_flag = %d\n"	    ,sps->vui_seq_parameters.vcl_hrd_parameters_present_flag    );// 0
	DEBUG_H264("pic_struct_present_flag = %d\n"             ,sps->vui_seq_parameters.pic_struct_present_flag            );// 0
	DEBUG_H264("bitstream_restriction_flag = %d\n"          ,sps->vui_seq_parameters.bitstream_restriction_flag         );// 1
	if(sps->vui_seq_parameters.bitstream_restriction_flag)
    {
	    DEBUG_H264("\tmotion_vectors_over_pic_boundaries_flag = %d\n" ,sps->vui_seq_parameters.motion_vectors_over_pic_boundaries_flag    );// 1
	    DEBUG_H264("\tmax_bytes_per_pic_denom = %d\n"                 ,sps->vui_seq_parameters.max_bytes_per_pic_denom                    );// 0
	    DEBUG_H264("\tmax_bits_per_mb_denom = %d\n"                   ,sps->vui_seq_parameters.max_bits_per_mb_denom                      );// 0
	    DEBUG_H264("\tlog2_max_mv_length_horizontal = %d\n"           ,sps->vui_seq_parameters.log2_max_mv_length_horizontal              );// 7
	    DEBUG_H264("\tlog2_max_mv_length_vertical = %d\n"             ,sps->vui_seq_parameters.log2_max_mv_length_vertical                );// 9
	    DEBUG_H264("\tnum_reorder_frames = %d\n"                      ,sps->vui_seq_parameters.num_reorder_frames                         );// 0
	    DEBUG_H264("\tmax_dec_frame_buffering = %d\n"                 ,sps->vui_seq_parameters.max_dec_frame_buffering                    );// 1
	}
    #endif
	#endif
#if 0
    ...
    {
        if(sps->pic_width_in_mbs_minus1 == 79)
            iduPlaybackMode(1280,720,1280);
        else if(sps->pic_width_in_mbs_minus1 == 39)
            iduPlaybackMode(640,480,640);
        else if(sps->pic_width_in_mbs_minus1 == 119)
        {
          #if 0
            if(TvOutMode == SYS_TV_OUT_HD720P60)
                iduPlaybackMode(1024,720,1280);
            else if((TvOutMode == SYS_TV_OUT_NTSC) || (TvOutMode == SYS_TV_OUT_PAL))
                iduPlaybackMode(1920,1080,1920);
          #else
            iduPlaybackMode(1024,RF_RX_2DISP_HEIGHT*2,RF_RX_2DISP_WIDTH*2);
          #endif
        }
    }
#endif
