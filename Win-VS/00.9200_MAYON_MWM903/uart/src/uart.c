...
/*------ 設定 UART 1:Baud rate-----*/
#if(CHIP_OPTION >= CHIP_A1022A)
	#define UART1_BAUD_RATE      115200
#elif( (SW_APPLICATION_OPTION == MR9300_RFDVR_RX1RX2) || (SW_APPLICATION_OPTION == MR9300_NETBOX_RX1RX2) || (SW_APPLICATION_OPTION == MR9211_DUALMODE_TX5) || (SW_APPLICATION_OPTION == MR9100_WIFI_DONGLE_AVSED_RX1) )
    #define UART1_BAUD_RATE      115200
...
#elif(UART_PTZ485_COMMAND_RX || UART_PTZ485_COMMAND_TX || UART_GPS_COMMAND)
    #define UART2_BAUD_RATE      9600
#else
...
#define UART3_BAUD_RATE      9600
  ...
  u32 sys_frequency=128000000;
#elif REDUCE_HCLK_TO_144M
  u32 sys_frequency=144000000;
#else
...
{
    UART_USE_ENABLE,
#if (UART_PTZ485_COMMAND_RX || UART_PTZ485_COMMAND_TX || HOME_RF_SUPPORT || UART_GPS_COMMAND)
    UART_USE_ENABLE,
#else
    UART_USE_DISABLE,
#endif
...
#if MARSS_SUPPORT
extern BYTE myHostCmdQueue[16][48];// __attribute__ ((aligned (4)));
extern BYTE myHostCmdIdx;
extern OS_EVENT *OSQHost_MarssEvent;
...
extern void USB_test_packet(void);
#if(NIC_SUPPORT == 1)
...
void uartHA_ACK(void)
{
    ...
    u8 prd_cmdStr[5] = {0x69, 0x96, 0x01, 0x42, 0x43};
        ...
        if((uartRcvBuf[HOMERF_UART_ID][uartRcvTail[HOMERF_UART_ID]-11])%UART_MAX_RCV_BUF_SIZ == 0x69)    
        if((uartRcvBuf[HOMERF_UART_ID][uartRcvTail[HOMERF_UART_ID]-10])%UART_MAX_RCV_BUF_SIZ == 0x96)
        if((uartRcvBuf[HOMERF_UART_ID][uartRcvTail[HOMERF_UART_ID]-  9])%UART_MAX_RCV_BUF_SIZ == 0x09)
        if((uartRcvBuf[HOMERF_UART_ID][uartRcvTail[HOMERF_UART_ID]-  8])%UART_MAX_RCV_BUF_SIZ == 0x11)        //HOMERF_RX_CMDID_TRIG_EVENET
#if MARSS_SUPPORT
		{
			myHostCmdIdx++;
			if( myHostCmdIdx>=16)
				myHostCmdIdx=0;
			memcpy(&myHostCmdQueue[myHostCmdIdx][0], tri_cmdStr,5);
...
#endif
        else if((uartRcvBuf[HOMERF_UART_ID][uartRcvTail[HOMERF_UART_ID]-  8])%UART_MAX_RCV_BUF_SIZ == 0x12)//HOMERF_RX_CMDID_PERIOD_EVENT
#if MARSS_SUPPORT
		{
			myHostCmdIdx++;
			if( myHostCmdIdx>=16)
				myHostCmdIdx=0;
			memcpy(&myHostCmdQueue[myHostCmdIdx][0], prd_cmdStr,5);
			OSQPost(OSQHost_MarssEvent, &myHostCmdQueue[myHostCmdIdx][0] );
			OSFlagPost (gMarssFlagGrp, 0x800,OS_FLAG_SET,0);
		}
#else
...
#else // 改成比對連續五筆資料有效,時間差且時間差小於5
        if (g_updatetimeFlag == 0 )
            return 1;
        if ((g_LocalTimeInSec - n_LocalTimeInSec) <= 5)
        {
            //DEBUG_UART("time : %x:%x \n", g_LocalTimeInSec, n_LocalTimeInSec);
            validcnt++;
            if (validcnt >=5)
            {
                DEBUG_UART("[GPS] %d:%d:%d  validcnt=%d Flag %d\n", gGPS_data.Hour, gGPS_data.Min, gGPS_data.Sec,validcnt,g_updatetimeFlag);
                DEBUG_UART("time : %d:%d\n", g_LocalTimeInSec, n_LocalTimeInSec);
                uiGPSUpdateRTC();
                RTC_Set_GMT_Time( (RTC_DATE_TIME *)&gGPS_data);
                RTCTime_Gmt_To_Local((RTC_DATE_TIME *)&gGPS_data, &g_LocalTime);
...
void UARTDoGPSComand(char *UartGPSCmdString)
{
    ...
    if (UARTDoGPSComand_Checksum(UartGPSCmdString)==0)
    {
        ...
        {
            uart_state[GPS_UART_ID]=0;
            uartRcvTail[GPS_UART_ID]=uartRcvHead[GPS_UART_ID]=0;
            gGPSPKErrorCount=0;
            gGPS_data.State = 0;
            uart_state[GPS_UART_ID]=1;
        }
        return;
    }
    else
...
void UartReadHM8563Reg(char *UartCmdString)
{
    ...
    if(val)
    {
        i2cRead_HM8563_1byte(u8addr, &u8value);
        printf("read HM8563 Adress: 0x%04X = 0x%04x\n", u8addr, u8value);
    }
}

void UartWriteHM8563Reg(char *UartCmdString)
{

    int val;
    u8 u8addr, u8val;

    val = sscanf(UartCmdString, "%x %x", &u8addr, &u8val);
    if(val==2)
    {
        DEBUG_UART("Write HM8563 Address 0x%x = 0x%x\n", u8addr, u8val);
        i2cWrite_HM8563_1byte(u8addr, u8val);
    }
...
void UartCmdParse(char *UartCmdString)
{
    ...
    switch(*UartCmdString)
    {
		...
		*/
          ...
          if(!strncmp(UartCmdString,"@P2PINFO",8))
                {
                    unsigned int info;
                    Check_P2P_info(&info);
                    printf("p2p server:%d.\n",info);
...
#endif
 case 'U':
            if(!strncmp(UartCmdString,"USBTEST", 5))
            {
       ...
       //--------UART command set---------//
        ...
        //-----Read register/SDRAM-----//
...
#if(EXT_RTC_SEL == RTC_HM8563)
			else if(!strncmp(UartCmdString,"RTG ",3))
			{
				BOOLEAN bRet=TRUE;
            ...
            else if(!strncmp(UartCmdString,"RTI ",3))
				HM8563_RTC_Init();
			else if(!strncmp(UartCmdString,"RTR ",4))
                UartReadHM8563Reg(UartCmdString+4);
#endif
        #if(TV_DECODER ==TI5150)
     ...
     //Ex: w 80000000 12345678
        ...
        #if(EXT_RTC_SEL == RTC_HM8563)
            else if(!strncmp(UartCmdString,"WTS",3))
            {
				BOOLEAN bRet=TRUE;
				RTC_DATE_TIME RtcTime;
				...
				RtcTime.year =17;
				//17/12/12 23:55:58
				bRet=Set_HM8563_RTC(&RtcTime);
			}
            else if(!strncmp(UartCmdString,"WTW ",4))
                UartWriteHM8563Reg(UartCmdString+4);
        #endif
        #if(TV_DECODER == TI5150)
...
INT32U marsGPSGetString(char *pcString, INT32U *pcBufferLen)
{
    INT32U uiBuferLen;
    ...
    *pcBufferLen = 0;
    while (*pcBufferLen < uiBuferLen)
    {
        ...
        *pcString = receive_char(GPS_UART_ID);
...
//      GPS_Init();
    ...
    //        #if (UART_GPS_COMMAND==0)  //GPS command 可漏掉. 161103 amon:GPS modify to uart2
                else
                {
...
#elif UART_GPS_COMMAND
    else if(*UartId == GPS_UART_ID)
    {
        uart_state[GPS_UART_ID]=1;
        while(1)
        {
            uiUartCmdBufSize = UART_CMDBUF_SIZE-1;
            marsGPSGetString(Uart2Command, &uiUartCmdBufSize);
            UartCmdParse(Uart2Command);
