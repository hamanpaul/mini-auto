...
#include "IT66121_hdmitx_input.h"
...
// 720x480@59.94Hz
// VIC = 2
_CODE RegSetEntry HDMITX_720x480p60TestPattern[] = {
    {0x0F, 0x01, 0x00},
    {0x90, 0xFF, 0x90},
    {0x91, 0xFF, 0x35},
    {0x92, 0xFF, 0x7A},
...
#endif
    hdmitx_LoadRegSetting((void *)HDMITX_DefaultVideo_Table);
    hdmitx_LoadRegSetting((void *)HDMITX_SetHDMI_Table);
    hdmitx_LoadRegSetting((void *)HDMITX_DefaultAVIInfo_Table);
    hdmitx_LoadRegSetting((void *)HDMITX_DeaultAudioInfo_Table);
    hdmitx_LoadRegSetting((void *)HDMITX_Aud_CHStatus_LPCM_20bit_48Khz);
    hdmitx_LoadRegSetting((void *)HDMITX_AUD_SPDIF_2ch_24bit);
...
BYTE CheckHDMITX(BYTE *pHPD,BYTE *pHPDChange)
{
    ...
    if(sysstat & B_TX_INT_ACTIVE)
    {
		...
		{
		    DEBUG_HDMI("DDC FIFO Error.\n");
		    hdmitx_ClearDDCFIFO();
		    hdmiTxDev[0].bAuthenticated= FALSE ;
		}
		if(intdata1 & B_TX_INT_DDC_BUS_HANG)
		{
		    DEBUG_HDMI("DDC BUS HANG.\n");
            hdmitx_AbortDDC();

            if(hdmiTxDev[0].bAuthenticated)
            {
                DEBUG_HDMI("when DDC hang,and aborted DDC,the HDCP authentication need to restart.\n");
...
void HDMITX_PowerOn()
{
    hdmitx_LoadRegSetting((void *)HDMITX_PwrOn_Table);
}

void HDMITX_PowerDown()
{
    hdmitx_LoadRegSetting((void *)HDMITX_PwrDown_Table);
}
...
void hdmitx_LoadRegSetting(RegSetEntry table[])
{
    ...
    for( i = 0 ;  ; i++ )
    {
        ...
        {
            HDMITX_DEBUG_PRINTF2(("delay(%d)\n",(int)table[i].offset));//HDMITX_DEBUG_PRINTF2
...
//        HDMITX_DEBUG_PRINTF(("getHDMITX_EDIDBytes(): Invallid pData pointer %08lX\n",(ULONG)pData));
    ...
    while(RemainedCount > 0)
    {
        ...
        HDMITX_DEBUG_PRINTF2(("getHDMITX_EDIDBytes(): ReqCount = %d,bCurrOffset = %d\n",(int)ReqCount,(int)bCurrOffset));
        ...
        for(TimeOut = 250 ; TimeOut > 0 ; TimeOut --)
        {
            ...
            {
                DEBUG_HDMI("getHDMITX_EDIDBytes(): DDC_STATUS = %02X,fail.\n",(int)ucdata);
        ...
        if(TimeOut == 0)
        {
            DEBUG_HDMI("getHDMITX_EDIDBytes(): DDC TimeOut. \n",(int)ucdata);
            // HDMITX_AndReg_Byte(REG_TX_INT_CTRL,~(1<<1));
            return ER_FAIL ;
        }
        do
...
//    CPDesire = HDMITX_ReadI2C_Byte(REG_TX_HDCP_DESIRE); //Amy
...
BOOL HDMITX_EnableVideoOutput(VIDEOPCLKLEVEL level,BYTE inputColorMode,BYTE outputColorMode,BYTE bHDMI)
{
    ...
    if(VIC == 3) //480p, HDMI_480p60_16x9
    {
        hdmitx_LoadRegSetting((void *)HDMITX_720x480p60TestPattern);
        HDMITX_WriteI2C_Byte(0xA8, 0x81);
        HDMITX_WriteI2C_Byte(0xA9, 0x71);
        DBG_printf("IT66121 720x480 test pattern\n");
    }
    hdmitx_FireAFE();
...
BOOL setHDMITX_SyncEmbeddedByVIC(BYTE VIC,BYTE bInputType)
{
    ...
    HDMITX_DEBUG_PRINTF1(("setHDMITX_SyncEmbeddedByVIC(%d,%x)\n",(int)VIC,(int)bInputType));
    if( VIC > 0 )
    {
        for(i=0;i< MaxIndex;i ++)
        {
            if(TimingTable[i].fmt==VIC)
            {
                ...
                HDMITX_DEBUG_PRINTF1(("***Fine Match Table ***\n"));
                break;
            }
        }
    }
...
void hdmitx_SetInputMode(BYTE InputColorMode,BYTE bInputSignalType)
{
    ...
    {
        ucData |= B_TX_PCLKDIV2 ; HDMITX_DEBUG_PRINTF1(("PCLK Divided by 2 mode\n"));
    }
    if(bInputSignalType & T_MODE_CCIR656)
    {
        ucData |= B_TX_2X656CLK ; HDMITX_DEBUG_PRINTF1(("CCIR656 mode\n"));
    }
    if(bInputSignalType & T_MODE_SYNCEMB)
    {
        ucData |= B_TX_SYNCEMB ; HDMITX_DEBUG_PRINTF1(("Sync Embedded mode\n"));
    }
    if(bInputSignalType & T_MODE_INDDR)
    {
        ucData |= B_TX_INDDR ; HDMITX_DEBUG_PRINTF1(("Input DDR mode\n"));
    }
...
void hdmitx_SetCSCScale(BYTE bInputMode,BYTE bOutputMode)
{
    ...
    HDMITX_DEBUG_PRINTF1(("hdmitx_SetCSCScale(BYTE bInputMode = %x,BYTE bOutputMode = %x)\n", (int)bInputMode, (int)bOutputMode)) ;
    switch(bInputMode&F_MODE_CLRMOD_MASK)
    {
    #ifdef SUPPORT_INPUTYUV444
    case F_MODE_YUV444:
        HDMITX_DEBUG_PRINTF1(("Input mode is YUV444 "));
        switch(bOutputMode&F_MODE_CLRMOD_MASK)
        {
        case F_MODE_YUV444:
            HDMITX_DEBUG_PRINTF1(("Output mode is YUV444\n"));
            csc = B_HDMITX_CSC_BYPASS ;
            break ;

        case F_MODE_YUV422:
            HDMITX_DEBUG_PRINTF1(("Output mode is YUV422\n"));
            ...
            break ;
        case F_MODE_RGB444:
            HDMITX_DEBUG_PRINTF1(("Output mode is RGB24\n"));
    ...
    case F_MODE_YUV422:
        HDMITX_DEBUG_PRINTF1(("Input mode is YUV422\n"));
        switch(bOutputMode&F_MODE_CLRMOD_MASK)
        {
        case F_MODE_YUV444:
            HDMITX_DEBUG_PRINTF1(("Output mode is YUV444\n"));
        ...
        case F_MODE_RGB444:
            HDMITX_DEBUG_PRINTF1(("Output mode is RGB444\n"));
            csc = B_HDMITX_CSC_YUV2RGB ;
    ...
    case F_MODE_RGB444:
        HDMITX_DEBUG_PRINTF1(("Input mode is RGB444\n"));
        switch(bOutputMode&F_MODE_CLRMOD_MASK)
        {
        case F_MODE_YUV444:
    ...
    {
        HDMITX_DEBUG_PRINTF1(("CSC = RGB2YUV %x ",csc));
        switch(bInputMode&(F_VIDMODE_ITU709|F_VIDMODE_16_235))
        {
        case F_VIDMODE_ITU709|F_VIDMODE_16_235:
            HDMITX_DEBUG_PRINTF1(("ITU709 16-235 "));
            for( i = 0 ; i < SIZEOF_CSCMTX ; i++ ){ HDMITX_WriteI2C_Byte(REG_TX_CSC_YOFF+i,bCSCMtx_RGB2YUV_ITU709_16_235[i]) ; HDMITX_DEBUG_PRINTF1(("reg%02X <- %02X\n",(int)(i+REG_TX_CSC_YOFF),(int)bCSCMtx_RGB2YUV_ITU709_16_235[i]));}
            break ;
        case F_VIDMODE_ITU709|F_VIDMODE_0_255:
            HDMITX_DEBUG_PRINTF1(("ITU709 0-255 "));
            for( i = 0 ; i < SIZEOF_CSCMTX ; i++ ){ HDMITX_WriteI2C_Byte(REG_TX_CSC_YOFF+i,bCSCMtx_RGB2YUV_ITU709_0_255[i]) ; HDMITX_DEBUG_PRINTF1(("reg%02X <- %02X\n",(int)(i+REG_TX_CSC_YOFF),(int)bCSCMtx_RGB2YUV_ITU709_0_255[i]));}
            break ;
        case F_VIDMODE_ITU601|F_VIDMODE_16_235:
            HDMITX_DEBUG_PRINTF1(("ITU601 16-235 "));
            for( i = 0 ; i < SIZEOF_CSCMTX ; i++ ){ HDMITX_WriteI2C_Byte(REG_TX_CSC_YOFF+i,bCSCMtx_RGB2YUV_ITU601_16_235[i]) ; HDMITX_DEBUG_PRINTF1(("reg%02X <- %02X\n",(int)(i+REG_TX_CSC_YOFF),(int)bCSCMtx_RGB2YUV_ITU601_16_235[i]));}
            break ;
        case F_VIDMODE_ITU601|F_VIDMODE_0_255:
        default:
            HDMITX_DEBUG_PRINTF1(("ITU601 0-255 "));
            for( i = 0 ; i < SIZEOF_CSCMTX ; i++ ){ HDMITX_WriteI2C_Byte(REG_TX_CSC_YOFF+i,bCSCMtx_RGB2YUV_ITU601_0_255[i]) ; HDMITX_DEBUG_PRINTF1(("reg%02X <- %02X\n",(int)(i+REG_TX_CSC_YOFF),(int)bCSCMtx_RGB2YUV_ITU601_0_255[i]));}
            break ;
        }
    }
    #endif
    ...
    {
        HDMITX_DEBUG_PRINTF1(("CSC = YUV2RGB %x ",csc));

        switch(bInputMode&(F_VIDMODE_ITU709|F_VIDMODE_16_235))
        {
        case F_VIDMODE_ITU709|F_VIDMODE_16_235:
            HDMITX_DEBUG_PRINTF1(("ITU709 16-235 "));
            for( i = 0 ; i < SIZEOF_CSCMTX ; i++ ){ HDMITX_WriteI2C_Byte(REG_TX_CSC_YOFF+i,bCSCMtx_YUV2RGB_ITU709_16_235[i]) ; HDMITX_DEBUG_PRINTF1(("reg%02X <- %02X\n",(int)(i+REG_TX_CSC_YOFF),(int)bCSCMtx_YUV2RGB_ITU709_16_235[i]));}
            break ;
        case F_VIDMODE_ITU709|F_VIDMODE_0_255:
            HDMITX_DEBUG_PRINTF1(("ITU709 0-255 "));
            for( i = 0 ; i < SIZEOF_CSCMTX ; i++ ){ HDMITX_WriteI2C_Byte(REG_TX_CSC_YOFF+i,bCSCMtx_YUV2RGB_ITU709_0_255[i]) ; HDMITX_DEBUG_PRINTF1(("reg%02X <- %02X\n",(int)(i+REG_TX_CSC_YOFF),(int)bCSCMtx_YUV2RGB_ITU709_0_255[i]));}
            break ;
        case F_VIDMODE_ITU601|F_VIDMODE_16_235:
            HDMITX_DEBUG_PRINTF1(("ITU601 16-235 "));
            for( i = 0 ; i < SIZEOF_CSCMTX ; i++ ){ HDMITX_WriteI2C_Byte(REG_TX_CSC_YOFF+i,bCSCMtx_YUV2RGB_ITU601_16_235[i]) ; HDMITX_DEBUG_PRINTF1(("reg%02X <- %02X\n",(int)(i+REG_TX_CSC_YOFF),(int)bCSCMtx_YUV2RGB_ITU601_16_235[i]));}
            break ;
        case F_VIDMODE_ITU601|F_VIDMODE_0_255:
        default:
            HDMITX_DEBUG_PRINTF1(("ITU601 0-255 "));
            for( i = 0 ; i < SIZEOF_CSCMTX ; i++ ){ HDMITX_WriteI2C_Byte(REG_TX_CSC_YOFF+i,bCSCMtx_YUV2RGB_ITU601_0_255[i]) ; HDMITX_DEBUG_PRINTF1(("reg%02X <- %02X\n",(int)(i+REG_TX_CSC_YOFF),(int)bCSCMtx_YUV2RGB_ITU601_0_255[i]));}
            break ;
        }
    }
    #endif
...
//    AudioFormat|=0x01;//mingchih add 32bit I2S
//    AudioFormat|=0x00;//mingchih add 16bit I2S
...
void HDMITX_EnableAudioOutput(BYTE AudioType, BOOL bSPDIF,  ULONG SampleFreq,  BYTE ChNum, BYTE *pIEC60958ChStat, ULONG TMDSClock)
{
    ...
    {
    case T_AUDIO_HBR:
        HDMITX_DEBUG_PRINTF1(("T_AUDIO_HBR\n"));
        ...
        break ;
    case T_AUDIO_DSD:
        HDMITX_DEBUG_PRINTF1(("T_AUDIO_DSD\n"));
        setHDMITX_DSDAudio();
        break ;
    case T_AUDIO_NLPCM:
        HDMITX_DEBUG_PRINTF1(("T_AUDIO_NLPCM\n"));
        pIEC60958ChStat[0] |= 1<<1 ;
        setHDMITX_ChStat(pIEC60958ChStat);
        setHDMITX_NLPCMAudio(bSPDIF);
        break ;
    case T_AUDIO_LPCM:
        HDMITX_DEBUG_PRINTF1(("T_AUDIO_LPCM\n"));
        pIEC60958ChStat[0] &= ~(1<<1);
...
void hdmitx_AutoAdjustAudio()
{
    ...
    HDMITX_DEBUG_PRINTF1(("PCLK = %u0,000\n",(WORD)(cTMDSClock/10000)));
    switch(uc & 0x70)
    {
    case 0x50:
        cTMDSClock *= 5 ;
    ...
    HDMITX_DEBUG_PRINTF1(("SampleFreq = %u0\n",(WORD)(SampleFreq/10)));
    ...
    {
        fs = AUDFS_OTHER;
        HDMITX_DEBUG_PRINTF1(("fs = AUDFS_OTHER\n"));
    }
