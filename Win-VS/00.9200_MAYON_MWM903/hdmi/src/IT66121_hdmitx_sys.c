...
#define HDMI_TIMEOUT 40
OS_EVENT* HDMISemReq;
OS_EVENT* HDMI_ChangeReso_SemReq;

unsigned char u8CurVdoSize = 4; //project.h default 720p
unsigned char is_Cap_VDO_1280x720P = 1;
#if (UI_BOOT_FROM_PANEL == 0) //always TV out, boot from TV
unsigned char is_Hot_Plug_forUI = 1;
    #if (TVOUT_DYNAMIC)
    unsigned char is_Cap_VDO_1920x1080I = 1; // TV out, value's updated by hotplug, 96R Plus
    #else
    unsigned char is_Cap_VDO_1920x1080I = 0; // TV out and fixed resolution 720p, like 9300
    #endif
#else
unsigned char is_Hot_Plug_forUI = 0;
unsigned char is_Cap_VDO_1920x1080I = 0; // boot from panel, value's updated by hotplug, basic 95R
#endif
...
void InitHDMITX_Variable()
{
    HDMITX_InitTxDev((void *)&InstanceData);
	HPDStatus = FALSE;
...
void HDMITX_DevLoopProc()
{
    ...
    if( HPDChangeStatus )
    {
        ...
        if( HPDStatus )
        {
            DBG_printf("IT66121 detect plug plug\n");
            HDMITX_PowerOn();
            ParseEDID();
            ...
            if (err != OS_NO_ERR)
            {
                DEBUG_HDMI("Error: hot_plug HDMISemReq is %d.\n", err);
            }

            #if (UI_BOOT_FROM_PANEL == 0) // always TV out
            // maybe add KEY for adjust audio setting
            sysPlaybackSetTVSupport1080I(1);
            #else
            is_Hot_Plug_forUI = 1;
			...
			{
                DBG_printf("IT66121 RxCapability not Valid HDMI\n");
				bHDMIMode = FALSE ;
				bAudioEnable = FALSE ;
				OutputColorDepth = B_TX_CD_NODEF;
            #if (TVOUT_DYNAMIC) 
        ...
        else
        {
            ...
            sysPlaybackSetTVSupport1080I(0);
            #else
            u8CurVdoSize = 0;
            is_Hot_Plug_forUI = 0;
            is_Cap_VDO_1920x1080I = 0;
            ...
            DBG_printf("IT66121 unplug unplug \n");
            // unplug mode, ...
...
BYTE ParseEDID()
{
    ...
    for( i = 0 ; i < 128 ; i++ )
    {
        ...
        if(i%8 == 0) DBG_printf(" ");
        DBG_printf("0x%02X, ", (int)EDID_Buf[i] );
        //HDMITX_DEBUG_PRINTF(("0x%02X%c, ",(int)EDID_Buf[i],(7 == (i&7))?'\n':' '));
    }
    #endif
    ...
    for( i = 1 ; i <= BlockCount ; i++ )
    {
        ...
        for( j = 0 ; j < 128 ; j++ )
        {
            if(j%16 == 0) DBG_printf("\r\n");
            if(j%8 == 0) DBG_printf(" ");
            DBG_printf("0x%02X, ", (int)EDID_Buf[j] );
            //EDID_DEBUG_PRINTF(("0x%02X%c, ",(int)EDID_Buf[j],(7 == (j&7))?'\n':' '));
        }
        #endif
...
static BOOL ParseCEAEDID(BYTE *pCEAEDID)
{
    ...
    BYTE tag ;
    int i ;
    u8 temp1080I = 0; //avoid freq get is_Cap_VDO_1920x1080I to get temp 0, then display abnormally
    ...
    for( offset = 4 ; offset < End ; )
    {
        ...
        case 0x01: // Audio Data Block ;
            ...
            EDID_DEBUG_PRINTF1(("RxCapability.AUDDesCount = %d\n",(int)RxCapability.AUDDesCount));
            offset++ ;
        ...
        case 0x02: // Video Data Block ;
            ...
            for( i = 0,RxCapability.NativeVDOMode = 0xff ; i < count ; i++)
            {
            	...
            	if(VIC<64)
            	{
                    ...
                    EDID_DEBUG_PRINTF((", %d ", (int)VIC));
                    EDID_DEBUG_PRINTF1(("VIC = %d, RxCapability.VDOMode[%d]=%02X\n",(int)VIC,(int)VIC/8,(int)RxCapability.VDOMode[VIC/8] ));
                    if(VIC == HDMI_1080i60)
                        temp1080I = 1;
            ...
            is_Cap_VDO_1920x1080I = temp1080I;
            offset += count ;
            break ;
...
void IT66121_Task()
{
    ...
    OSSemPend(HDMI_ChangeReso_SemReq, HDMI_TIMEOUT, &err);
    if (err != OS_NO_ERR)
    {
        DEBUG_HDMI("Error: HDMI_ChangeReso_SemReq is %d.\n", err);
    }
    HDMITX_DevLoopProc();
    OSSemPost(HDMI_ChangeReso_SemReq);
}

void IT66121_init(HDMI_Video_Type OutputVideoTiming)
{
    unsigned char err = 0;
    //init_timer();
    //printf( "\n" VERSION_STRING "\n");
...
//////////////////////////////////////////////////
    ...
    HDMI_ChangeReso_SemReq = OSSemCreate(1);
    ...
    //EMI problem
    #if ((HW_BOARD_OPTION == MR9200_RX_TRANWO_D8710R) || (HW_BOARD_OPTION == MR9200_RX_TRANWO_SH8710R))
    HDMITX_SetI2C_Byte(0x0f,1, 0);
    HDMITX_WriteI2C_Byte(0x63, 38);
    #else
    //KITE W 63 18       [011 ¹w³]ªº­È]
//    HDMITX_SetI2C_Byte(0x0f,1, 0);
//    HDMITX_WriteI2C_Byte(0x63, 0);
    #endif
/////////////////////////////////////////////////////////////////////////////////////////////////////////////
    OSSemPend(HDMI_ChangeReso_SemReq, HDMI_TIMEOUT, &err);
    if (err != OS_NO_ERR)
    ...
    HDMITX_ChangeDisplayOption(OutputVideoTiming, HDMI_RGB444);
    u8CurVdoSize = OutputVideoTiming;
    OSSemPost(HDMI_ChangeReso_SemReq);   
}
...
{
    DEBUG_HDMI("is_Hot_Plug %d\n", is_Hot_Plug_forUI);
    return is_Hot_Plug_forUI;
}

void HDMITX_AudioEnable(unsigned char audioEnable)
{   
    unsigned char err = 0;
    if(is_Hot_Plug_forUI)
    {
        OSSemPend(HDMI_ChangeReso_SemReq, HDMI_TIMEOUT, &err);
        if (err != OS_NO_ERR)
        {
...
{   
    unsigned char err = 0;
    DBG_printf("u8CurVdoSize %d swtich to VdoSize %d\n", u8CurVdoSize, VdoSize);
    if(u8CurVdoSize != VdoSize)
    {
        ...
        HDMITX_DisableVideoOutput();
    	HDMITX_ChangeDisplayOption(VdoSize,HDMI_RGB444);
        HDMITX_SetOutput();
        OSSemPost(HDMI_ChangeReso_SemReq);
        u8CurVdoSize = VdoSize;
    }
