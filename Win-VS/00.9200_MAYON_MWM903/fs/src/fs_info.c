...
extern const FS__device_type    FS__ramdevice_driver1;
#endif /* FS_USE_SMC_DRIVER */

#if FS_USE_USBFS_DRIVER
extern const FS__device_type    FS__usbfsdevice_driver;
#endif /* FS_USE_SMC_DRIVER */
...
/*------------------------ USBFS cache settings CVBNM---------------------*///raymond add
#ifndef FS_USBFS_CACHENUM
#define FS_USBFS_CACHENUM 0
#endif


#if ((FS_USBFS_CACHENUM) && (FS_USE_USBFS_DRIVER))
static FS__CACHE_BUFFER  _FS__USBFS_Buffers[FS_USBFS_MAXUNIT][FS_USBFS_CACHENUM];
static FS__LB_CACHE      _FS__USBFS_Cache[FS_USBFS_MAXUNIT] =
#if (FS_USBFS_MAXUNIT==1)
{
    {FS_USBFS_CACHENUM, 0, &_FS__USBFS_Buffers[0][0]}
};
#elif (FS_USBFS_MAXUNIT==2)
{
    {FS_USBFS_CACHENUM, 0, &_FS__USBFS_Buffers[0][0]},
    {FS_USBFS_CACHENUM, 0, &_FS__USBFS_Buffers[1][0]}
};
#else
#error Please define _FS__USBFS_Cache for desired number of units
#endif
#define FS_CACHEINFO_USBFS_DRIVER     &_FS__USBFS_Cache[0],
#else
#define FS_CACHEINFO_USBFS_DRIVER     0,
#endif
...
#if FS_USE_MMC_DRIVER
#define FS_DEVINFO_DEVMMC      { "sdmmc",    &FS__fat_functable, &FS__mmcdevice_driver, FS_CACHEINFO_MMC_DRIVER 0 },
#define FS_DEVINFO_DEVRAM1     { "ram1",    &FS__fat_functable, &FS__ramdevice_driver1, FS_CACHEINFO_RAMDISK_DRIVER 0 },
#else
#define FS_DEVINFO_DEVMMC
#define FS_DEVINFO_DEVRAM1
#endif


#if FS_USE_USBFS_DRIVER
#define FS_DEVINFO_DEVUSB      { "usbfs",    &FS__fat_functable, &FS__usbfsdevice_driver, FS_CACHEINFO_USBFS_DRIVER 0 },
#else
#define FS_DEVINFO_DEVUSB
#endif
...
#endif
/* cytsai: mass storage */
#define FS_DEVINFO FS_DEVINFO_DEVSMC FS_DEVINFO_DEVMMC FS_DEVINFO_DEVUSB FS_DEVINFO_DEVIDE FS_DEVINFO_DEVFLASH FS_DEVINFO_DEVWINDRV FS_DEVINFO_DEVRAM FS_DEVINFO_DEVRAM1

#endif  /* FS_DEVINFO */

FS__devinfo_type _FS__devinfo[] =
{
    FS_DEVINFO
};
//const FS__devinfo_type _FS__devinfo[] = { FS_DEVINFO_DEVMMC };	/* cytsai */

FS__devinfo_type *FS__pDevInfo = _FS__devinfo;
//const FS__devinfo_type *const FS__pDevInfo = _FS__devinfo;		/* cytsai */
...
int DCF_GetDeviceIndex(char *DevName)
{
    unsigned int i;
    ...
    while (1)
    {
        if (i >= FS__maxdev)
        {
            printf("---->Warnning!! DCF_GetDeviceIndex error!\n");
            break;  /* Driver not found */
        }
        if ( 0==strcmp(FS__pDevInfo[i].devname,DevName) )
        {
            break;  /* Driver found */
        }
        i++;
