...
static  OS_EVENT  *FS_SemDeviceOps[FS_DEV_ONLINE_MAX];
...
void  FS_X_OS_LockDeviceOp(const FS__device_type *driver, u32 id)
{
    INT8U  err;
    OSSemPend(FS_SemDeviceOps[id], 0, &err);
}
...
void  FS_X_OS_UnlockDeviceOp(const FS__device_type *driver, u32 id)
{
    OSSemPost(FS_SemDeviceOps[id]);
}
...
u16  FS_X_OS_GetDate (void)
{
    ...
    ltime  = localtime(&t);
    fdate  = ltime->tm_mday;
    fdate += ((u16)(ltime->tm_mon  +  1) << 5);
    fdate += ((u16)(ltime->tm_year - 80) << 9);
#else
    u16      fdate;
...
u16  FS_X_OS_GetTime (void)
{
    ...
    ftime += ((u16) ltime->tm_min  <<  5);
    ftime += ((u16) ltime->tm_hour << 11);
#else
    u16 ftime;
...
int  FS_X_OS_Init(void)
{
    ...
    for(i = 0; i < FS_DEV_ONLINE_MAX; i++)
        FS_SemDeviceOps[i] = OSSemCreate(1);
#if FS_POSIX_DIR_SUPPORT
...
int  FS_X_OS_Exit (void)
{
    ...
    for(i = 0; i < FS_DEV_ONLINE_MAX; i++)
        FS_SemDeviceOps[i] = OSSemDel(FS_SemDeviceOps[i] , OS_DEL_ALWAYS, &err);
#if FS_POSIX_DIR_SUPPORT
