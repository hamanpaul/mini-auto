...
static void _write_file(const char *name, const char *txt) 
{
  	...
  	if (myfile) {
    		/* write to file */
    		result = FS_FWrite(myfile, txt, strlen(txt), &x);
    		/* all data written ? */
    		if (result < 0 || x!=(int)strlen(txt)) 
    		{
...
static void _dump_file(const char *name) 
{
  	...
  	if (myfile) 
  	{
    		...
    		do 
    		{
      			result = FS_FRead(myfile, mybuffer, sizeof(mybuffer)-1, &x);
      			mybuffer[x]=0;
      			if (x && result >= 0) 
      			{
        			_log(mybuffer);
