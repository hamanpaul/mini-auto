...
#include "general.h"
#if (Sensor_OPTION == Sensor_MT9M131_YUV601)
        #include "board.h"
   ...
   gRfiuUnitCntl[0].RFpara.TX_SensorType=TX_SENSORTYPE_VGA;
    ...
    {	
       ...
       case SIUMODE_MPEGAVI: 
...
#if 1
            i2cWrite_SENSOR(0xf0, 0x0000);	// Page Map 0
            i2cWrite_SENSOR(0x05, 0x00CA);	//Horizontal Blank (B) = 202
            i2cWrite_SENSOR(0x06, 0x01E0);	//Vertical Blank (B) = 480  
            i2cWrite_SENSOR(0x07, 0x00CA);	//Horizontal Blank (A) = 202
            i2cWrite_SENSOR(0x08, 0x023A);	//Vertical Blank (A) = 570  
            i2cWrite_SENSOR(0x20, 0x0300);	//Read Mode (B) = 768       
            i2cWrite_SENSOR(0x21, 0x0000);	//Read Mode (A) = 0         

            i2cWrite_SENSOR(0xf0, 0x0002);	// Page Map 2
            i2cWrite_SENSOR(0x39, 0x0352);	//Auto Exp Line Size (A) = 850    
            i2cWrite_SENSOR(0x3A, 0x05D2);	//Auto Exp Line Size (B) = 1490   
            i2cWrite_SENSOR(0x3B, 0x0352);	//Auto Exp Delay (A) = 850        
            i2cWrite_SENSOR(0x3C, 0x05D2);	//Auto Exp Delay (B) = 1490       
            i2cWrite_SENSOR(0x57, 0x0422);	//Flicker Frame Time 60 (A) = 1058
            i2cWrite_SENSOR(0x58, 0x04F6);	//Flicker Frame Time 50 (A) = 1270
            i2cWrite_SENSOR(0x59, 0x025C);	//Flicker Frame Time 60 (B) = 604 
            i2cWrite_SENSOR(0x5A, 0x02D4);	//Flicker Frame Time 50 (B) = 724 
            i2cWrite_SENSOR(0x5C, 0x241E);	//Flicker Srh Range 60 = 9246     
            i2cWrite_SENSOR(0x5D, 0x2B25);	//Flicker Srh Range 50 = 11045    
            i2cWrite_SENSOR(0x64, 0x5E1C);	//Flicker Parameter = 24092       

            i2cWrite_SENSOR(0xf0, 0x0001);	// Page Map 1
            i2cWrite_SENSOR(0xA3, 0x02D0);	// VERT_ZOOM_RESIZE_B
            i2cWrite_SENSOR(0xA4, 0x02D0);	// VERT_SIZE_RESIZE_B
            i2cWrite_SENSOR(0x3A, 0x0000);	// Output format control 2—context A (R/W)
            i2cWrite_SENSOR(0x9B, 0x0000);	// Output format control 2—context B (R/W)

            i2cWrite_SENSOR(0xf0, 0x0000);	// Page Map 0
            i2cWrite_SENSOR(0x03, 0x02D0);	// ROW_WINDOW_SIZE_REG

            i2cWrite_SENSOR(0xf0, 0x0002);	// Page Map 2
            i2cWrite_SENSOR(0xD2, 0x007F);	// DEFAULT_CONFIG
            i2cWrite_SENSOR(0xCC, 0x0004);	// PROGRAM_SELECT
            i2cWrite_SENSOR(0xCB, 0x0001);	// PROGRAM_ADVANCE

            i2cWrite_SENSOR(0xf0, 0x0000);	// Page Map 0
            i2cWrite_SENSOR(0x01, 0x00A4);	// ROW_WINDOW_START_REG

        #if(VIDEO_RESOLUTION_SEL == VIDEO_HD_IN_HD_OUT)
            i2cWrite_SENSOR(0xC8, 0x000B);	//Use context B
            i2cWrite_SENSOR(0x37, 0x0100);	//(1) Auto Exposure Gain Zone Limit to 1/15 sec
            FrameRate=15;
        #else   // VGA
          #if 1 // cut windows to VGA
            i2cWrite_SENSOR(0xC8, 0x0000);	//Use context A
            i2cWrite_SENSOR(0x37, 0x8080);	//(1) Auto Exposure Gain Zone Limit to 1/30 sec
          #else // scalar to VGA
            i2cWrite_SENSOR(0xf0, 0x0000);  // Page Map 0
            i2cWrite_SENSOR(0x0D, 0x0009);  // RESET_REG
            i2cWrite_SENSOR(0x0D, 0x0029);  // RESET_REG
            i2cWrite_SENSOR(0x0D, 0x0008);  // RESET_REG

            i2cWrite_SENSOR(0xf0, 0x0002);  // Page Map 2
            i2cWrite_SENSOR(0xD2, 0x0000);  // DEFAULT_CONFIG
            i2cWrite_SENSOR(0xCC, 0x0004);  // PROGRAM_SELECT
            i2cWrite_SENSOR(0xCB, 0x0001);  // PROGRAM_ADVANCE

            i2cWrite_SENSOR(0xf0, 0x0000);  // Page Map 0
            i2cWrite_SENSOR(0x21, 0x000C);  // READ_MODE_A
...
#else   // parameter come from CT
            i2cWrite_SENSOR(0xf0, 0x0000);	// Page Map 0
            i2cWrite_SENSOR(0x05, 0x00CA);	//Horizontal Blank (B) = 202
            i2cWrite_SENSOR(0x06, 0x0159);	//Vertical Blank (B) = 345
            i2cWrite_SENSOR(0x07, 0x0072);	//Horizontal Blank (A) = 114
            i2cWrite_SENSOR(0x08, 0x0034);	//Vertical Blank (A) = 52
            i2cWrite_SENSOR(0x20, 0x0300);	//Read Mode (B) = 768
            i2cWrite_SENSOR(0x21, 0x040C);	//Read Mode (A) = 1036

            i2cWrite_SENSOR(0xf0, 0x0002);	// Page Map 2
            i2cWrite_SENSOR(0x39, 0x05F4);	//Auto Exp Line Size (A) = 1524
            i2cWrite_SENSOR(0x3A, 0x05D2);	//Auto Exp Line Size (B) = 1490
            i2cWrite_SENSOR(0x3B, 0x05F4);	//Auto Exp Delay (A) = 1524
            i2cWrite_SENSOR(0x3C, 0x05D2);	//Auto Exp Delay (B) = 1490
            i2cWrite_SENSOR(0x57, 0x020C);	//Flicker Frame Time 60 (A) = 524
            i2cWrite_SENSOR(0x58, 0x0275);	//Flicker Frame Time 50 (A) = 629
            i2cWrite_SENSOR(0x59, 0x0218);	//Flicker Frame Time 60 (B) = 536
            i2cWrite_SENSOR(0x5A, 0x0284);	//Flicker Frame Time 50 (B) = 644
            i2cWrite_SENSOR(0x5C, 0x120D);	//Flicker Srh Range 60 = 4621
            i2cWrite_SENSOR(0x5D, 0x1712);	//Flicker Srh Range 50 = 5906
            i2cWrite_SENSOR(0x64, 0x5E1C);	//Flicker Parameter = 24092
            ...
            i2cWrite_SENSOR(0x3A, 0x0000);	// Output format control 2—context A (R/W)
            i2cWrite_SENSOR(0x9B, 0x0000);	// Output format control 2—context B (R/W)
            //i2cWrite_SENSOR(0x08, 0x0094);	// Output format control (R/W)
            ...
            for(i = 0; i < 0x100; i++)
            {
                ...
                DEBUG_SIU("MT9M131(0x0%02x)=0x%04x\n", i, data);
            }
            ...
            {
                i2cRead_SENSOR(i, &data);
                DEBUG_SIU("MT9M131(0x1%02x)=0x%04x\n", i, data);
            }
            ...
            {
                i2cRead_SENSOR(i, &data);
                DEBUG_SIU("MT9M131(0x2%02x)=0x%04x\n", i, data);
            }
#endif
