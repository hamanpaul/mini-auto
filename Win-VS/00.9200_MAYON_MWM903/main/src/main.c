...
FS_DISKFREE_T Backup_diskInfo;


u8 got_disk_info;
u8  gInsertNAND;
u8 PowerOn_HotPlug;  //§PÂ_¬O§_¬° Power-on, HotPlug status.
...
u8 SDRAM_Test(u32* test_base1)
{
    ...
    if( memTestDataBus((volatile datum *)test_base))
    {
        DEBUG_MAIN("memTestDataBus Fail\n");
        return 0;
    }
 
    if( memTestAddressBus((volatile datum *)test_base, 65536) )
    {
...
void main(void)
{
    ...
    WDTctrBase &= ~(WDT_EN);	// Jump here(0x80000000) successfully, close the WDT to prevent reboot.
    ...
    {
        DEBUG_MAIN("########################################\r\n");
        DEBUG_MAIN("##    mainTask Priority Overflow %d   ##\r\n", MAIN_TASK_PRIORITY_END);
        DEBUG_MAIN("########################################\r\n");
        while(1);
    }
...
void main_intInit(void)
{
...
#if (NIC_SUPPORT && !ICOMMWIFI_SUPPORT)
    marsIntIRQDefIsr(INT_IRQID_FTMAC, ftmac110_isr);
  #endif
    ...
    marsIntIRQDefIsr(INT_IRQID_SD2, sdcIntHandler);   // for SD2
#endif
    ...
    marsIntIRQDefIsr(INT_IRQID_USB2, usbHostIntHandler); // usb2
#elif (USB_DEVICE == 1)
    ...
    marsIntFIQDefIsr(INT_FIQID_ISU,  ciuIntHandler_CH5);
  #else
    ...
    #if MULTI_CHANNEL_VIDEO_REC
    marsIntFIQDefIsr(INT_FIQID_H264, MultiChannelH264IntHandler);
    #else
                      ...
                      )
{
   ...
   {
      DEBUG_MAIN("MCPU's Paramater is illegle!\n");
      return;
   }
...
int marsMcpu_Test()
{
    ...
    for(k=0;k<128;k++)
    {
        ...
        DEBUG_MCPU("----->Test Block Copy(%d,%d,%d):",Width,Height,Stride);
        WDT_Reset_Count();
