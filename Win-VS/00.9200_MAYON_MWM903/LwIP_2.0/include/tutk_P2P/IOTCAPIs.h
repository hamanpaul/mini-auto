...
/**
 * \brief Set memory pool of IOTC module
 *
 * \details Set pool pointer and pool size of IOTC module.
 *
 * \param Pool [in] Pool pointer.
 * \param nSize [in] Pool size (bytes).
 * 
 * \return #IOTC_ER_NoERROR if set pool success
 * \return Error code if return value < 0
 *			- #IOTC_ER_INVALID_ARG Pool pointer is NULL or pool size too small
 *
 * \attention	This function shall be called before IOTC_Initialize) or IOTC_Initialize2()
 */
P2PAPI_API int IOTC_Mem_Init(const char *Pool,unsigned int nSize);
...
/**
 * \brief Set pbuf threshold count.
 *
 * \details IOTC module drop packets when pbuf size is more than threshold count on sending.
 *
 * \param count [in] pbuf limit count, set 0 means no threshold.
 *
 */
P2PAPI_API void IOTC_Set_Pbuf_Threshold_Count(int count);
...
*			- #IOTC_ER_SERVICE_IS_NOT_STARTED The start function is not called
*/
P2PAPI_API int IOTC_Search_Device_Stop(void);
