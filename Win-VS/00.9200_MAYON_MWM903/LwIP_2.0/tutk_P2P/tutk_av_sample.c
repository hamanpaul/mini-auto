    ...
    **************************************/
    ...
    if(P2PEnableStreaming[ch] == 0)
    {
        ...
        else if(source == RX_receive)
        {
            ...
            if( gFirstConnect == 0) //APP進檢視, 無條件設定resolution為HD, 修正變數值
            {
...
//                SMsgAVIoctrlAVStream *p = (SMsgAVIoctrlAVStream *)buf;
        ...
        case IOTYPE_USER_IPCAM_GETSUPPORTSTREAM_REQ:
        {
            ...
            if(avSendIOCtrl3(avIndex, IOTYPE_USER_IPCAM_GETSUPPORTSTREAM_RESP, (char *)p, size) == AV_ER_NoERROR)
            {
                ...
                if(gRfiuUnitCntl[sysRFRxInMainCHsel].TX_PicWidth > RF_RX_2DISP_WIDTH*2) //依當前RF修正resolution
                {
...
///////////////////////////////////////////////////////////
        ...
        case IOTYPE_USER_IPCAM_GET_SCHEDULESETTING_REQ://未完成
        {
        ...
        case IOTYPE_USER_IPCAM_SET_SCHEDULESETTING_REQ://未完成
        {
...
u32 GetResendBufUsage(int avCh, int *buffering)
{
    ...
    OSSemPend(ResendBufUsageSem, 0, &err);
    if(avResendBufUsageRate(avCh) < 0.0)
    {
        OSSemPost(ResendBufUsageSem);
        if(buffering)
            *buffering = 0;
    ...
    result = ((resend_buffering_data_size*105)/TUTK_RESEND_BUF_SIZE_BYTES);
    OSSemPost(ResendBufUsageSem);
    return result;
#else
    u32 result;
    ...
    usage = avResendBufUsageRate(avCh);
    if(buffering)
        *buffering = usage*TUTK_RESEND_BUF_SIZE+0.5;
    
    result = usage*100.0+0.5;
    return result;
#endif    
}